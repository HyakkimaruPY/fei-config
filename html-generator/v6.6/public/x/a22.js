...
S.boot=async()=>{document.body.dataset.theme=cfg.theme||'graphene';$('#appTitle').textContent=cfg.appName||'Meu App';S.lockZoom();S.state.favorites=S.readFavorites();S.state.history=await S.hydrateHistory();await S.hydrateMiniUiState?.();S.reconcileContinueHidden?.();S.bindShell?.();await S.loadCatalog?.();window.dispatchEvent(new Event('srh25:ready'));setTimeout(()=>S.account(),0)};
})();
...
function miniStateDbOpen(){
  return new Promise((resolve,reject)=>{
    if(!window.indexedDB){reject(new Error('IndexedDB indisponível'));return}
    let q;try{q=indexedDB.open('srh25-history-v1',1)}catch(e){reject(e);return}
    q.onupgradeneeded=()=>{try{if(!q.result.objectStoreNames.contains('state'))q.result.createObjectStore('state')}catch{}};
    q.onsuccess=()=>resolve(q.result);q.onerror=()=>reject(q.error||new Error('IndexedDB falhou'))
  })
}
async function miniStateDbRead(key){
  try{
    const db=await miniStateDbOpen();
    return await new Promise(resolve=>{
      let tx,r;try{tx=db.transaction('state','readonly');r=tx.objectStore('state').get(key)}
      catch{resolve(null);return}
      r.onsuccess=()=>resolve(r.result==null?null:r.result);r.onerror=()=>resolve(null)
    })
  }catch{return null}
}
async function miniStateDbWrite(key,value){
  try{
    const db=await miniStateDbOpen();
    await new Promise(resolve=>{
      let tx;try{tx=db.transaction('state','readwrite');tx.objectStore('state').put(value,key)}
      catch{resolve();return}
      tx.oncomplete=()=>resolve();tx.onerror=()=>resolve();tx.onabort=()=>resolve()
    });
    return true
...
async function miniStateDbDelete(key){
  try{
    const db=await miniStateDbOpen();
    await new Promise(resolve=>{
      let tx;try{tx=db.transaction('state','readwrite');tx.objectStore('state').delete(key)}
      catch{resolve();return}
      tx.oncomplete=()=>resolve();tx.onerror=()=>resolve();tx.onabort=()=>resolve()
    })
  }catch{}
...
function normalizeMiniPosition(p){
  ...
  const side=p.side==='right'?'right':'left',top=Number(p.top);
  if(Number.isFinite(top))return {side,top:Math.max(0,top)};
  const legacyOffset=Number(p.bottomOffset);
  if(Number.isFinite(legacyOffset))return {side,bottomOffset:Math.max(0,legacyOffset)};
  const y=Number(p.y);
  if(Number.isFinite(y))return {side,y:miniClamp(y,0,1)};
  return null
}
function saveMiniPosition(p){
  const value=normalizeMiniPosition(p);if(!value)return;
  miniPositionMemory=value;
  miniWrite(MINI_POS_KEY,value);
  void miniStateDbWrite(MINI_POS_KEY,value)
}
function clearMiniDismissal(){
  miniDismissedId='';
  miniRemove(MINI_DISMISS_KEY);
  void miniStateDbDelete(MINI_DISMISS_KEY)
}
function dismissMiniResume(){
  ...
  void miniStateDbWrite(MINI_DISMISS_KEY,miniDismissedId);
  hideMiniResume()
}
S.hydrateMiniUiState=async()=>{
  const [dbPos,dbDismiss]=await Promise.all([miniStateDbRead(MINI_POS_KEY),miniStateDbRead(MINI_DISMISS_KEY)]);
  miniPositionMemory=normalizeMiniPosition(dbPos)||normalizeMiniPosition(miniRead(MINI_POS_KEY,null));
  const localDismiss=miniRead(MINI_DISMISS_KEY,'');
  miniDismissedId=String(dbDismiss||localDismiss||'');
  if(miniPositionMemory){miniWrite(MINI_POS_KEY,miniPositionMemory);void miniStateDbWrite(MINI_POS_KEY,miniPositionMemory)}
  return true
};
...
function ensureMiniStyles(){
  ...
  style.textContent=
    ...
    '.srh25-mini-resume.is-compact .srh25-mini-thumb:after{content:"";position:absolute;z-index:1;left:0;right:0;bottom:0;height:42%;background:linear-gradient(180deg,rgba(0,0,0,0),rgba(0,0,0,.12) 38%,rgba(0,0,0,.5));pointer-events:none}' +
    ...
    '.srh25-mini-resume.is-compact .srh25-mini-episode{display:block;margin:0 0 5px;text-align:right;color:rgba(255,255,255,.7);font-size:9px;font-weight:760;line-height:1;text-shadow:0 2px 7px rgba(0,0,0,.55)}' +
    ...
    '.srh25-mini-resume.is-compact .srh25-mini-progress i{opacity:.72}' +
...
function readMiniPosition(){
  const p=normalizeMiniPosition(miniPositionMemory)||normalizeMiniPosition(miniRead(MINI_POS_KEY,null));
  if(p)return p;
  return {side:'left',y:1}
}
...
function setPlayerBuffering(on){
  ...
  playPauseButton?.classList.toggle('is-buffering',active);
  if(loading)loading.classList.toggle('is-hidden',!active);
  if(active)showPlayerControls(false)
}
function toggleCenterPlayback(){
  if(video.paused){
    setPlayerBuffering(true);
    video.play().catch(()=>setPlayerBuffering(false))
  }else{
    video.pause();
...
function ensurePlayerControls(){
  ...
  if(!document.getElementById('srh25PlayerLoadingStyle')){
    const ls=document.createElement('style');ls.id='srh25PlayerLoadingStyle';
    ls.textContent='.srh25-player__loading{z-index:11!important;left:50%!important;top:calc(50% + 50px)!important;transform:translate(-50%,0)!important;display:block;place-items:center;pointer-events:none}.srh25-player__loading.is-hidden{display:none!important}.srh25-player__loading .srh25-player__spinner{display:none!important}.srh25-player__loading small{display:block;padding:5px 9px!important;background:rgba(0,0,0,.42)!important;color:rgba(255,255,255,.78);font-size:10px;line-height:1;border-radius:999px}.srh25-player__transport--main{position:relative}.srh25-player__transport--main.is-buffering:after{content:"";position:absolute;inset:-7px;border:3px solid rgba(255,255,255,.18);border-top-color:rgba(255,255,255,.92);border-radius:50%;pointer-events:none;animation:srh25CenterBufferSpin .72s linear infinite}@keyframes srh25CenterBufferSpin{to{transform:rotate(360deg)}}';
...
function shouldAutoComplete(position,duration){
  const pos=Math.max(0,Number(position)||0),dur=Number(duration);
  if(!Number.isFinite(dur)||dur<60||!Number.isFinite(pos)||pos<=0)return false;
  return dur-pos<=30
}
function persistExitSnapshot(){
  const cur=S.state.current;
  if(!cur)return false;
  const pos=Math.max(0,Number(video.currentTime)||0),dur=Number(video.duration);
  const safeDur=Number.isFinite(dur)&&dur>0?dur:0;
  const ok=S.saveProgress(cur.item,pos,safeDur);
  safeSaveMiniResume(cur.item,pos,safeDur);
  if(shouldAutoComplete(pos,safeDur))S.hideContinue(cur.item,'complete');
  else S.restoreCompletedContinue(cur.item);
  return ok
}
...
async function loadVideo(item,resume){
  const url=S.stream(item),ready=()=>{const d=Number(video.duration),p=Math.max(0,Number(resume?.position)||0);S.saveProgress(item,p,Number.isFinite(d)&&d>0?d:0);safeSaveMiniResume(item,p,Number.isFinite(d)&&d>0?d:0);setPlayerBuffering(true);if(resume?.position>0&&resume.position<video.duration-3){video.currentTime=resume.position;const done=()=>{poster.classList.add('is-hidden');video.play().catch(()=>setPlayerBuffering(false))};video.addEventListener('seeked',done,{once:true})}else{poster.classList.add('is-hidden');video.play().catch(()=>setPlayerBuffering(false))}drawArcs()};
  if(/\.m3u8(?:$|\?)/i.test(url)){
...
S.openPlayer=(item,resumeOverride)=>{
  ...
  safeSaveMiniResume(item,startPosition,startDuration);
...
function persistCurrentProgress(force=false){const cur=S.state.current,pos=Math.max(0,Number(video.currentTime)||0);if(!cur)return false;if(!force&&Math.abs(pos-lastSavedPosition)<5)return false;const ok=S.saveProgress(cur.item,pos,Number(video.duration));if(ok){lastSavedPosition=pos;safeSaveMiniResume(cur.item,pos,Number(video.duration))}return ok}
video.ontimeupdate=()=>{
  const cur=S.state.current,pos=Number(video.currentTime||0),dur=Number(video.duration);
  updatePlayerHeading(pos);
  if(cur&&shouldAutoComplete(pos,dur))S.hideContinue(cur.item,'complete');
  else if(cur)S.restoreCompletedContinue(cur.item);
  persistCurrentProgress(false);
  syncArcSelection(false)
};
video.onplay=()=>{syncPlayPauseButton();setPlayerBuffering(true);showPlayerControls(false)};
video.onplaying=()=>{setPlayerBuffering(false);syncPlayPauseButton();setTimeout(()=>{if(!video.paused)hidePlayerControls()},650)};
video.onwaiting=()=>{if(!video.paused)setPlayerBuffering(true)};
video.onstalled=()=>{if(!video.paused)setPlayerBuffering(true)};
video.onseeking=()=>{if(!video.paused)setPlayerBuffering(true)};
video.onpause=()=>{setPlayerBuffering(false);syncPlayPauseButton();if(!video.ended){persistCurrentProgress(true);showPlayerControls(false)}};
video.onended=()=>{setPlayerBuffering(false);if(S.state.current){S.saveProgress(S.state.current.item,video.currentTime,video.duration);S.hideContinue(S.state.current.item,'complete');clearMiniResume(S.state.current.item)}lastSavedPosition=0;hideMiniResume()};
...
window.addEventListener('srh25:ready',()=>{try{localStorage.setItem('srh25:last-runtime','a22-r34')}catch{}miniRemove(MINI_DISMISS_KEY);ensurePlayerControls();safeRenderMiniResume(false);setTimeout(()=>safeRenderMiniResume(false),120)},{once:true});
