...
function toggleCenterPlayback(){
  if(video.paused){
    ...
    try{
      const p=video.play();
      if(p&&typeof p.catch==='function')p.catch(()=>setPlayerBuffering(false))
    }catch{setPlayerBuffering(false)}
  }else{
    video.pause();
...
async function loadVideo(item,resume){
  ...
  const ready=()=>{
    ...
    if(p>0&&p<video.duration-3){
      ...
      const done=()=>{
        ...
        try{const q=video.play();if(q&&typeof q.catch==='function')q.catch(()=>{})}catch{}
      };
  ...
  if(/\.m3u8(?:$|\?)/i.test(url)){
    if(video.canPlayType('application/vnd.apple.mpegurl')){
      ...
      video.onerror=()=>S.toast('Mídia indisponível');
      video.load();
      tryFreshPlay();
      return
    }
    try{
      ...
      if(H?.isSupported?.()){
        ...
        h.on(H.Events.MANIFEST_PARSED,()=>{
          ...
          h.autoLevelCapping=highest;h.currentLevel=highest;h.nextLevel=highest;
          ready()
        });
        ...
        h.on(H.Events.ERROR,(_,d)=>{if(d.fatal)S.toast('Falha ao iniciar o vídeo')});
        return
      }
    }catch(e){S.toast(e?.message||'HLS indisponível')}
  }
  video.src=url;
  video.onloadedmetadata=ready;
  video.onerror=()=>S.toast('Mídia indisponível');
...
video.onplay=()=>{syncPlayPauseButton();setPlayerBuffering(true);showPlayerControls(true)};
...
window.addEventListener('srh25:ready',()=>{try{localStorage.setItem('srh25:last-runtime','a25-r37')}catch{}miniRemove(MINI_DISMISS_KEY);ensurePlayerControls();safeRenderMiniResume(false);setTimeout(()=>safeRenderMiniResume(false),120)},{once:true});
