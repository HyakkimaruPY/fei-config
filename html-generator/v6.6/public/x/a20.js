...
S.writeHistory=list=>{
  ...
  const write=limit=>{try{localStorage.setItem(histKey,JSON.stringify(rows.slice(0,limit)));return true}catch{return false}};
  if(write(80))return true;
  if(write(40))return true;
  return write(20)
};
...
S.saveProgress=(item,position,duration)=>{
  ...
  let persisted=S.readHistory();
  if(!persisted.some(x=>String(x?.id)===id)){
    try{
      localStorage.setItem(histKey,JSON.stringify([record,...persisted.filter(x=>String(x?.id)!==id)].slice(0,40)));
      persisted=S.readHistory()
    }catch{}
  }
  S.state.history=persisted;
  S.refreshLibraryView?.();
  return persisted.some(x=>String(x?.id)===id)
};
...
S.boot=async()=>{document.body.dataset.theme=cfg.theme||'graphene';$('#appTitle').textContent=cfg.appName||'Meu App';S.lockZoom();S.state.favorites=S.readFavorites();S.state.history=S.readHistory();S.reconcileContinueHidden?.();S.bindShell?.();await S.loadCatalog?.();window.dispatchEvent(new Event('srh25:ready'));setTimeout(()=>S.account(),0)};
})();
...
function ensureMiniStyles(){
  ...
  style.textContent=
    ...
    '.srh25-mini-resume.is-compact .srh25-mini-copy,.srh25-mini-resume.is-compact .srh25-mini-close,.srh25-mini-resume.is-compact .srh25-mini-play{display:none!important}' +
...
function readMiniPosition(){
  const p=miniRead(MINI_POS_KEY,{side:'left',bottomOffset:0})||{};
  const side=p.side==='right'?'right':'left';
  const offset=Number(p.bottomOffset);
  if(Number.isFinite(offset))return {side,bottomOffset:Math.max(0,offset)};
  const top=Number(p.top);
  if(Number.isFinite(top))return {side,top};
  return {side,y:miniClamp(Number(p.y==null?1:p.y),0,1)}
}
...
function placeMiniDock(force){
  ...
  let top;
  if(Number.isFinite(p.bottomOffset))top=bounds.bottom-p.bottomOffset;
  else if(Number.isFinite(p.top))top=p.top;
  else top=bounds.top+(bounds.bottom-bounds.top)*p.y;
  top=miniClamp(top,bounds.top,bounds.bottom);
  const bottomOffset=Math.max(0,bounds.bottom-top);
  miniWrite(MINI_POS_KEY,{side:p.side,bottomOffset:bottomOffset});
  miniDock.style.top='auto';
  miniDock.style.bottom=(18+bottomOffset)+'px';
  miniDock.style.right='auto';
...
function compactMiniDock(){
  if(!miniDock||miniDock.hidden)return;
  const p=readMiniPosition();
  miniDock.classList.add('is-compact');
  applyMiniDockMetrics(true);
  miniDock.style.top='auto';
  miniDock.style.bottom='18px';
  miniDock.style.right='auto';
  const size=compactMiniSize(),vw=Math.max(1,Number(window.innerWidth||document.documentElement.clientWidth||0));
  miniDock.style.left=(p.side==='right'?Math.max(12,vw-size.width-12):12)+'px';
...
function renderMiniResume(expand=false){
  ...
  miniEpisode.textContent='Episódio '+(Math.max(0,Math.floor((Number(record.position)||0)/120))+1);
...
function verifyMiniDockLayout(expand){
  ...
  if(expand&&!miniDock.classList.contains('is-compact')){
    ...
    if(miniDock.querySelector('.srh25-mini-copy'))miniDock.querySelector('.srh25-mini-copy').style.display='none';
    if(miniClose)miniClose.style.display='none'
  }
}
function finishMiniDragPosition(lastLeft,lastTop,rect,bounds){
  ...
  const bottomOffset=Math.max(0,bounds.bottom-top);
  miniWrite(MINI_POS_KEY,{side,bottomOffset});
  miniDock.style.transform='';
  miniDock.style.top='auto';
  miniDock.style.bottom=(18+bottomOffset)+'px';
  miniDock.style.right='auto';
...
async function loadVideo(item,resume){
  const url=S.stream(item),ready=()=>{const d=Number(video.duration),p=Math.max(0,Number(resume?.position)||0);S.saveProgress(item,p,Number.isFinite(d)&&d>0?d:0);safeSaveMiniResume(item,p,Number.isFinite(d)&&d>0?d:0);if(resume?.position>0&&resume.position<video.duration-3){video.currentTime=resume.position;const done=()=>{poster.classList.add('is-hidden');loading.classList.add('is-hidden');video.play().catch(()=>{})};video.addEventListener('seeked',done,{once:true})}else{poster.classList.add('is-hidden');loading.classList.add('is-hidden');video.play().catch(()=>{})}drawArcs()};
  if(/\.m3u8(?:$|\?)/i.test(url)){
...
window.addEventListener('srh25:ready',()=>{try{localStorage.setItem('srh25:last-runtime','a20-r32')}catch{}miniRemove(MINI_DISMISS_KEY);ensurePlayerControls();safeRenderMiniResume(false);setTimeout(()=>safeRenderMiniResume(false),120)},{once:true});
