...
S.boot=async()=>{document.body.dataset.theme=cfg.theme||'graphene';$('#appTitle').textContent=cfg.appName||'Meu App';S.lockZoom();S.state.favorites=S.readFavorites();S.state.history=await S.hydrateHistory();S.reconcileContinueHidden?.();S.bindShell?.();await S.loadCatalog?.();window.dispatchEvent(new Event('srh25:ready'));setTimeout(()=>S.account(),0)};
})();
...
let miniDock=null,miniOpen=null,miniImage=null,miniTitle=null,miniEpisode=null,miniProgress=null,miniClose=null,miniMemoryRecord=null;
...
function seedMiniFromHistory(){
  ...
  for(const h of history){
    ...
    const record=saveMiniResume(item,position,duration);
    if(record)return record;
    miniMemoryRecord={id:S.id(item),title:S.title(item),image:S.image(item),position,duration,updatedAt:Number(h.updatedAt||Date.now()),item:miniItemSnapshot(item)};
    return miniMemoryRecord
  }
  return null
}
...
window.addEventListener('srh25:ready',()=>{try{localStorage.setItem('srh25:last-runtime','a21-r33')}catch{}miniRemove(MINI_DISMISS_KEY);ensurePlayerControls();safeRenderMiniResume(false);setTimeout(()=>safeRenderMiniResume(false),120)},{once:true});
