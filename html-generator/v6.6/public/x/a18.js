...
function hideMiniResume(){if(miniDock){miniDock.classList.add('is-hidden');miniDock.style.display='none'}}
function ensureMiniStyles(){
  ...
  style.textContent='.srh25-mini-resume{position:fixed;z-index:94;left:12px;right:12px;bottom:16px;height:82px;display:block;box-sizing:border-box;border:1px solid rgba(255,255,255,.16);border-radius:18px;background:rgba(16,22,29,.97);box-shadow:0 18px 50px rgba(0,0,0,.42);overflow:hidden;transition:width .24s ease,height .24s ease,opacity .18s ease,transform .2s ease;touch-action:none;-webkit-user-select:none;user-select:none}.srh25-mini-resume.is-hidden{display:none!important}.srh25-mini-resume.is-dragging{transition:none!important}.srh25-mini-open{box-sizing:border-box;min-width:0;width:100%;height:100%;display:grid;grid-template-columns:66px 1fr;align-items:center;gap:10px;padding:7px 44px 7px 7px;background:transparent;color:inherit;text-align:left}.srh25-mini-thumb{position:relative;display:block;width:66px;height:68px;border-radius:12px;overflow:hidden;background:#111820}.srh25-mini-thumb img{width:100%;height:100%;display:block;object-fit:cover;background:#111820;pointer-events:none;-webkit-user-drag:none}.srh25-mini-play{position:absolute;left:50%;top:50%;width:28px;height:28px;margin-left:-14px;margin-top:-14px;display:grid;place-items:center;border:1px solid rgba(255,255,255,.28);border-radius:50%;background:rgba(0,0,0,.52)}.srh25-mini-play svg{width:13px;height:13px;fill:none;stroke:#fff;stroke-width:2;stroke-linejoin:round}.srh25-mini-copy{min-width:0;display:block}.srh25-mini-title{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:12px;font-weight:800}.srh25-mini-episode{display:block;margin-top:4px;color:#929aa5;font-size:10px;font-weight:760}.srh25-mini-progress{display:block;height:3px;margin-top:8px;border-radius:999px;overflow:hidden;background:rgba(255,255,255,.12)}.srh25-mini-progress i{display:block;width:0;height:100%;border-radius:999px;background:var(--accent,#d8dee7)}.srh25-mini-close{position:absolute;z-index:4;top:7px;right:7px;width:30px;height:30px;display:grid;place-items:center;border:1px solid rgba(255,255,255,.14);border-radius:50%;background:rgba(0,0,0,.34);color:#c7cdd6}.srh25-mini-close svg{width:16px;height:16px;fill:none;stroke:currentColor;stroke-width:1.8;stroke-linecap:round}.srh25-mini-resume.is-compact{right:auto;width:96px;height:144px;min-height:144px;border-radius:9px;background:#111;padding:0}.srh25-mini-resume.is-compact .srh25-mini-open{display:block;width:100%;height:100%;padding:0}.srh25-mini-resume.is-compact .srh25-mini-thumb{display:block;width:100%;height:100%;border-radius:0}.srh25-mini-resume.is-compact .srh25-mini-copy,.srh25-mini-resume.is-compact .srh25-mini-close,.srh25-mini-resume.is-compact .srh25-mini-play{display:none!important}@media(min-width:700px){.srh25-mini-resume.is-compact{width:120px;height:180px;min-height:180px}}';
...
function ensureMiniDock(){
  ...
  miniDock.style.zIndex='94';
  miniDock.style.left='12px';
  miniDock.style.right='auto';
  miniDock.style.bottom='16px';
  miniDock.style.width='96px';
  miniDock.style.height='144px';
  miniDock.style.display='none';
  miniDock.style.overflow='hidden';
  ...
  if(window.PointerEvent)miniDock.addEventListener('pointerdown',startMiniDrag);else miniDock.addEventListener('touchstart',startMiniTouch,{passive:true});
}
function miniViewport(){const v=window.visualViewport;return {width:Number(v&&v.width)||window.innerWidth,height:Number(v&&v.height)||window.innerHeight}}
...
function applyMiniDockMetrics(compact){
  ...
  if(compact){
    ...
    miniDock.style.height='82px';
    miniDock.style.minHeight='82px'
  }
}
function pinMiniDock(){
  if(!miniDock)return;
...
function startMiniTouch(event){
  ...
  const rect=miniDock.getBoundingClientRect(),bounds=miniBounds(rect);
  ...
  const finish=()=>{
    ...
    if(!moved){miniDock.style.transform='';return}
    miniSuppressOpen=true;clearTimeout(miniSuppressTimer);miniSuppressTimer=setTimeout(()=>{miniSuppressOpen=false},240);
    const side=lastLeft+rect.width/2<=bounds.center?'left':'right';
    const top=miniClamp(lastTop,bounds.top,bounds.bottom),span=Math.max(1,bounds.bottom-bounds.top);
    miniWrite(MINI_POS_KEY,{side:side,y:miniClamp((top-bounds.top)/span,0,1)});
    miniDock.style.transform='';
...
window.addEventListener('resize',()=>{if(miniDock&&!miniDock.classList.contains('is-hidden')){if(miniDock.classList.contains('is-compact'))applyMiniDockMetrics(true);placeMiniDock()}},{passive:true});
...
window.addEventListener('srh25:ready',()=>{miniRemove(MINI_DISMISS_KEY);ensurePlayerControls();renderMiniResume(false);setTimeout(()=>renderMiniResume(false),120)},{once:true});
