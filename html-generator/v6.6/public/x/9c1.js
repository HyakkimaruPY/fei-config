...
function saveMiniResume(item,position,duration){
  ...
  if(!item||!Number.isFinite(pos)||pos<3)return null;
  if(Number.isFinite(dur)&&dur>0&&pos/dur>=.97){clearMiniResume(item);return null}
...
function stop(){const cur=S.state.current,pos=Number(video.currentTime||0),dur=Number(video.duration||0);if(cur){S.saveProgress(cur.item,pos,dur);saveMiniResume(cur.item,pos,dur)}closeArcs();destroyHls();video.pause();video.removeAttribute('src');video.load();S.state.current=null;player.classList.add('is-hidden');renderMiniResume(true)}
...
video.onended=()=>{if(S.state.current){clearMiniResume(S.state.current.item);S.removeHistory(S.state.current.item)}lastSavedPosition=0;hideMiniResume()};
...
window.addEventListener('srh25:ready',()=>renderMiniResume(false),{once:true});
