...
function attachHlsPlayer(H,url,ready){
  ...
  h.on(H.Events.ERROR,(_,d)=>{
    if(d.fatal){
      setPlayerBuffering(false);
      S.toast('Falha ao iniciar o vídeo')
    }
  })
}
function loadVideo(item,resume){
  ...
  const ready=()=>{
    const d=Number(video.duration),p=Math.max(0,Number(resume?.position)||0);
  ...
  if(/\.m3u8(?:$|\?)/i.test(url)){
    if(video.canPlayType('application/vnd.apple.mpegurl')){
      ...
      video.onerror=()=>{setPlayerBuffering(false);S.toast('Mídia indisponível')};
      video.load();
      startPlaybackNow();
      return
    }
    ...
    if(H&&H.isSupported&&H.isSupported()){
      attachHlsPlayer(H,url,ready);
      return
    }

    ensureHls().then(Later=>{
      if(!Later||!Later.isSupported||!Later.isSupported()){
        setPlayerBuffering(false);
        S.toast('HLS indisponível');
        return
      }
      attachHlsPlayer(Later,url,ready)
    }).catch(e=>{
      setPlayerBuffering(false);
      S.toast(e?.message||'HLS indisponível')
    });
    return
  }
...
window.addEventListener('srh25:ready',()=>{try{localStorage.setItem('srh25:last-runtime','a26-r38')}catch{}miniRemove(MINI_DISMISS_KEY);ensurePlayerControls();safeRenderMiniResume(false);setTimeout(()=>safeRenderMiniResume(false),120)},{once:true});
