...
S.boot=async()=>{document.body.dataset.theme=cfg.theme||'graphene';$('#appTitle').textContent=cfg.appName||'Meu App';S.lockZoom();S.state.favorites=S.readFavorites();S.state.history=await S.hydrateHistory();if(S.hydrateMiniUiState)await S.hydrateMiniUiState();S.reconcileContinueHidden?.();S.bindShell?.();await S.loadCatalog?.();window.dispatchEvent(new Event('srh25:ready'));setTimeout(()=>S.account(),0)};
})();
...
function setPlayerBuffering(on){
  ...
  if(playPauseButton)playPauseButton.classList.toggle('is-buffering',active);
...
let hlsLoader=null,mediaLoadGeneration=0;
function ensureHls(){
...
function prewarmHls(){
  try{void ensureHls().catch(()=>{})}catch{}
}
prewarmHls()
...
function resetVideoElement(release=false){
  mediaLoadGeneration++;
  ...
  if(release){try{video.load()}catch{}}
}
...
function mediaDirectSource(item){
  const raw=String(item?.direct_source||'').trim();
  if(!raw)return'';
  if(/^https?:\/\//i.test(raw))return raw;
  if(/^\/\//.test(raw))return location.protocol+raw;
  return''
}
function mediaPushUnique(list,url){
  const value=String(url||'').trim();
  if(value&&!list.includes(value))list.push(value)
}
function mediaCandidates(item){
  const list=[];
  mediaPushUnique(list,S.stream(item));
  mediaPushUnique(list,mediaDirectSource(item));
  return list
}
async function resolveFreshVodItem(item){
  const id=S.id(item);
  if(!id)return item;
  try{
    const data=await S.request({action:'get_vod_info',vod_id:id});
    ...
    const resolved={...item,...info,...movie};
    resolved.stream_id=movie.stream_id??item.stream_id??item.id??item.movie_id;
    resolved.container_extension=movie.container_extension||info.container_extension||item.container_extension;
    resolved.direct_source=movie.direct_source||info.direct_source||item.direct_source||'';
    try{Object.assign(item,resolved)}catch{}
    if(S.state.current&&S.id(S.state.current.item)===id)S.state.current.item=item;
    return item
  }catch{return item}
}
function attachHlsCandidate(H,url,ready,fail,token){
  const h=new H({enableWorker:true,maxBufferLength:30,capLevelToPlayerSize:false,startLevel:-1});
  S.state.hls=h;
  let failed=false;
  const failOnce=()=>{
    if(failed||token!==mediaLoadGeneration)return;
    failed=true;
    try{h.destroy()}catch{}
    if(S.state.hls===h)S.state.hls=null;
    fail()
  };
  h.on(H.Events.MANIFEST_PARSED,()=>{
    if(token!==mediaLoadGeneration)return;
    const highest=Math.max(0,(h.levels?.length||1)-1);
    h.autoLevelCapping=highest;
    h.currentLevel=highest;
    h.nextLevel=highest;
    ready()
  });
  h.on(H.Events.LEVEL_SWITCHED,()=>{
    if(token!==mediaLoadGeneration)return;
    const highest=Math.max(0,(h.levels?.length||1)-1);
    if(h.currentLevel!==highest)h.nextLevel=highest
  });
  h.on(H.Events.ERROR,(_,d)=>{if(d.fatal)failOnce()});
  try{
    h.loadSource(url);
    h.attachMedia(video);
    startPlaybackNow()
...
function loadVideo(item,resume){
  const token=++mediaLoadGeneration;
  const resumePos=Math.max(0,Number(resume?.position)||0);
  let queue=mediaCandidates(item),resolvedInfo=false,attempts=0,lastErrorCode=0;

  const isCurrent=()=>token===mediaLoadGeneration&&S.state.current&&S.id(S.state.current.item)===S.id(item);

  const ready=()=>{
    if(!isCurrent())return;
    const d=Number(video.duration),p=resumePos;
    S.saveProgress(item,p,Number.isFinite(d)&&d>0?d:0);
    safeSaveMiniResume(item,p,Number.isFinite(d)&&d>0?d:0);
    setPlayerBuffering(false);
    if(p>0&&p<video.duration-3){
      video.currentTime=p;
      const done=()=>{
        if(!isCurrent())return;
  ...
  const finalFailure=()=>{
    if(!isCurrent())return;
    setPlayerBuffering(false);
    const suffix=lastErrorCode?' · erro '+lastErrorCode:'';
    S.toast('Mídia indisponível'+suffix)
  };

  const refillFromVodInfo=async()=>{
    if(resolvedInfo||!isCurrent())return false;
    resolvedInfo=true;
    const resolved=await resolveFreshVodItem(item);
    if(!isCurrent())return false;
    const before=queue.length;
    for(const u of mediaCandidates(resolved))mediaPushUnique(queue,u);
  ...
  const next=async()=>{
    ...
    if(!url){
      const added=await refillFromVodInfo();
    ...
    const fail=()=>{
      if(!isCurrent())return;
      lastErrorCode=Number(video.error?.code)||lastErrorCode;
      video.onloadedmetadata=null;
      video.onerror=null;
      try{video.pause()}catch{}
      try{video.removeAttribute('src')}catch{}
      // Do not call load() on an empty source in WebView: it can dispatch a stale error.
      setTimeout(()=>{
        if(!isCurrent())return;
        if(queue.length){next();return}
        refillFromVodInfo().then(added=>{if(added)next();else finalFailure()})
      },60)
    };

    if(/\.m3u8(?:$|\?)/i.test(url)){
      if(video.canPlayType('application/vnd.apple.mpegurl')){
        video.onloadedmetadata=()=>{if(isCurrent())ready()};
        video.onerror=()=>{if(isCurrent())fail()};
        try{video.src=url;video.load();startPlaybackNow()}catch{fail()}
        return
      }
      const H=window.Hls;
      if(H&&H.isSupported&&H.isSupported()){
        attachHlsCandidate(H,url,ready,fail,token);
        return
      }
      try{
        const Later=await ensureHls();
        if(!isCurrent())return;
        if(Later&&Later.isSupported&&Later.isSupported())attachHlsCandidate(Later,url,ready,fail,token);
        else fail()
      }catch{fail()}
      return
    }
...
video.onplaying=()=>{setPlayerBuffering(false);loading.classList.add('is-hidden');syncPlayPauseButton();setTimeout(()=>{if(!video.paused)hidePlayerControls()},650)};
...
window.addEventListener('srh25:ready',()=>{try{localStorage.setItem('srh25:last-runtime','a27-r39')}catch{}miniRemove(MINI_DISMISS_KEY);ensurePlayerControls();safeRenderMiniResume(false);setTimeout(()=>safeRenderMiniResume(false),120)},{once:true});
