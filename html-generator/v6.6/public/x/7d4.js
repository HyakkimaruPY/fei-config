...
let standardDbPromise=null,standardWriteTail=Promise.resolve();
...
function standardWriteLock(task){const run=async()=>navigator.locks?.request?navigator.locks.request('srhell-standard:'+STANDARD_APP_NS,{mode:'exclusive'},task):task();const next=standardWriteTail.catch(()=>{}).then(run);standardWriteTail=next.catch(()=>{});return next}
...
async function standardStateSet(key,value){return standardWriteLock(async()=>{try{const db=await standardDbOpen();return await new Promise(resolve=>{let tx;try{tx=db.transaction(STANDARD_STORE,'readwrite');tx.objectStore(STANDARD_STORE).put(value,key)}catch{resolve(false);return}tx.oncomplete=()=>resolve(true);tx.onerror=()=>resolve(false);tx.onabort=()=>resolve(false)})}catch{return false}})}
async function standardStateDelete(key){return standardWriteLock(async()=>{try{const db=await standardDbOpen();return await new Promise(resolve=>{let tx;try{tx=db.transaction(STANDARD_STORE,'readwrite');tx.objectStore(STANDARD_STORE).delete(key)}catch{resolve(false);return}tx.oncomplete=()=>resolve(true);tx.onerror=()=>resolve(false);tx.onabort=()=>resolve(false)})}catch{return false}})}
...
const liveLogoProbeCache=new Map(),liveLogoProbeInflight=new Map();
...
async function inspectLiveLogoUrl(src,timeout=3400){
  const raw=String(src||'').trim(),meta=liveLogoUrlMeta(raw);
  if(!meta.valid)return{ok:false,stage:'url',reason:'url-invalida',...meta};
  if(liveLogoProbeCache.has(raw))return liveLogoProbeCache.get(raw);
  if(liveLogoProbeInflight.has(raw))return liveLogoProbeInflight.get(raw);
  const job=(async()=>{
  ...
  })().then(result=>{liveLogoProbeCache.set(raw,result);if(liveLogoProbeCache.size>80)liveLogoProbeCache.delete(liveLogoProbeCache.keys().next().value);return result}).finally(()=>liveLogoProbeInflight.delete(raw));
  liveLogoProbeInflight.set(raw,job);return job
}
...
async function applyLiveLogoFallback(img,baseReason){
  if(!img||img.dataset.srhLiveLogoFallback==='1'||img.dataset.srhLiveLogoDiagnosing==='1')return;
  img.dataset.srhLiveLogoDiagnosing='1';
  const src=String(img.currentSrc||img.src||''),diagnostic=await inspectLiveLogoUrl(src).catch(e=>({ok:false,reason:'diagnostic-error',message:e?.message||String(e)}));
...
function installLiveLogoPipeline(){
  ...
  document.addEventListener('load',e=>{
    ...
    if(!img.naturalWidth||!img.naturalHeight){void applyLiveLogoFallback(img,'dimensoes-invalidas');return}
    ...
    const decodeStarted=performance.now();Promise.resolve(img.decode?.()).then(()=>undefined).catch(e=>undefined);
    requestAnimationFrame(()=>{if(!img.isConnected)return;const r=img.getBoundingClientRect();if(r.width<2||r.height<2)void applyLiveLogoFallback(img,'erro-renderizacao')});
  ...
  document.addEventListener('error',e=>{
    ...
    void applyLiveLogoFallback(img,reason)
  },true)
}
function installPlaybackVisualCompatibility(){
  if(document.getElementById('srhPlaybackVisualCompatibility'))return;
  const style=document.createElement('style');style.id='srhPlaybackVisualCompatibility';
...
function registerMediaResource(url){return mediaOrigin(url)}
function warmMediaOrigin(url){try{const origin=registerMediaResource(url);if(!origin||origin==='null'||origin==='media'||warmedMediaOrigins.has(origin))return;warmedMediaOrigins.add(origin);for(const rel of ['dns-prefetch','preconnect']){const link=document.createElement('link');link.rel=rel;link.href=origin;document.head.appendChild(link)}undefined}catch{}}
...
function rememberOnDemandFormat(url,type){const ext=mediaExtension(url),key=onDemandPreferenceKey(url,type);if(!key.startsWith('|')&&ext){const prior=onDemandFormatPreference.get(key)||'';onDemandFormatPreference.set(key,ext);if(prior!==ext)undefined}}
function noteOnDemandHlsFailure(url,type,details=''){if(type==='live')return;const key=onDemandPreferenceKey(url,type),now=Date.now(),prior=onDemandHlsHealth.get(key),fresh=prior&&now-prior.lastAt<15*60*1000,row={count:fresh?prior.count+1:1,lastAt:now};onDemandHlsHealth.set(key,row);undefined}
function rememberOnDemandHlsSuccess(url,type){if(type==='live')return;const key=onDemandPreferenceKey(url,type);if(onDemandHlsHealth.has(key)){onDemandHlsHealth.delete(key);undefined}}
function preferOnDemandSources(values,type,pinned='',pinnedStartupMs=0){const list=uniqueMediaUrls(Array.isArray(values)?values:[values]);if(type==='live'||list.length<2)return list;const pin=String(pinned||'').trim(),base=pin?list.filter(x=>x!==pin):list.slice(),probe=base[0]||pin,pref=probe?onDemandFormatPreference.get(onDemandPreferenceKey(probe,type)):'';if(pref&&pref!=='m3u8')base.sort((a,b)=>(mediaExtension(b)===pref?1:0)-(mediaExtension(a)===pref?1:0));base.sort((a,b)=>(mediaExtension(a)==='m3u8'?1:0)-(mediaExtension(b)==='m3u8'?1:0));if(pin)undefined;return uniqueMediaUrls(pin?[pin,...base]:base)}
...
async function persistProgress(video=el.video,opts=null){
  ...
  if(video?.dataset?.srhResumeState==='pending'||video?.dataset?.srhResumeState==='applying'){undefined;return}
...
async function srhReleaseWakeLock(reason='release'){
  ...
  if(lock){try{await lock.release?.()}catch{}undefined}
}
async function srhAcquireWakeLock(video){
  ...
  if(!navigator.wakeLock?.request){
    if(!srhWakeLockUnsupportedLogged){srhWakeLockUnsupportedLogged=true;undefined}
    return false
  }
  try{
    ...
    return true
  }catch(e){undefined;return false}
}
...
async function enterFullscreen(node){
  try{if(!srhFullscreenElement()){if(node.requestFullscreen)await node.requestFullscreen();else if(node.webkitRequestFullscreen)await node.webkitRequestFullscreen()}}catch(e){undefined}
...
async function loadSeriesDetailStable(item,{force=false}={}){
  ...
  if(cached&&!force){undefined;return cloneDetailPayload(cached.data)}
  ...
  const job=(async()=>{
    try{
      ...
      if(cached){undefined;return cloneDetailPayload(cached.data)}
      throw e
    }
...
async function renderSeriesRecoveryModal(item,error,token){
  try{
    ...
    if(retry)retry.onclick=e=>{e.stopPropagation();retry.disabled=true;retry.textContent='Carregando…';const retryStarted=performance.now();undefined;void openSeries(item,{forceEpisodes:true,retrySource:'safe-shell'}).then(()=>{undefined}).catch(err=>{retry.disabled=false;retry.textContent='Tentar carregar episódios';undefined})};
    return true
  }catch(shellError){
    return false
  }
...
setUpdateAccordionIcon(el.updateAccordionBody?.classList.contains('is-hidden')!==false);el.settingsButton.onclick=()=>{const opening=el.settingsPanel.classList.contains('is-hidden');el.settingsPanel.classList.toggle('is-hidden');if(opening)void refreshAccount().catch(e=>undefined)};el.updateAccordionButton.onclick=()=>{const hidden=el.updateAccordionBody.classList.toggle('is-hidden');setUpdateAccordionIcon(hidden)};el.updateLoad.onclick=()=>{void loadUpdateCategories().catch(e=>undefined)};el.updateApply.onclick=()=>{void applyUpdate().catch(e=>undefined)};
document.addEventListener('fullscreenchange',()=>{if(!document.fullscreenElement&&isMobile())screen.orientation?.lock?.('portrait')?.catch?.(()=>{})});
function init(){installImageFallback();installLiveLogoPipeline();installPlaybackVisualCompatibility();document.body.dataset.theme=CONFIG.theme||'graphene';document.title=CONFIG.appName;el.title.textContent=CONFIG.appName;const types=selectedTypes();if(!types.length){el.homeStatus.textContent='Nenhuma categoria configurada.';return}state.activeType=types[0];renderActiveType();refreshAccount()}
...
async function srhFixStopInlineDetailVideo(restoreImage=true){
  ...
  const teardown=beginPlaybackVideoTeardown(iv.video,'inline-close',e=>undefined);
...
async function srhFixInlinePlayer(url,title,entry=null){
  ...
  actions.querySelector('[data-inline-close]').onclick=e=>{
    e.stopPropagation();
    void srhFixStopInlineDetailVideo(true).catch(err=>undefined);
  };
...
async function resolveModalDetailPackage(type,item,providerData){
  ...
  let tmdb=null;
  try{tmdb=await resolveCatalogMetadata?.(kind,item,providerData)}catch(e){undefined}
  const candidates=[
...
(function installLiteUiOrganization(){
  ...
  function preloadContinueArt(url,timeout=5200){
    ...
    return new Promise(resolve=>{
      ...
      const finish=ok=>{if(done)return;done=true;clearTimeout(timer);img.onload=null;img.onerror=null;resolve(!!ok)};
      const timer=setTimeout(()=>finish(false),timeout);
      img.onload=()=>{const valid=img.naturalWidth>0&&img.naturalHeight>0&&!continueArtLooksBlank(img);if(!valid)undefined;finish(valid)};
  ...
  function renderContinueCardsInto(section,row,type,list,{onBeforeOpen=null}={}){
    ...
    row.querySelectorAll('[data-history]').forEach(card=>{
      ...
      card.onclick=()=>{
        ...
        void openContinueEntry(x,type).catch(e=>{undefined;toast('Não foi possível abrir este item.')})
      };
...
(function installTmdbEnrichment(){
  ...
  try{for(let i=localStorage.length-1;i>=0;i--){const k=localStorage.key(i)||'';if(/^srhell:tmdb:cache:/i.test(k)&&k!==CACHE_STORE)localStorage.removeItem(k)}}catch{}
    ...
    const job=(async()=>{
      ...
      for(const url of urls){
        ...
        try{
          ...
          if(detailResponseCache.size>24)detailResponseCache.delete(detailResponseCache.keys().next().value);
          if(ms>2600)undefined;
          return cloneDetailPayload(data)
        }catch(e){last=e;undefined}
      }
      const ms=Math.round(performance.now()-started);
      if(cached&&age<DETAIL_STALE_TTL){
        return cloneDetailPayload(cached.data)
...
(function installMainStreamStyle(){
  ...
  async function buildHero(token){
    ...
    if(type==='series'&&seriesProviderGuardBlocked()){node.classList.add('is-hidden');undefined;return}
...
(function installStandardModalGuards(){
  if(el.detailClose){
    let closing=false;
    el.detailClose.onclick=async e=>{
      ...
      if(closing)return;
      closing=true;
      try{await closeDetail()}catch(err){undefined}finally{closing=false}
    };
  }
  let scrollTimer=0;
...
Promise.resolve().then(()=>hydrateStandardRuntime()).then(()=>window.__srhHydrateStandardPersonal?.()).then(()=>hydrateAccountExpiryCache()).then(()=>window.__srhPrimeStoredHeroReserves?.()).catch(e=>undefined).finally(()=>{undefined;init();setTimeout(()=>ensureHls().then(()=>undefined).catch(e=>undefined),700);setTimeout(()=>{const run=()=>window.__srhMaintainHeroReserves?.();if(typeof requestIdleCallback==='function')requestIdleCallback(run,{timeout:4500});else run()},2200)});
})();
})();
