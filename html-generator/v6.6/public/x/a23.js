...
function saveMiniPosition(p){
  ...
  void miniUiDbWrite(MINI_POS_KEY,value)
}
...
S.hydrateMiniUiState=async()=>{
  const dbPos=await miniUiDbRead(MINI_POS_KEY);
  const dbDismiss=await miniUiDbRead(MINI_DISMISS_KEY);
  miniPositionMemory=normalizeMiniPosition(dbPos)
    ||normalizeMiniPosition(miniRead(MINI_POS_KEY,null))
    ||normalizeMiniPosition(miniRead(MINI_POS_LEGACY_KEY,null));
  miniDismissedId=String(dbDismiss||miniRead(MINI_DISMISS_KEY,'')||'');
  if(miniPositionMemory){
    miniWrite(MINI_POS_KEY,miniPositionMemory);
    void miniUiDbWrite(MINI_POS_KEY,miniPositionMemory)
  }
  return true
};
...
function readMiniPosition(){
  return normalizeMiniPosition(miniPositionMemory)
    ||normalizeMiniPosition(miniRead(MINI_POS_KEY,null))
    ||normalizeMiniPosition(miniRead(MINI_POS_LEGACY_KEY,null))
    ||{side:'left',y:1}
}
...
window.addEventListener('srh25:ready',()=>{try{localStorage.setItem('srh25:last-runtime','a23-r35')}catch{}miniRemove(MINI_DISMISS_KEY);ensurePlayerControls();safeRenderMiniResume(false);setTimeout(()=>safeRenderMiniResume(false),120)},{once:true});
