...
function arcCount(seconds){return seconds>0?Math.max(1,Math.ceil(seconds/120)):0}
...
function drawArcs(){const d=el.shortVideo.duration;if(!Number.isFinite(d)||d<=0){el.arcGrid.innerHTML='<div class="update-status">Duração indisponível.</div>';return}const count=Math.max(1,Math.ceil(d/120)),current=Math.min(count-1,Math.floor(el.shortVideo.currentTime/120));el.arcHead.textContent=count+' arco(s) · 2 min cada';el.arcGrid.innerHTML=Array.from({length:count},(_,i)=>`<button class="arc-button${i===current?' is-active':''}" data-arc="${i}">Arco ${i+1}<br><small>${Math.floor(i*2)} min</small></button>`).join('');el.arcGrid.querySelectorAll('[data-arc]').forEach(b=>b.onclick=e=>{e.stopPropagation();seekArc(Number(b.dataset.arc));el.arcDrawer.classList.add('is-hidden')})}
function seekArc(index){const d=el.shortVideo.duration;if(!Number.isFinite(d)||d<=0)return;const count=Math.max(1,Math.ceil(d/120)),i=Math.max(0,Math.min(count-1,index));el.shortVideo.currentTime=Math.min(i*120,Math.max(0,d-.25));el.shortVideo.play().catch(()=>{});drawArcs()}
