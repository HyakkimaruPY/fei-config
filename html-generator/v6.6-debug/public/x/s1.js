...
function preferOnDemandSources(values,type,pinned=''){const list=uniqueMediaUrls(Array.isArray(values)?values:[values]);if(type==='live'||list.length<2)return list;const pin=String(pinned||'').trim(),base=pin?list.filter(x=>x!==pin):list.slice(),probe=base[0]||pin,pref=probe?onDemandFormatPreference.get(onDemandPreferenceKey(probe,type)):'';if(pref)base.sort((a,b)=>(mediaExtension(b)===pref?1:0)-(mediaExtension(a)===pref?1:0));return uniqueMediaUrls([pin,...base])}
...
function attachVideo(video,url,onReady,onError){
 ...
 const start=async(current,attempt)=>{
  ...
  const markReady=()=>{if(!alive())return;clearTimeout(metadataTimer);clearTimeout(liveStartupTimer);const mediaId=String(video.dataset.srhMediaId||state.currentMedia?.key||''),mediaType=String(video.dataset.srhMediaType||state.currentMedia?.type||''),sourceKind=String(video.dataset.srhSourceKind||kind);if(state.currentMedia&&mediaType!=='live'){state.currentMedia.lastWorkingUrl=current;state.currentMedia.containerExtension=mediaExtension(current)||state.currentMedia.containerExtension||'';rememberOnDemandFormat(current,mediaType)}lastAdvanceAt=Date.now();markAdvance();try{window.SRHDebug?.noteRecovery?.({kind:'player.stall',layer:'media',origin:mediaOrigin(current),mediaId,status:200,message:'Reprodução retomada',details:{kind,sourceKind,currentTime:Number(video.currentTime)||0}})}catch{}applyResume();if(failedOrigins.size){for(const origin of failedOrigins){try{const recovery={layer:'media',origin,status:200,message:'Fonte alternativa iniciou a reprodução',details:{fallbackCount,sourceKind,winningOrigin:mediaOrigin(current)}};window.SRHDebug?.noteRecovery?.({kind:'player.error',mediaId,...recovery});window.SRHDebug?.noteRecovery?.({kind:'player.error',mediaId:'',...recovery});window.SRHDebug?.noteRecovery?.({kind:'player.stall',mediaId,...recovery});window.SRHDebug?.noteRecovery?.({kind:'playlist',layer:'fetch',origin,status:200,message:'Fonte alternativa iniciou a reprodução',details:{fallbackCount,sourceKind,winningOrigin:mediaOrigin(current)}})}catch{}}playbackDebug('recovered',{mediaId,mediaType,sourceKind,fallbackCount,failedOrigins:[...failedOrigins],winningOrigin:mediaOrigin(current)});failedOrigins.clear()}if(!readyNotified){readyNotified=true;onReady?.(current)}playbackDebug('ready',{kind,sourceKind,mediaId,attempt,live:isLive(),preload:video.preload});const p=video.play();if(p&&typeof p.catch==='function')p.catch(()=>{})};
  const fail=extra=>{
...
function srhFixOverlayPlayer(url,title,entry=null){
  const rawSources=mediaCandidates(entry?.lastWorkingUrl?[entry.lastWorkingUrl,...(Array.isArray(entry?.sources)?entry.sources:[entry?.sources||url])]:entry?.sources||url),sources=preferOnDemandSources(rawSources,entry?.type||'live',entry?.lastWorkingUrl||'');
