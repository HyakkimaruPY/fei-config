...
function attachVideo(video,url,onReady,onError){
 ...
 const start=async(current,attempt)=>{
  ...
  lastUrl=current;video.pause();video.onerror=null;video.onloadedmetadata=null;video.dataset.srhMediaOrigin=registerMediaResource(current);video.dataset.srhMediaKind=kind;video.dataset.srhMediaId=String(state.currentMedia?.key||state.currentMedia?.mediaId||'');video.removeAttribute('src');video.preload='auto';try{video.load()}catch{}warmMediaOrigin(current)
  ...
  const applyResume=()=>{if(resumeAt>1&&Number.isFinite(video.duration)&&video.duration>0&&resumeAt<video.duration-3&&Math.abs((Number(video.currentTime)||0)-resumeAt)>1){try{video.currentTime=resumeAt;playbackDebug('resume-position',{position:Math.round(resumeAt*1000)/1000,kind})}catch{}}};
  const markReady=()=>{if(!alive())return;clearTimeout(metadataTimer);lastAdvanceAt=Date.now();markAdvance();const incidentMediaId=String(video.dataset.srhMediaId||'');try{window.SRHDebug?.noteRecovery?.({kind:'player.stall',layer:'media',origin:mediaOrigin(current),mediaId:incidentMediaId,status:200,message:'Reprodução retomada',details:{kind,currentTime:Number(video.currentTime)||0}});if(incidentMediaId)window.SRHDebug?.noteRecovery?.({kind:'player.stall',layer:'media',origin:mediaOrigin(current),mediaId:'',status:200,message:'Reprodução retomada',details:{kind,currentTime:Number(video.currentTime)||0}})}catch{}applyResume();if(failedOrigins.size){for(const origin of failedOrigins){try{window.SRHDebug?.noteRecovery?.({kind:'player.error',layer:'media',origin,mediaId:incidentMediaId,status:200,message:'Fonte alternativa iniciou a reprodução',details:{fallbackCount,winningOrigin:mediaOrigin(current)}});window.SRHDebug?.noteRecovery?.({kind:'player.stall',layer:'media',origin,mediaId:incidentMediaId,status:200,message:'Fonte alternativa retomou a reprodução',details:{fallbackCount,winningOrigin:mediaOrigin(current)}});if(incidentMediaId){window.SRHDebug?.noteRecovery?.({kind:'player.error',layer:'media',origin,mediaId:'',status:200,message:'Fonte alternativa iniciou a reprodução',details:{fallbackCount,winningOrigin:mediaOrigin(current)}});window.SRHDebug?.noteRecovery?.({kind:'player.stall',layer:'media',origin,mediaId:'',status:200,message:'Fonte alternativa retomou a reprodução',details:{fallbackCount,winningOrigin:mediaOrigin(current)}})}}catch{}}playbackDebug('recovered',{fallbackCount,failedOrigins:[...failedOrigins],winningOrigin:mediaOrigin(current)});failedOrigins.clear()}if(!readyNotified){readyNotified=true;onReady?.(current)}playbackDebug('ready',{kind,attempt,live:isLive(),preload:video.preload});const p=video.play();if(p&&typeof p.catch==='function')p.catch(()=>{})};
