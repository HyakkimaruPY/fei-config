...
function srhFixOverlayPlayer(url,title,entry=null){
  ...
  el.video.onended=()=>{const cur=state.currentMedia;if(!cur)return;if(cur.type==='vod'){markStandardTitleCompleted(cur,video.duration);clearContinueFrame(cur);removeContinueForEntry(cur)}else if(cur.type==='series')finalizeSeriesEpisode(cur,video.duration)}
}
