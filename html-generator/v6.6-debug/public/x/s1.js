...
const standardStoragePerf={reads:0,writes:0,deletes:0,slow:0,maxReadMs:0,maxWriteMs:0,maxDeleteMs:0,maxQueueMs:0};
const standardCompletedMemory={},standardLastWatchedMemory={vod:null,series:null};
function standardStorageObserve(op,key,started,ok=true){
  const ms=Math.max(0,performance.now()-started),metric=op==='get'?'Read':op==='set'?'Write':'Delete',countKey=op==='get'?'reads':op==='set'?'writes':'deletes',maxKey='max'+metric+'Ms';
  standardStoragePerf[countKey]++;standardStoragePerf[maxKey]=Math.max(Number(standardStoragePerf[maxKey])||0,ms);
  const threshold=op==='get'?80:120;
  if(ms>=threshold){standardStoragePerf.slow++;try{window.SRHDebug?.log?.('info','standard.storage.slow',{op,key:String(key||'').slice(0,120),ms:Math.round(ms),ok,queuedWrites:Math.max(0,standardWritesQueued-standardWritesDone)})}catch{}}
  return ms
}
...
function standardWriteLock(task){standardWritesQueued++;const queuedAt=performance.now();const run=async()=>{const queueMs=Math.max(0,performance.now()-queuedAt);standardStoragePerf.maxQueueMs=Math.max(standardStoragePerf.maxQueueMs,queueMs);try{if(navigator.locks?.request)return await navigator.locks.request('srhell-standard:'+STANDARD_APP_NS,{mode:'exclusive'},task);return await task()}finally{standardWritesDone++}};const next=standardWriteTail.catch(()=>{}).then(run);standardWriteTail=next.catch(()=>{});return next}
async function standardStateGet(key){const started=performance.now();let value=null,ok=false;try{const db=await standardDbOpen();value=await new Promise(resolve=>{let r;try{r=db.transaction(STANDARD_STORE,'readonly').objectStore(STANDARD_STORE).get(key)}catch{resolve(null);return}r.onsuccess=()=>resolve(r.result??null);r.onerror=()=>resolve(null)});ok=true;return value}catch{return null}finally{standardStorageObserve('get',key,started,ok)}}
async function standardStateSet(key,value){const started=performance.now();let ok=false;try{ok=await standardWriteLock(async()=>{try{const db=await standardDbOpen();return await new Promise(resolve=>{let tx;try{tx=db.transaction(STANDARD_STORE,'readwrite');tx.objectStore(STANDARD_STORE).put(value,key)}catch{resolve(false);return}tx.oncomplete=()=>resolve(true);tx.onerror=()=>resolve(false);tx.onabort=()=>resolve(false)})}catch{return false}});return ok}finally{standardStorageObserve('set',key,started,ok)}}
async function standardStateDelete(key){const started=performance.now();let ok=false;try{ok=await standardWriteLock(async()=>{try{const db=await standardDbOpen();return await new Promise(resolve=>{let tx;try{tx=db.transaction(STANDARD_STORE,'readwrite');tx.objectStore(STANDARD_STORE).delete(key)}catch{resolve(false);return}tx.oncomplete=()=>resolve(true);tx.onerror=()=>resolve(false);tx.onabort=()=>resolve(false)})}catch{return false}});return ok}finally{standardStorageObserve('delete',key,started,ok)}}
...
window.__SRH_STANDARD_STORAGE__={backend:'indexeddb',db:STANDARD_DB,stats:()=>({queuedWrites:Math.max(0,standardWritesQueued-standardWritesDone),writesQueued:standardWritesQueued,writesDone:standardWritesDone,webLocks:!!navigator.locks?.request,...standardStoragePerf})};
const state={activeType:null,categoryMaps:new Map(),railInstances:[],gridInstance:null,categoryObserver:null,retentionObserver:null,renderToken:0,detailToken:0,seriesPlayToken:0,hls:null,currentMedia:null,currentDetail:null,currentSeries:null,seriesVideo:null,saveTick:0,playerActive:false,playbackLifecycle:'idle',playbackLifecycleAt:0,playbackLifecycleSeq:0,updateCandidate:null,updateCategories:[],updateSelected:new Set(),searchTimer:0,searchDataset:null,searchType:null,synopsisExpanded:false,synopsisText:'',synopsisNode:null,collectionOpen:false};
...
function readSeriesProviderGuard(){
  ...
  try{const all=JSON.parse(sessionStorage.getItem(SERIES_PROVIDER_GUARD_KEY)||'{}'),row=all?.[fingerprint];if(row&&typeof row==='object')return{fingerprint,count:Number(row.count)||0,blocked:!!row.blocked,lastAt:Number(row.lastAt)||0,lastKind:String(row.lastKind||''),lastMessage:String(row.lastMessage||''),mediaIds:Array.isArray(row.mediaIds)?row.mediaIds.map(String).slice(-SERIES_PROVIDER_GUARD_LIMIT):[]}}
  catch{}
  return{fingerprint,count:0,blocked:false,lastAt:0,lastKind:'',lastMessage:'',mediaIds:[]}
}
function writeSeriesProviderGuard(row){
  try{const all=JSON.parse(sessionStorage.getItem(SERIES_PROVIDER_GUARD_KEY)||'{}');all[row.fingerprint]={count:Number(row.count)||0,blocked:!!row.blocked,lastAt:Number(row.lastAt)||Date.now(),lastKind:String(row.lastKind||''),lastMessage:String(row.lastMessage||''),mediaIds:Array.isArray(row.mediaIds)?row.mediaIds.map(String).slice(-SERIES_PROVIDER_GUARD_LIMIT):[]};sessionStorage.setItem(SERIES_PROVIDER_GUARD_KEY,JSON.stringify(all))}catch{}
...
function seriesProviderGuardSuccess(){
  ...
  const next={...row,count:0,blocked:false,lastAt:Date.now(),lastKind:'',lastMessage:'',mediaIds:[]};
...
function seriesProviderGuardFailure(error,item){
  const row=currentSeriesProviderGuard(),failure=classifySeriesProviderFailure(error),mediaId=String(item?.series_id||item?.id||''),mediaIds=[...(Array.isArray(row.mediaIds)?row.mediaIds:[])];
  if(mediaId&&!mediaIds.includes(mediaId))mediaIds.push(mediaId);
  const compact=mediaIds.slice(-SERIES_PROVIDER_GUARD_LIMIT),count=Math.min(SERIES_PROVIDER_GUARD_LIMIT,compact.length),blocked=count>=SERIES_PROVIDER_GUARD_LIMIT,next={...row,count,blocked,lastAt:Date.now(),lastKind:failure.kind,lastMessage:failure.raw,mediaIds:compact};
  writeSeriesProviderGuard(next);
  playbackDebug('series-provider-guard-failure',{provider:standardProviderId(),mediaId,distinctSeries:count,limit:SERIES_PROVIDER_GUARD_LIMIT,blocked,errorType:failure.kind,rawMessage:failure.raw});
...
function liveLogoFallback(img,reason,details={}){
  ...
  const src=String(img.currentSrc||img.src||''),status=liveLogoHttpStatus(img);
  img.dataset.srhLiveLogoFallback='1';img.dataset.srhLiveLogoFailure=reason;
...
function inspectLiveLogoPixels(img){
  ...
  const src=String(img.currentSrc||img.src||'');if(!src||liveLogoProbeSeen.has(src))return;liveLogoProbeSeen.add(src);
  const run=()=>{try{
    const cv=document.createElement('canvas');cv.width=20;cv.height=12;
    const cx=cv.getContext('2d',{willReadFrequently:true});if(!cx)return;
    cx.clearRect(0,0,20,12);cx.drawImage(img,0,0,20,12);
...
function installR104VisualOverrides(){
  if(document.getElementById('srhR104VisualOverrides'))return;
  const style=document.createElement('style');style.id='srhR104VisualOverrides';
  style.textContent='body.srh-main-stream-style .detail-art.is-playing:after,body.srh-main-stream-style .detail-art.srh-media-active:after,body.srh-main-stream-style .detail-art.srh-custom-fs-active:after{content:none!important;display:none!important;background:none!important}body.srh-main-stream-style .srh-playback-feedback--loading .srh-playback-feedback__logo{max-height:58px!important}.poster-card--live .live-channel-logo,.grid-card--live .live-channel-logo,.channel-logo{object-fit:contain!important;object-position:center!important;max-width:100%!important;max-height:100%!important}@media(max-width:680px){body.srh-main-stream-style .srh-playback-feedback--loading .srh-playback-feedback__logo{max-height:50px!important}}';
  document.head.appendChild(style)
}
function installImageFallback(){document.addEventListener('error',e=>{const img=e.target;if(!(img instanceof HTMLImageElement)||img.dataset.fallback==='1'||img.dataset.srhImageChain==='1'||isBrandingImage(img)||isLiveLogoImage(img))return;img.dataset.fallback='1';img.classList.add('image-fallback');img.src=IMAGE_PLACEHOLDER},true)}
...
function providerEpisodeArtValue(ep){return String(ep?.__providerEpisodeArt||ep?.info?.movie_image||ep?.info?.cover_big||ep?.info?.cover||ep?.stream_icon||'').trim()}
...
const PLAYBACK_LIFECYCLE_STATES=new Set(['idle','preparing','loading','playing','paused','stalled','error','destroying']);
function setPlaybackLifecycle(next,details={}){
  const value=PLAYBACK_LIFECYCLE_STATES.has(next)?next:'idle',prev=String(state.playbackLifecycle||'idle');
  if(prev===value&&!details.force)return value;
  state.playbackLifecycle=value;
  state.playbackLifecycleAt=Date.now();
  state.playbackLifecycleSeq=(Number(state.playbackLifecycleSeq)||0)+1;
  playbackDebug('lifecycle',{seq:state.playbackLifecycleSeq,from:prev,to:value,mediaId:String(details.mediaId??state.currentMedia?.key??''),type:String(details.type??state.currentMedia?.type??''),reason:String(details.reason||''),attachId:Number(details.attachId)||0,epoch:Number(state.mediaEpoch)||0});
  return value
}
...
function preferOnDemandSources(values,type,pinned='',pinnedStartupMs=0){const list=uniqueMediaUrls(Array.isArray(values)?values:[values]);if(type==='live'||list.length<2)return list;const pin=String(pinned||'').trim(),base=pin?list.filter(x=>x!==pin):list.slice(),probe=base[0]||pin,pref=probe?onDemandFormatPreference.get(onDemandPreferenceKey(probe,type)):'';if(pref&&pref!=='m3u8')base.sort((a,b)=>(mediaExtension(b)===pref?1:0)-(mediaExtension(a)===pref?1:0));base.sort((a,b)=>(mediaExtension(a)==='m3u8'?1:0)-(mediaExtension(b)==='m3u8'?1:0));if(pin)playbackDebug('known-source-retained',{type,host:mediaHost(pin),extension:mediaExtension(pin),lastStartupMs:Math.round(Number(pinnedStartupMs)||0),alternatives:base.length});return uniqueMediaUrls(pin?[pin,...base]:base)}
...
function playbackVideoSnapshot(video){
  return{connected:!!video?.isConnected,paused:!!video?.paused,ended:!!video?.ended,readyState:Number(video?.readyState)||0,networkState:Number(video?.networkState)||0,currentTime:Number((Number(video?.currentTime)||0).toFixed(3)),src:String(video?.currentSrc||video?.src||'').slice(0,500)}
}
function resetPlaybackVideo(video){
  if(!video)return;
  try{video.pause()}catch{}
  video.onloadedmetadata=null;video.onerror=null;video.ontimeupdate=null;video.onpause=null;video.onended=null;
  try{video.removeAttribute('src');video.load()}catch{}
}
function beginPlaybackVideoTeardown(video,reason,onPersistError=null){
  if(!video)return{persistTask:Promise.resolve(),finalize:()=>{}};
  video.onpause=null;
  const persistTask=Promise.resolve(persistProgress(video,{frame:true})).catch(error=>{
    try{onPersistError?.(error)}catch{}
  });
  let finalized=false;
  const finalize=()=>{
    if(finalized)return;
...
function notePlaybackLaunch(target,entry,sources,video){
  ...
  const row={target,key,launch,first:launch===1,type:String(entry?.type||''),path:String(entry?.openPath||'direct'),position:Math.round((Number(entry?.position)||0)*1000)/1000,lastWorking:!!entry?.lastWorkingUrl,lastStartupMs:Math.round(Number(entry?.lastStartupMs)||0),formats:(sources||[]).map(mediaExtension),origins:(sources||[]).map(mediaOrigin),dependency:{hls:!!window.Hls?.isSupported,mpegts:!!window.mpegts?.createPlayer},transport:{hls:!!state.hls,mpegts:!!state.mpegtsPlayer,cleanup:!!state.mediaCleanup},video:playbackVideoSnapshot(video)};
  playbackDebug('playback-launch',row);return launch
}
...
function destroyHls(reason='transport-reset'){
  const before={epoch:Number(state.mediaEpoch)||0,hls:!!state.hls,mpegts:!!state.mpegtsPlayer,cleanup:!!state.mediaCleanup};
  state.mediaEpoch=(state.mediaEpoch||0)+1;
  if(state.mediaCleanup){const fn=state.mediaCleanup;state.mediaCleanup=null;try{fn()}catch{}}
  if(state.mpegtsPlayer){try{state.mpegtsPlayer.pause?.();state.mpegtsPlayer.unload?.();state.mpegtsPlayer.detachMediaElement?.();state.mpegtsPlayer.destroy?.()}catch{}state.mpegtsPlayer=null}
  if(state.hls){try{state.hls.destroy()}catch{}state.hls=null}
  playbackDebug('playback-transport-reset',{reason,before,afterEpoch:state.mediaEpoch})
}
function dispatchMediaAutoplayState(video,phase,stage,error=null){
  try{
    window.dispatchEvent(new CustomEvent('srh25:media-autoplay',{detail:{
      phase,stage,mediaId:String(video?.dataset?.srhMediaId||state.currentMedia?.key||''),sourceKind:String(video?.dataset?.srhSourceKind||''),name:error?.name||'',message:error?.message||'',readyState:video?.readyState||0,networkState:video?.networkState||0
    }}))
  }catch{}
}
function dispatchMediaPipeline(video,phase,data={}){
  try{window.dispatchEvent(new CustomEvent('srh25:media-pipeline',{detail:{phase,mediaId:String(video?.dataset?.srhMediaId||state.currentMedia?.key||''),mediaType:String(video?.dataset?.srhMediaType||state.currentMedia?.type||''),sourceKind:String(video?.dataset?.srhSourceKind||''),videoWidth:video?.videoWidth||0,videoHeight:video?.videoHeight||0,...data}}))}catch{}
}
function requestMediaPlay(video,stage,onBlocked=null){
  if(!video)return null;
  const rejected=error=>{
    if(error?.name==='NotAllowedError'){
      const first=video.dataset.srhAutoplayBlocked!=='1';
      video.dataset.srhAutoplayBlocked='1';
      video.controls=true;
      if(first){
        playbackDebug('autoplay-blocked',{stage,mediaId:String(video.dataset?.srhMediaId||state.currentMedia?.key||''),sourceKind:String(video.dataset?.srhSourceKind||''),readyState:video.readyState,networkState:video.networkState});
        dispatchMediaAutoplayState(video,'rejected',stage,error);
        try{onBlocked?.(error)}catch{}
      }
      return
    }
    if(error?.name!=='AbortError')playbackDebug('play-rejected',{stage,name:error?.name||'',message:error?.message||String(error||'')})
  };
  try{
    const p=video.play();
    if(p&&typeof p.then==='function')p.then(()=>{delete video.dataset.srhAutoplayBlocked;dispatchMediaAutoplayState(video,'started',stage)}).catch(rejected);
    return p
  }catch(e){rejected(e);return null}
}
let playbackAttachSeq=0;
function attachVideo(video,url,onReady,onError,onAutoplayBlocked=null){
 ...
 const exhausted=async()=>{destroyHls('sources-exhausted');const terminalToken=state.mediaEpoch,err=last||{message:'mídia indisponível'};if(!isLive()&&!err.status&&lastUrl){const status=await finalProviderStatus(lastUrl);if(status>=400)err.status=status}if(terminalToken!==state.mediaEpoch)return;setPlaybackLifecycle('error',{attachId,reason:err?.type||err?.message||'sources-exhausted'});onError?.(err)};
 const next=()=>{if(index>=queue.length){exhausted();return}start(queue[index++],0)};
 const start=async(current,attempt)=>{
  destroyHls('candidate-switch');const token=state.mediaEpoch,kind=mediaKind(current),sourceStartedAt=Date.now(),mediaType=String(state.currentMedia?.type||'live');let sourceFailed=false,sourceReady=false,retryTimer=0,watchdog=0,metadataTimer=0,liveStartupTimer=0,lastAdvanceAt=Date.now(),lastTime=0,hlsNetworkRecoveries=0,hlsMediaRecoveries=0,lastPipelineFragAt=0,dependencyReadyAt=sourceStartedAt;
  const resume={target:isLive()?0:Math.max(0,Number(resumeAt)||0),phase:!isLive()&&Number(resumeAt)>1?'pending':'none',requests:0,applies:0,startedAt:0,component:''};
  const resumeRanges=()=>{
    const out=[];
    try{for(let i=0;i<video.seekable.length;i++)out.push([Number(video.seekable.start(i))||0,Number(video.seekable.end(i))||0])}catch{}
    return out
  };
  const resumeSnapshot=()=>({target:Math.round(resume.target*1000)/1000,phase:resume.phase,requests:resume.requests,applies:resume.applies,currentTime:Math.round((Number(video.currentTime)||0)*1000)/1000,duration:Number.isFinite(video.duration)?Math.round(video.duration*1000)/1000:null,readyState:video.readyState,networkState:video.networkState,seekable:resumeRanges().map(r=>r.map(x=>Math.round(x*1000)/1000))});
  const completeResume=reason=>{
    ...
    playbackDebug('resume.complete',{attachId,kind,component:resume.component||reason,reason,seekMs:resume.startedAt?Math.round(performance.now()-resume.startedAt):0,...resumeSnapshot()});
    return true
  };
  const requestResume=component=>{
    ...
    if(resume.phase==='complete'||resume.phase==='applying'){
      playbackDebug('resume.reapply-blocked',{attachId,kind,component,...resumeSnapshot()});
      return resume.phase==='complete'
    }
    const duration=Number(video.duration),current=Number(video.currentTime)||0,ranges=resumeRanges();
    playbackDebug('resume.request',{attachId,kind,component,...resumeSnapshot()});
    ...
    if(ranges.length&&!ranges.some(([start,end])=>resume.target>=start-.25&&resume.target<=end+.25)){
      playbackDebug('resume.wait',{attachId,kind,component,reason:'outside-seekable',...resumeSnapshot()});
      return false
    }
    ...
    try{
      ...
      playbackDebug('resume.apply',{attachId,kind,component,...resumeSnapshot()});
      ...
      playbackDebug('resume.wait',{attachId,kind,component,reason:'seek-exception',message:e?.message||String(e),...resumeSnapshot()});
      return false
    }
  };
  const armMetadataTimer=()=>{
    ...
    metadataTimer=setTimeout(()=>{if(!alive()||sourceFailed||sourceReady||video.readyState>=1)return;if(video.networkState===2){playbackDebug('metadata-slow-loading',{attachId,kind,origin:mediaOrigin(current),readyState:video.readyState,networkState:video.networkState,elapsedMs:Date.now()-sourceStartedAt,sourceElapsedMs:Date.now()-armedAt,dependencyMs:dependencyReadyAt-sourceStartedAt});metadataTimer=setTimeout(()=>{if(alive()&&!sourceFailed&&!sourceReady&&video.readyState<1)fail({type:'metadata-timeout',details:'metadados não chegaram após attach da fonte'})},2800);return}fail({type:'metadata-timeout',details:'fonte sem metadados após attach'})},5200)
  };
  ...
  const markReady=()=>{
    if(!alive())return;
    if(sourceReady){requestResume('playing-repeat');playbackDebug('ready-repeat',{attachId,kind,video:playbackVideoSnapshot(video),resume:resumeSnapshot()});return}
    ...
    playbackDebug('source-ready',{attachId,kind,sourceKind,mediaId,live:isLive(),route:mediaLogRoute(current),startupMs,attachTotalMs,dependencyMs:dependencyReadyAt-sourceStartedAt,readyState:video.readyState,networkState:video.networkState});
    playbackDebug('playback-startup-summary',{attachId,mediaId,type:mediaType,path:openPath,resumed:resume.target>1,attachMs:attachTotalMs,sourceMs:startupMs,dependencyMs:dependencyReadyAt-sourceStartedAt,resumeRequests:resume.requests,resumeApplies:resume.applies,resumePhase:resume.phase,resumeTarget:Math.round(resume.target*1000)/1000});
    try{
  ...
  const onLoadedMetadata=()=>{playbackDebug('playback-event',{attachId,event:'loadedmetadata',kind,elapsedMs:Date.now()-sourceStartedAt,video:playbackVideoSnapshot(video)});requestResume('loadedmetadata')};
  const onCanPlay=()=>{playbackDebug('playback-event',{attachId,event:'canplay',kind,elapsedMs:Date.now()-sourceStartedAt,video:playbackVideoSnapshot(video)});requestResume('canplay')};
  ...
  const onSeeking=()=>{if(resume.phase==='applying')playbackDebug('resume.seeking',{attachId,kind,...resumeSnapshot()})};
  const onSeeked=()=>{if(resume.phase==='applying')completeResume('seeked')};
  const onWaiting=()=>{if(sourceReady)setPlaybackLifecycle('stalled',{attachId,reason:'waiting'});playbackDebug('playback-event',{attachId,event:'waiting',kind,elapsedMs:Date.now()-sourceStartedAt,video:playbackVideoSnapshot(video)})};
  const onStalled=()=>{if(sourceReady)setPlaybackLifecycle('stalled',{attachId,reason:'stalled'});playbackDebug('playback-event',{attachId,event:'stalled',kind,elapsedMs:Date.now()-sourceStartedAt,video:playbackVideoSnapshot(video)})};
  ...
  const onPlaying=()=>{setPlaybackLifecycle('playing',{attachId,reason:'playing'});playbackDebug('playback-event',{attachId,event:'playing',kind,elapsedMs:Date.now()-sourceStartedAt,video:playbackVideoSnapshot(video)});markReady()};
...
function continueFrameSources(entry,video=null){
  ...
  if(entry?.type==='vod'){
    ...
    if(id){out.push(mediaUrlWithExtension('vod',id,ext));out.push(mediaUrlWithExtension('vod',id,'m3u8'))}
  }else if(entry?.type==='series'){
    const id=snap.stream_id||entry?.streamId||'';
    const ext=String(entry?.containerExtension||snap.container_extension||'mp4').replace(/^\./,'').toLowerCase();
    if(id){out.push(mediaUrlWithExtension('series',id,ext));out.push(mediaUrlWithExtension('series',id,'m3u8'))}
  }
...
async function storeContinueFrame(entry,video,force=false){
  ...
  const now=Date.now(),key=continueFrameKey(entry),last=Number(continueFrameCaptureAt.get(key)||0),position=Number(video.currentTime)||Number(entry.position)||0,lastPosition=Number(continueFrameCapturePosition.get(key));
  if(!force&&now-last<20000)return false;
  if(!force&&Number.isFinite(lastPosition)&&Math.abs(position-lastPosition)<8)return false;
  continueFrameCaptureAt.set(key,now);continueFrameCapturePosition.set(key,position);
...
async function persistProgress(video=el.video,opts=null){
  ...
  if(video?.dataset?.srhResumeState==='pending'||video?.dataset?.srhResumeState==='applying'){playbackDebug('resume.persist-blocked',{mediaId:String(state.currentMedia?.key||''),phase:String(video.dataset.srhResumeState),currentTime:Math.round((Number(video.currentTime)||0)*1000)/1000});return}
...
function installFullscreenPlaybackUi(){
  ...
  const mount=()=>{
    ...
    on(play,'click',e=>{e.stopPropagation();if(video.paused)requestMediaPlay(video,'fullscreen-control');else video.pause();show()});
...
function normalizeEpisodes(v){
  ...
  const episodeLike=x=>!!x&&typeof x==='object'&&(x.id!=null||x.stream_id!=null||x.episode_id!=null||x.episode_num!=null||x.episode_number!=null);
  const add=(season,rows)=>{const key=seasonKey(season,'1'),list=(Array.isArray(rows)?rows:[rows]).filter(episodeLike);if(!list.length)return;if(!out[key])out[key]=[];const seen=new Set(out[key].map(x=>String(x?.id??x?.stream_id??x?.episode_id??x?.episode_num??'')));for(const ep of list){const id=String(ep?.id??ep?.stream_id??ep?.episode_id??ep?.episode_num??'');if(id&&seen.has(id))continue;if(id)seen.add(id);out[key].push(ep)}};
  const walk=(value,hint='1',depth=0)=>{
    if(value==null||depth>4)return;
    if(Array.isArray(value)){
      if(value.some(episodeLike)){const buckets=new Map();for(const ep of value){if(!episodeLike(ep))continue;const key=seasonKey(ep?.season??ep?.season_number??ep?.season_num??ep?.info?.season??hint,hint);if(!buckets.has(key))buckets.set(key,[]);buckets.get(key).push(ep)}for(const [key,rows] of buckets)add(key,rows);return}
      for(const row of value)walk(row,hint,depth+1);return
    }
    if(typeof value!=='object')return;
    if(episodeLike(value)){add(hint,value);return}
    for(const key of ['episodes','episode','items','data','results'])if(value[key]!=null){walk(value[key],hint,depth+1);if(Object.keys(out).length)return}
    for(const [key,row] of Object.entries(value)){if(['info','series_info','metadata','seasons','season'].includes(key))continue;if(Array.isArray(row)){add(key,row);continue}if(row&&typeof row==='object'){if(Array.isArray(row.episodes)||Array.isArray(row.items))walk(row.episodes||row.items,key,depth+1);else if(episodeLike(row))add(key,row)}}
  };
  walk(v);return out
}
function normalizeSeriesDetailPayload(data){
  if(!data||typeof data!=='object')return data;
  const roots=[data,data.data,data.result,data.response,data.series_info,data.series,data.payload].filter(v=>v&&typeof v==='object');
  let best=data,bestGroups=normalizeEpisodes(data.episodes);
  const countGroups=g=>Object.values(g||{}).reduce((n,rows)=>n+(Array.isArray(rows)?rows.length:0),0);
  for(const root of roots){const groups=normalizeEpisodes(root.episodes??root.episode??root.items);if(countGroups(groups)>countGroups(bestGroups)){best=root;bestGroups=groups}}
  return{...data,info:data.info||best.info||best.series_info||{},seasons:data.seasons||best.seasons||[],episodes:bestGroups}
}
...
let seriesEpisodesRequestSeq=0;
async function requestSeriesEpisodesBounded(item,{force=false,budgetMs=10500}={}){
  const requestId=++seriesEpisodesRequestSeq,mode=force?'manual-retry':'initial',mediaId=String(item?.series_id??item?.id??''),params={action:'get_series_info',series_id:mediaId},provider=normalizeServer(CONFIG.server),seen=new Set(),routes=[];
  const add=(base,endpoint='player_api.php',kind='route',proxied=false)=>addDetailCandidate(routes,seen,base,endpoint,params,CONFIG,kind,proxied);
  const preferred=xuiRouteState.preferred.get(provider+'|get_series_info');
  if(preferred)add(preferred.base,preferred.endpoint||'player_api.php','preferred',!!preferred.proxied);
  add(CONFIG.server,'player_api.php','configured-direct',false);
  if(CONFIG.corsProxy)add(CONFIG.server,'player_api.php','configured-proxy',true);
  for(const base of xuiRouteState.bases.slice(0,4)){add(base,'player_api.php','discovered-direct',false);if(CONFIG.corsProxy)add(base,'player_api.php','discovered-proxy',true)}
  const twin=protocolTwinBase(CONFIG.server);if(twin)add(twin,'player_api.php','protocol-twin',false);
  const discovery=xuiRouteState.bases.length?Promise.resolve([]):ensureXuiRouteDiscovery(CONFIG,1800).catch(()=>[]);
  const startedAll=performance.now(),totalBudget=Math.max(4500,Number(budgetMs)||10500),deadline=startedAll+totalBudget;
  let last=null,index=0;
  playbackDebug('series-episodes-request-start',{requestId,mode,mediaId,budgetMs:Math.round(totalBudget),routeCount:routes.length,preferred:!!preferred,preferredLastSuccessMs:Math.round(Number(preferred?.lastSuccessMs)||0)});
  while(performance.now()<deadline){
    ...
    const trusted=candidate.kind==='preferred'||candidate.kind==='configured-direct'||candidate.kind==='discovered-direct'||candidate.kind==='xui-preferred'||candidate.kind==='xui-player';
    const learnedMs=candidate.kind==='preferred'?Number(preferred?.lastSuccessMs)||0:0;
    const targetWindow=candidate.kind==='preferred'?Math.max(6500,Math.min(7600,learnedMs>0?Math.round(learnedMs*1.2+450):6500)):trusted?6200:3800;
    const hasFallback=index<routes.length||xuiRouteState.bases.length===0;
    const reserveMs=index===1&&hasFallback&&trusted?Math.min(3000,Math.max(2200,Math.round(totalBudget*.26))):0;
    const availableForRoute=Math.max(900,remaining-reserveMs);
    const timeout=Math.max(900,Math.min(targetWindow,availableForRoute,remaining));
    const started=performance.now(),origin=(()=>{try{return new URL(candidate.url).origin}catch{return''}})();
    playbackDebug('series-episodes-route-start',{requestId,mode,mediaId,routeIndex:index,transport:candidate.kind,endpoint:candidate.endpoint,origin,timeoutMs:Math.round(timeout),remainingMs:Math.max(0,Math.round(remaining)),reserveMs:Math.round(reserveMs),learnedMs:Math.round(learnedMs)});
    try{
      const raw=await requestJson(candidate.url,timeout,{srhBypassCircuit:true}),stats=seriesEpisodeStats(raw),ms=Math.round(performance.now()-started);
      if(!stats.episodes){const e=new Error('O provedor respondeu sem temporadas/episódios.');e.srhKind='invalid-payload';throw e}
      if(candidate.base)xuiRouteState.preferred.set(provider+'|get_series_info',{base:candidate.base,endpoint:candidate.endpoint,proxied:candidate.proxied,lastSuccessMs:ms});
      playbackDebug('series-episodes-route-success',{requestId,mode,mediaId,transport:candidate.kind,endpoint:candidate.endpoint,origin,ms,seasons:stats.seasons.length,episodes:stats.episodes,budgetUsedMs:Math.round(performance.now()-startedAll)});
      playbackDebug('series-episodes-request-done',{requestId,mode,mediaId,elapsedMs:Math.round(performance.now()-startedAll),transport:candidate.kind,seasons:stats.seasons.length,episodes:stats.episodes});
      window.SRHDebug?.noteRecovery?.({kind:'series.episodes',action:'get_series_info',mediaId,origin:provider,layer:'fetch',ms:Math.round(performance.now()-startedAll),status:200,message:'Temporadas e episódios carregados'});
      return stats.data
    }catch(e){
      last=e;
      const elapsedMs=Math.round(performance.now()-started),remainingMs=Math.max(0,Math.round(deadline-performance.now())),errorType=String(e?.srhKind||'provider-detail');
      playbackDebug('series-episodes-route-failed',{requestId,mode,mediaId,transport:candidate.kind,endpoint:candidate.endpoint,origin,ms:elapsedMs,timeoutMs:Math.round(timeout),errorType,message:e?.message||String(e),remainingMs,nearTimeout:errorType==='timeout'&&Math.abs(elapsedMs-timeout)<180})
    }
  }
  const elapsed=Math.round(performance.now()-startedAll),error=last||new Error('Não foi possível carregar temporadas e episódios.');
  playbackDebug('series-episodes-request-error',{requestId,mode,mediaId,elapsedMs:elapsed,budgetMs:Math.round(totalBudget),routesTried:index,errorType:String(error?.srhKind||'provider-detail'),message:error?.message||String(error)});
  window.SRHDebug?.noteFailure?.({kind:'series.episodes',action:'get_series_info',mediaId,origin:provider,layer:'fetch',ms:elapsed,message:error?.message||String(error),details:{budgetMs:Math.round(totalBudget),routesTried:index,requestId,mode}});
  throw error
}
async function loadSeriesDetailStable(item,{force=false}={}){
  const mediaId=String(item?.series_id??item?.id??''),stableKey=seriesDetailCacheKey(item);
  const cached=await readSeriesDetailCache(item);
  if(cached&&!force){playbackDebug('series-detail-cache-hit',{mediaId,ageMs:Date.now()-Number(cached.at||0),seasons:cached.stats.seasons.length,episodes:cached.stats.episodes});return cloneDetailPayload(cached.data)}
  if(seriesDetailStableInflight.has(stableKey))return seriesDetailStableInflight.get(stableKey);
  const job=(async()=>{
    try{
      const data=await requestSeriesEpisodesBounded(item,{force,budgetMs:force?11000:10500}),stats=seriesEpisodeStats(data);
      await saveSeriesDetailCache(item,stats.data,'bounded-provider');
      playbackDebug('series-detail-restored',{mediaId,source:'bounded-provider',seasons:stats.seasons.length,episodes:stats.episodes});
      return stats.data
    }catch(e){
      if(cached){playbackDebug('series-detail-cache-stale',{mediaId,ageMs:Date.now()-Number(cached.at||0),reason:e?.message||'provider-failure'});return cloneDetailPayload(cached.data)}
      throw e
    }
  })().finally(()=>seriesDetailStableInflight.delete(stableKey));
  seriesDetailStableInflight.set(stableKey,job);return job
}
...
async function renderSeriesRecoveryModal(item,error,token){
  try{
    ...
    if(retry)retry.onclick=e=>{e.stopPropagation();retry.disabled=true;retry.textContent='Carregando…';const retryStarted=performance.now();playbackDebug('series-retry-start',{mediaId:String(item?.series_id||''),token,source:'safe-shell'});void openSeries(item,{forceEpisodes:true,retrySource:'safe-shell'}).then(()=>{playbackDebug('series-retry-finished',{mediaId:String(item?.series_id||''),token,source:'safe-shell',elapsedMs:Math.round(performance.now()-retryStarted),phase:String(state.srhSeriesEpisodes?.phase||'')})}).catch(err=>{retry.disabled=false;retry.textContent='Tentar carregar episódios';playbackDebug('series-retry-failed',{mediaId:String(item?.series_id||''),token,source:'safe-shell',elapsedMs:Math.round(performance.now()-retryStarted),message:err?.message||String(err)})})};
...
function setSeriesEpisodesState(token,phase,details={}){
  if(!isDetailCurrent(token))return false;
  state.srhSeriesEpisodes={token,phase,at:Date.now(),...details};
  const modal=detailModal();if(modal)modal.dataset.srhEpisodesState=phase;
  playbackDebug('series-episodes-state',{phase,mediaId:String(details.mediaId||state.currentSeries?.item?.series_id||''),...details});
  return true
}
function seriesEpisodeLoadingMarkup(){
  const row='<article class="episode srh-episode-skeleton" aria-hidden="true"><span class="episode__thumb srh-shimmer-block"></span><div class="srh-episode-skeleton__body"><span class="srh-shimmer-line"></span><span class="srh-shimmer-line"></span><span class="episode__progress"></span></div><span class="episode__play srh-shimmer-button"></span></article>';
  return row+row+row
}
function renderSeriesEpisodesFailure(item,error,token,phase='episodes-error'){
  if(!setSeriesEpisodesState(token,phase,{mediaId:String(item?.series_id||''),message:error?.message||String(error||'')}))return;
  const list=el.detailBody.querySelector('#episodeList')||el.detailBody.querySelector('.episode-list'),trigger=el.detailBody.querySelector('#seasonTrigger'),label=el.detailBody.querySelector('#seasonLabel');
  if(trigger)trigger.disabled=true;if(label)label.textContent=phase==='episodes-empty'?'Sem episódios':'Temporadas indisponíveis';
  if(!list)return;
  list.innerHTML='<div class="rail-load-error srh-series-detail-partial"><span>'+escapeHtml(phase==='episodes-empty'?'A API respondeu, mas não retornou episódios utilizáveis.':'As informações da série estão disponíveis, mas os episódios não puderam ser carregados.')+'</span><button type="button" data-series-episodes-retry>Tentar carregar episódios</button></div>';
  const retry=list.querySelector('[data-series-episodes-retry]');if(retry)retry.onclick=e=>{e.stopPropagation();void retrySeriesEpisodesOnly(item,token,retry)}
}
async function renderSeriesMetadataPreview(item,token){
  const startedAt=performance.now(),fallback=seriesDetailFallback(item,null);
  try{
    let timeoutId=0;
    const timeout=new Promise(resolve=>{timeoutId=setTimeout(()=>resolve(null),6200)});
    const resolved=resolveModalDetailPackage('series',item,fallback).catch(()=>null);
    let pkg=await Promise.race([resolved,timeout]);
    clearTimeout(timeoutId);
    if(!isDetailCurrent(token))return null;
    if(!pkg){
      const info=fallback.info||{},art=modalImageValue(info.backdrop_path,info.backdrop,info.cover_big,info.movie_image,info.cover,item?.backdrop_path,item?.backdrop,item?.cover_big,item?.cover,item?.stream_icon)||IMAGE_PLACEHOLDER;
      const title=stripEmoji(info.name||itemTitle(item),'Sem título'),overview=String(info.plot||info.description||item?.plot||item?.description||'').trim();
      pkg={art,title,overview,year:String(info.year||item?.year||''),vote:Number.isFinite(Number(info.rating??item?.rating))?Number(info.rating??item?.rating):null,logo:'',brand:{id:String(info.tmdb_id||item?.tmdb_id||''),title,backdrop:art,overview,year:String(info.year||item?.year||''),vote:Number.isFinite(Number(info.rating??item?.rating))?Number(info.rating??item?.rating):null,logo:'',artSource:'catalog-fallback',metadataVersion:3},tmdb:null,__srhMetadataTimeout:true};
      playbackDebug('series-metadata-timeout-fallback',{mediaId:String(item?.series_id||''),metadataMs:Math.round(performance.now()-startedAt)})
    }
    playbackDebug('series-metadata-ready',{mediaId:String(item?.series_id||''),metadataMs:Math.round(performance.now()-startedAt),timedOut:!!pkg.__srhMetadataTimeout,artSource:String(pkg?.brand?.artSource||pkg?.artSource||'none'),logo:!!pkg?.brand?.logo});
    return pkg
  }catch(e){playbackDebug('series-metadata-preview-failed',{mediaId:String(item?.series_id||''),metadataMs:Math.round(performance.now()-startedAt),message:e?.message||String(e)});return null}
}
async function retrySeriesEpisodesOnly(item,token,button){
  if(!isDetailCurrent(token))return;
  const retryStarted=performance.now();playbackDebug('series-retry-start',{mediaId:String(item?.series_id||''),token,source:'episodes-only'});
  if(button){button.disabled=true;button.textContent='Carregando…'}
  setSeriesEpisodesState(token,'episodes-loading',{mediaId:String(item?.series_id||''),retry:true});
  const list=el.detailBody.querySelector('#episodeList')||el.detailBody.querySelector('.episode-list');if(list)list.innerHTML=seriesEpisodeLoadingMarkup();
  try{const data=await loadSeriesDetailStable(item,{force:true});if(!isDetailCurrent(token))return;seriesProviderGuardSuccess();await openSeries(item,{preloadedData:data,reuseModal:true,detailToken:token,retrySource:'episodes-only'});playbackDebug('series-retry-success',{mediaId:String(item?.series_id||''),token,source:'episodes-only',elapsedMs:Math.round(performance.now()-retryStarted)})}
  catch(e){if(!isDetailCurrent(token))return;seriesProviderGuardFailure(e,item);playbackDebug('series-retry-failed',{mediaId:String(item?.series_id||''),token,source:'episodes-only',elapsedMs:Math.round(performance.now()-retryStarted),message:e?.message||String(e)});renderSeriesEpisodesFailure(item,e,token,'episodes-error')}
}
async function stopInlineSeriesVideo(opts=null){const sv=state.seriesVideo;if(!sv)return;const preserveGesture=!!opts?.preserveGesture,media=state.currentMedia;setPlaybackLifecycle('destroying',{mediaId:media?.key||'',type:media?.type||'',reason:'series-close'});srhClearPlaybackFeedback(sv.art,{restoreBrand:true});playbackDebug('playback-series-teardown-begin',{mediaId:String(media?.key||''),preserveGesture,video:playbackVideoSnapshot(sv.video)});const teardown=beginPlaybackVideoTeardown(sv.video,'series-close',e=>playbackDebug('series-progress-save-failed',{message:e?.message||String(e)}));if(!preserveGesture)await teardown.persistTask;teardown.finalize();sv.video.classList.add('is-hidden');sv.image.classList.remove('is-hidden');sv.actions.classList.add('is-hidden');state.seriesVideo=null;state.currentMedia=null;setPlaybackLifecycle('idle',{mediaId:media?.key||'',type:media?.type||'',reason:'series-closed'});playbackDebug('playback-series-teardown-end',{mediaId:String(media?.key||''),video:playbackVideoSnapshot(sv.video)});const leaveTask=Promise.resolve(leaveFullscreenPortrait()).catch(()=>{});if(!preserveGesture)await leaveTask}
async function openSeries(item,opts=null){if(seriesProviderGuardBlocked()){showSeriesProviderGuardPopup();if(state.activeType==='series')renderActiveType();return}const openStartedAt=performance.now(),openMode=opts?.forceEpisodes?'manual-retry':opts?.reuseModal?'reuse':'initial';playbackDebug('series-open-start',{mediaId:String(item?.series_id||item?.id||''),mode:openMode,retrySource:String(opts?.retrySource||''),currentToken:Number(state.detailToken)||0});const reuse=!!opts?.reuseModal&&isDetailCurrent(Number(opts?.detailToken));if(!reuse&&!(await openDetail(itemTitle(item))))return;const token=reuse?Number(opts.detailToken):state.detailToken;detailModal()?.classList.add('is-series');if(!reuse)await waitDetailSkeletonPaint(token);if(!isDetailCurrent(token))return;const seriesStartedAt=performance.now(),episodesStartedAt=performance.now();let metadataMs=0,episodesMs=0;const previewTask=(reuse?Promise.resolve(state.srhDetailPackage?.brand?{art:state.currentSeries?.backdrop||state.srhDetailPackage.brand.backdrop||IMAGE_PLACEHOLDER,overview:state.synopsisText||state.srhDetailPackage.brand.overview||'',brand:state.srhDetailPackage.brand}:null):renderSeriesMetadataPreview(item,token)).then(pkg=>{metadataMs=Math.round(performance.now()-seriesStartedAt);return pkg});setSeriesEpisodesState(token,'episodes-loading',{mediaId:String(item?.series_id||''),reuse});try{let data=null;try{data=opts?.preloadedData||await loadSeriesDetailStable(item,{force:!!opts?.forceEpisodes});episodesMs=Math.round(performance.now()-episodesStartedAt);seriesProviderGuardSuccess()}catch(detailError){episodesMs=Math.round(performance.now()-episodesStartedAt);const guard=seriesProviderGuardFailure(detailError,item);await previewTask.catch(()=>null);if(guard.blocked){await activateSeriesProviderGuard();return}data=seriesDetailFallback(item,detailError);playbackDebug('series-detail-partial-fallback',{mediaId:String(item?.series_id||''),resumeRecovered:!!data.__srhResumeRecovered,message:detailError?.message||String(detailError),errorType:detailError?.srhKind||'',rawMessage:detailError?.srhRawMessage||''});if(!data.__srhResumeRecovered){setSeriesEpisodesState(token,'episodes-error',{mediaId:String(item?.series_id||''),message:detailError?.message||String(detailError)});const recovered=await renderSeriesRecoveryModal(item,detailError,token);if(!recovered&&isDetailCurrent(token))renderSeriesEpisodesFailure(item,detailError,token,'episodes-error');playbackDebug('series-required-payload',{mediaId:String(item?.series_id||''),metadataMs,episodesMs,requiredPayloadMs:Math.round(performance.now()-seriesStartedAt),status:'error'});return}}if(!isDetailCurrent(token))return;const previewPackage=await previewTask.catch(()=>null);if(!isDetailCurrent(token))return;const needsProviderPackage=!previewPackage?.art||!String(previewPackage?.overview||previewPackage?.brand?.overview||'').trim(),modalPackage=reuse&&state.srhDetailPackage?.token===token&&state.srhDetailPackage?.brand?{art:state.currentSeries?.backdrop||state.srhDetailPackage.brand.backdrop||IMAGE_PLACEHOLDER,overview:state.synopsisText||state.srhDetailPackage.brand.overview||'',brand:state.srhDetailPackage.brand}:(!needsProviderPackage?previewPackage:await resolveModalDetailPackage('series',item,data));if(!isDetailCurrent(token))return;const info=data?.info||{},episodes=normalizeEpisodes(data?.episodes),seasons=Object.keys(episodes).filter(k=>Array.isArray(episodes[k])&&episodes[k].length).sort((a,b)=>Number(a)-Number(b)),backdrop=modalPackage.art||IMAGE_PLACEHOLDER,plot=modalPackage.overview||'',episodeCount=seasons.reduce((n,k)=>n+episodes[k].length,0),requiredPayloadMs=Math.round(performance.now()-seriesStartedAt);playbackDebug('series-required-payload',{mediaId:String(item?.series_id||''),metadataMs,episodesMs,seasonsMs:episodesMs,requiredPayloadMs,seasons:seasons.length,episodes:episodeCount,status:seasons.length?'ready':'empty'});setSeriesEpisodesState(token,seasons.length?'episodes-ready':'episodes-empty',{mediaId:String(item?.series_id||''),seasons:seasons.length,episodes:episodeCount});state.currentSeries={item,data,episodes,backdrop,tmdbId:modalPackage.brand?.id||info?.tmdb_id||null,tmdbBackdrop:String(modalPackage.brand?.tmdbBackdrop||modalPackage.brand?.backdrop||'').trim(),tmdbPoster:String(modalPackage.brand?.poster||'').trim(),providerBackdrop:String(modalPackage.brand?.providerBackdrop||'').trim(),providerCover:String(modalPackage.brand?.providerCover||'').trim()};state.srhDetailPackage={token,type:'series',mediaId:String(item.series_id||''),brand:modalPackage.brand};if(state.srhOpeningContinue&&state.srhContinueFramePromise){await state.srhContinueFramePromise.catch(()=>null);if(!isDetailCurrent(token))return}const displayBackdrop=state.srhOpeningContinue&&state.srhContinueFrameUrl?state.srhContinueFrameUrl:backdrop,commitStartedAt=performance.now();el.detailBody.innerHTML=`<div class="detail-content"><div class="series-static"><div class="detail-art" id="seriesArt"><img class="backdrop" id="seriesImage" src="${escapeHtml(displayBackdrop||IMAGE_PLACEHOLDER)}" alt=""><video class="is-hidden" id="seriesInlineVideo" controls autoplay playsinline></video><div class="detail-art__actions is-hidden" id="seriesVideoActions"><button class="floating-action" id="seriesStop" title="Fechar vídeo"><svg class="ui-svg" viewBox="0 0 24 24"><path d="M6.5 6.5l11 11M17.5 6.5l-11 11"/></svg></button><button class="floating-action" id="seriesExpand" title="Expandir ou minimizar"><svg class="ui-svg" viewBox="0 0 24 24"><path d="M8.5 4.5h-4v4M15.5 4.5h4v4M8.5 19.5h-4v-4M15.5 19.5h4v-4"/></svg></button></div></div><div class="detail-title-row"><h2 class="detail-title">${escapeHtml(itemTitle(item))}</h2></div><div class="synopsis" id="seriesSynopsis">${synopsisMarkup(plot,false)}</div><div class="season-box"><button class="season-trigger" id="seasonTrigger"><span id="seasonLabel">${seasons[0]?'Temporada '+escapeHtml(seasons[0]):'Temporadas'}</span><span class="season-trigger__icon" aria-hidden="true">${UI_ICON_CHEVRON_DOWN}</span></button><div class="season-menu is-hidden${seasons.length>4?' season-menu--scroll':''}" id="seasonMenu"></div></div></div><div class="episode-container"><div class="episode-list" id="episodeList"></div></div></div>`;playbackDebug('series-modal-commit',{mediaId:String(item?.series_id||''),metadataMs,episodesMs,seasonsMs:episodesMs,requiredPayloadMs,modalCommitMs:Math.round(performance.now()-commitStartedAt),seasons:seasons.length,episodes:episodeCount});const art=$('#seriesArt'),image=$('#seriesImage'),video=$('#seriesInlineVideo'),actions=$('#seriesVideoActions'),seasonMenu=$('#seasonMenu'),seasonLabel=$('#seasonLabel'),list=$('#episodeList'),syn=$('#seriesSynopsis');applyManagedImageChain(image,[displayBackdrop,modalPackage.brand?.backdrop,modalPackage.brand?.providerBackdrop,modalPackage.brand?.poster,modalPackage.brand?.providerCover,item?.backdrop_path,item?.backdrop,item?.cover_big,item?.cover,item?.stream_icon],{label:'series-detail',eager:true});setSynopsisState(syn,plot,false);syn.onclick=e=>{e.stopPropagation();setSynopsisState(syn,plot,!state.synopsisExpanded)};seasonMenu.innerHTML=seasons.map((s,i)=>`<button class="season-option${i===0?' is-active':''}" data-season="${escapeHtml(s)}">Temporada ${escapeHtml(s)}</button>`).join('');$('#seasonTrigger').onclick=e=>{e.stopPropagation();seasonMenu.classList.toggle('is-hidden')};function drawSeason(key){seasonLabel.textContent='Temporada '+key;seasonMenu.classList.add('is-hidden');seasonMenu.querySelectorAll('[data-season]').forEach(b=>b.classList.toggle('is-active',b.dataset.season===key));const eps=Array.isArray(episodes[key])?episodes[key]:[],history=new Map(getHistory('series').map(x=>[x.key,x]));list.innerHTML=eps.map((ep,idx)=>{const n=ep.episode_num??idx+1,thumb=ep.__srhEpisodeThumb||ep.info?.movie_image||ep.info?.cover_big||ep.info?.cover||ep.stream_icon||backdrop||IMAGE_PLACEHOLDER,h=history.get(episodeKey(item,ep)),done=getStandardCompleted(episodeKey(item,ep)),pct=h?.duration?Math.min(100,h.position/h.duration*100):(done?100:0);return `<article class="episode" data-ep="${idx}" role="button" tabindex="0" aria-label="Assistir episódio ${escapeHtml(n)}"><img class="episode__thumb" src="${escapeHtml(thumb)}" alt="" loading="${idx<6?'eager':'lazy'}" decoding="async"><div><div class="episode__title">Episódio ${escapeHtml(n)}</div><div class="episode__meta">${escapeHtml(ep.info?.duration||'')}</div><div class="episode__progress"><span style="width:${pct}%"></span></div></div><div class="episode__play" aria-hidden="true"><svg class="ui-svg" viewBox="0 0 24 24"><path d="M9 6.5v11l8-5.5z"/></svg></div></article>`}).join('');list.querySelectorAll('[data-ep]').forEach(row=>{const epIndex=Number(row.dataset.ep),ep=eps[epIndex],img=row.querySelector('.episode__thumb');applyManagedImageChain(img,[ep?.__tmdbStill,ep?.__providerEpisodeArt,ep?.info?.movie_image,ep?.info?.cover_big,ep?.info?.cover,ep?.stream_icon,state.currentSeries?.tmdbBackdrop,state.currentSeries?.tmdbPoster,state.currentSeries?.providerBackdrop,state.currentSeries?.providerCover,backdrop,item?.backdrop_path,item?.backdrop,item?.cover_big,item?.cover,item?.stream_icon],{label:'episode-'+String(ep?.episode_num??ep?.episode_number??epIndex+1),eager:epIndex<6});const activate=()=>{void playEpisode(ep,epIndex,key).catch(err=>{playbackDebug('series-episode-click-failed',{mediaId:String(ep?.id||ep?.stream_id||''),message:err?.message||String(err)});toast('Não foi possível iniciar este episódio.')})};row.onclick=activate;row.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();activate()}}});list.scrollTop=0}async function playEpisode(ep,idx,season){const playToken=++state.seriesPlayToken;void stopInlineSeriesVideo({preserveGesture:true});if(!isDetailCurrent(token)||playToken!==state.seriesPlayToken)return;const n=ep.episode_num??idx+1,eid=ep?.id||ep?.stream_id,catalogExt=String(ep?.container_extension||'mp4').replace(/^\./,'').toLowerCase(),infoExt=String(ep?.info?.container_extension||'').replace(/^\./,'').toLowerCase(),declaredExt=infoExt||catalogExt||'mp4',direct=normalizeMediaUrl(ep?.direct_source||ep?.info?.direct_source||''),declaredUrl=mediaUrlWithExtension('series',eid,declaredExt),catalogUrl=catalogExt&&catalogExt!==declaredExt?mediaUrlWithExtension('series',eid,catalogExt):'',hls=mediaUrlWithExtension('series',eid,'m3u8'),sources=uniqueMediaUrls([declaredUrl,catalogUrl,direct,hls]),key=episodeKey(item,ep),old=getHistory('series').find(x=>String(x.seriesId??'')===String(item.series_id)&&String(x.season??'')===String(season)&&(String(x.itemSnapshot?.stream_id??'')===String(eid)||String(x.episodeNumber??'')===String(n))),entry={key,type:'series',title:itemTitle(item)+' — Episódio '+n,image:ep.__srhWaitingArt||ep.__tmdbStill||ep.info?.movie_image||ep.info?.cover_big||ep.info?.cover||ep.stream_icon||backdrop,providerEpisodeImage:providerEpisodeArtValue(ep),url:sources[0],sources,seriesId:item.series_id,season,episodeNumber:n,tmdbId:state.currentSeries?.tmdbId||data?.__tmdb?.id||null,containerExtension:infoExt||catalogExt,itemSnapshot:{series_id:item.series_id,stream_id:eid,name:itemTitle(item),cover:item.cover||backdrop,series_cover:item.cover||item.stream_icon||'',series_backdrop:state.currentSeries?.providerBackdrop||item.backdrop_path||item.backdrop||'',episode_image:providerEpisodeArtValue(ep),tmdb_id:state.currentSeries?.tmdbId||data?.__tmdb?.id||null,container_extension:infoExt||catalogExt,movie_image:ep.info?.movie_image||''},position:0,duration:0};sources.forEach(warmMediaOrigin);const episodeArt=ep.__srhWaitingArt||ep.__tmdbStill||ep.info?.movie_image||ep.info?.cover_big||ep.info?.cover||ep.stream_icon||backdrop||IMAGE_PLACEHOLDER;applyManagedImageChain(image,ep.__srhEpisodeArtCandidates||[ep.__tmdbStill,providerEpisodeArtValue(ep),state.currentSeries?.tmdbBackdrop,state.currentSeries?.tmdbPoster,state.currentSeries?.providerBackdrop,state.currentSeries?.providerCover,episodeArt,backdrop],{label:'series-playing-'+String(n),eager:true});image.classList.remove('is-hidden');video.classList.add('is-hidden');video.muted=false;video.volume=1;video.controls=true;video.autoplay=true;actions.classList.remove('is-hidden');const playbackEntry=old?{...entry,...old,url:entry.url||sources[0],tmdbId:entry.tmdbId||old.tmdbId||old.itemSnapshot?.tmdb_id||null,itemSnapshot:{...(old.itemSnapshot||{}),...(entry.itemSnapshot||{})},sources}:entry,playbackSources=resolvePlaybackSources(sources,playbackEntry);state.currentMedia={...playbackEntry,sources:playbackSources,url:entry.url||playbackSources[0]};state.saveTick=0;state.seriesVideo={video,image,actions,art};notePlaybackLaunch('series-inline',state.currentMedia,playbackSources,video);srhShowPlaybackLoading(art,itemTitle(item));attachVideo(video,playbackSources,()=>{srhClearPlaybackFeedback(art,{restoreBrand:true});image.classList.add('is-hidden');video.classList.remove('is-hidden');art.classList.add('is-playing')},err=>{image.classList.remove('is-hidden');video.classList.add('is-hidden');art.classList.remove('is-playing');srhShowProviderError(art,err);toast('Não foi possível reproduzir este episódio.')},()=>{srhClearPlaybackFeedback(art,{restoreBrand:true});image.classList.add('is-hidden');video.classList.remove('is-hidden');video.controls=true;art.classList.add('is-playing');playbackDebug('series-autoplay-user-action-required',{mediaId:String(entry.key||'')});toast('Toque em Play para iniciar o episódio.')});video.ontimeupdate=()=>{if(++state.saveTick%25===0)persistProgress(video)};video.onpause=()=>persistProgress(video,{frame:true});video.onended=()=>{const cur=state.currentMedia||entry;finalizeSeriesEpisode(cur,video.duration);drawSeason(season)};const stopButton=$('#seriesStop');if(stopButton)stopButton.onclick=e=>{e.stopPropagation();stopInlineSeriesVideo().then(()=>drawSeason(season))};$('#seriesExpand').onclick=e=>{e.stopPropagation();toggleFullscreen(art)}}seasonMenu.querySelectorAll('[data-season]').forEach(b=>b.onclick=e=>{e.stopPropagation();drawSeason(b.dataset.season)});if(seasons[0])drawSeason(seasons[0]);else{list.innerHTML='<div class="rail-load-error srh-series-detail-partial"><span>'+escapeHtml(data?.__srhPartialSeries?'Os dados dos episódios não chegaram do provedor.':'A API não retornou episódios.')+'</span><button type="button">Tentar carregar episódios</button></div>';const retry=list.querySelector('button');if(retry)retry.onclick=e=>{e.stopPropagation();void retrySeriesEpisodesOnly(item,token,retry)}}if(data?.__srhPartialSeries){playbackDebug('series-detail-partial-rendered',{mediaId:String(item?.series_id||''),seasons:seasons.length,resumeRecovered:!!data.__srhResumeRecovered})}}catch(e){if(!isDetailCurrent(token))return;const recovered=await renderSeriesRecoveryModal(item,e,token);if(!recovered&&isDetailCurrent(token)){el.detailBody.innerHTML='<div class="rail-load-error srh-series-detail-partial"><span>Não foi possível montar os episódios agora.</span><button type="button">Tentar novamente</button></div>';el.detailBody.querySelector('button').onclick=()=>openSeries(item,{forceEpisodes:true})}}}
async function openLive(group){if(!await openDetail(group.baseName))return;const token=state.detailToken;await waitDetailSkeletonPaint(token);if(!isDetailCurrent(token))return;state.currentDetail={type:'live',group};const logo=group.image||IMAGE_PLACEHOLDER,variants=group.variants||[];el.detailBody.innerHTML=`<div class="detail-content"><div class="detail-art"><img class="channel-logo" src="${escapeHtml(logo)}" alt=""></div><div class="detail-title-row"><h2 class="detail-title">${escapeHtml(group.baseName)}</h2></div><div class="quality-list">${variants.map((v,i)=>`<button class="quality-row" data-quality-index="${i}"><span class="quality-row__name">${escapeHtml(group.baseName)}</span><span class="quality-row__quality">${escapeHtml(v._quality||'Padrão')}</span></button>`).join('')}</div></div>`;el.detailBody.querySelectorAll('[data-quality-index]').forEach(b=>b.onclick=()=>{const v=variants[Number(b.dataset.qualityIndex)],url=streamUrl('live',v);warmMediaOrigin(url);playbackDebug('live-stable-route',{mediaId:String(v?.stream_id??v?.id??''),extension:String(CONFIG.liveExtension||'m3u8')});openGeneralPlayer(url,group.baseName+' · '+(v._quality||'Padrão'),{key:'live:'+v.stream_id,type:'live',title:group.baseName,url,image:logo,position:0,duration:0})})}
function openItem(item,type){const task=type==='live'?openLive(item):type==='series'?openSeries(item):openFilm(item);if(task&&typeof task.catch==='function')task.catch(e=>{playbackDebug('ui-open-failed',{type,mediaId:String(itemId(item,type)||''),message:e?.message||String(e)});toast('Não foi possível abrir este conteúdo.')});return task}
...
async function srhFixStopInlineDetailVideo(restoreImage=true){
  ...
  setPlaybackLifecycle('destroying',{mediaId:mediaAtStop?.key||'',type:mediaAtStop?.type||'',reason:'inline-close'});
  srhClearPlaybackFeedback(iv.art,{restoreBrand:true});
  const teardown=beginPlaybackVideoTeardown(iv.video,'inline-close',e=>playbackDebug('continue-frame-teardown-failed',{type:mediaAtStop?.type||'',key:mediaAtStop?.key||'',message:e?.message||String(e)}));
  await teardown.persistTask;teardown.finalize();
  ...
  setPlaybackLifecycle('idle',{mediaId:mediaAtStop?.key||'',type:mediaAtStop?.type||'',reason:'inline-closed'});
}
function srhFixOverlayPlayer(url,title,entry=null){
  const sources=resolvePlaybackSources(url,entry);
  notePlaybackLaunch('overlay',entry,sources,el.video);
...
async function srhFixInlinePlayer(url,title,entry=null){
  ...
  state.currentMedia=entry?{...entry,sources,url:entry.url||sources[0]}:{key:'tmp:'+Date.now(),type:'live',title,url:sources[0],sources,image:'',position:0,duration:0};
  notePlaybackLaunch('inline',state.currentMedia,sources,video);
  ...
  actions.querySelector('[data-inline-close]').onclick=e=>{
    e.stopPropagation();
    void srhFixStopInlineDetailVideo(true).catch(err=>playbackDebug('inline-close-failed',{message:err?.message||String(err)}));
  };
...
function serializePlaybackNavigation(label,task){
  ...
  const run=playbackNavigationTail.catch(()=>{}).then(async()=>{
    const startedAt=performance.now();
    playbackDebug('playback-transition-start',{seq,label,queueMs:Math.round(startedAt-queuedAt),mediaId:String(state.currentMedia?.key||''),inline:!!state.detailInlineVideo,overlay:!!state.playerActive,transport:{hls:!!state.hls,mpegts:!!state.mpegtsPlayer,cleanup:!!state.mediaCleanup}});
    try{return await task()}
    finally{playbackDebug('playback-transition-end',{seq,label,totalMs:Math.round(performance.now()-startedAt),mediaId:String(state.currentMedia?.key||''),inline:!!state.detailInlineVideo,overlay:!!state.playerActive,transport:{hls:!!state.hls,mpegts:!!state.mpegtsPlayer,cleanup:!!state.mediaCleanup}})}
  });
  playbackNavigationTail=run.catch(()=>{});
  return run
}
...
(function installLiteUiOrganization(){
  ...
  function continueArtLooksBlank(img){
    if(!img?.naturalWidth||!img.naturalHeight)return true;
    try{
      const cv=document.createElement('canvas');cv.width=20;cv.height=12;
      const cx=cv.getContext('2d',{willReadFrequently:true});if(!cx)return false;
      cx.drawImage(img,0,0,20,12);const d=cx.getImageData(0,0,20,12).data;
      let visible=0,maxL=0,sumL=0;
      for(let i=0;i<d.length;i+=4){if(d[i+3]<=8)continue;visible++;const l=(d[i]*.299+d[i+1]*.587+d[i+2]*.114);sumL+=l;if(l>maxL)maxL=l}
      if(visible<=2)return true;
      return maxL<10&&(sumL/visible)<4
    }catch{return false}
  }
  function preloadContinueArt(url,timeout=5200){
    ...
    return new Promise(resolve=>{
      ...
      try{const host=new URL(src,location.href).hostname;if(host==='image.tmdb.org'||host.endsWith('.image.tmdb.org'))img.crossOrigin='anonymous'}catch{}
      const finish=ok=>{if(done)return;done=true;clearTimeout(timer);img.onload=null;img.onerror=null;resolve(!!ok)};
      const timer=setTimeout(()=>finish(false),timeout);
      img.onload=()=>{const valid=img.naturalWidth>0&&img.naturalHeight>0&&!continueArtLooksBlank(img);if(!valid)playbackDebug('continue-art-reject',{reason:'blank-or-transparent',source:src,naturalWidth:img.naturalWidth||0,naturalHeight:img.naturalHeight||0});finish(valid)};
  ...
  const srhRailScheduler={active:0,max:3,backgroundActive:0,backgroundMax:1,queue:[],seq:0};
  function pumpRailQueue(){
    while(srhRailScheduler.active<srhRailScheduler.max&&srhRailScheduler.queue.length){
      ...
      const pick=srhRailScheduler.queue.findIndex(job=>job.lane!=='background'||srhRailScheduler.backgroundActive<srhRailScheduler.backgroundMax);
      if(pick<0)break;
      const job=srhRailScheduler.queue.splice(pick,1)[0];
      if(job.token!==state.renderToken){job.resolve([]);continue}
      srhRailScheduler.active++;
      if(job.lane==='background')srhRailScheduler.backgroundActive++;
      Promise.resolve().then(job.task).then(job.resolve,job.reject).finally(()=>{
        srhRailScheduler.active=Math.max(0,srhRailScheduler.active-1);
        if(job.lane==='background')srhRailScheduler.backgroundActive=Math.max(0,srhRailScheduler.backgroundActive-1);
        pumpRailQueue();
      });
    }
  }
  ...
  async function openContinueMovie(entry){
    const flowStartedAt=performance.now(),item=historyVodItem(entry);
    ...
    try{
      ...
      playbackDebug('continue-movie-ready',{key:entry.key,historySource:'memory',historyMs:0,requiredModalMs:Math.round(performance.now()-flowStartedAt),frameReady:!!frame,openPath:'continue'});
...
(function installTmdbEnrichment(){
  ...
  function installBranding(type,item,data){
    ...
    if(video&&video.dataset.srhBrandingMediaBound!=='1'){
      video.dataset.srhBrandingMediaBound='1';
  ...
  async function enrichVisibleSeason(tvId,season){
    ...
    try{
      ...
      eps.forEach((ep,idx)=>{
        ...
        const provider=providerEpisodeArtValue(ep);if(provider&&!ep.__providerEpisodeArt)ep.__providerEpisodeArt=provider;
        if(hit?.still_path)ep.__tmdbStill=imageUrl(hit.still_path,'w500')
      });
      ...
      eps.forEach(ep=>{
        const provider=providerEpisodeArtValue(ep),tmdb=String(ep.__tmdbStill||'').trim();
...
(function installStandardRecommendations(){
  ...
  function completeSimilarRows(rows){
    const list=Array.isArray(rows)?rows:[],count=list.length>=6?6:list.length>=3?3:0;
    return count?list.slice(0,count):[]
  }
  function renderSimilarRows(panel,rows,type){
    const grid=panel?.querySelector('.srh-similar-grid');if(!grid)return;
    rows=completeSimilarRows(rows);if(!rows.length){closeSimilar();return}
  ...
  function toggleSimilar(type,item,button){
    ...
    const rows=completeSimilarRows(button._srhRows||[]);
  ...
  function installSimilarButton(type,item,rows){
    ...
    const panel=el.detailBody.querySelector('.srh-similar-panel');
    if(panel&&b.classList.contains('is-active'))renderSimilarRows(panel,b._srhRows,type);
    return b
  }
  async function prepareSimilar(type,item,token){
    const source=sourceItem(type,item)||item,limit=6;
...
(function installMainStreamStyle(){
  ...
  async function collectCleanLogoHeroItems(targets,type,token,bucket,limit=10){
    ...
    for(let ti=0;ti<ordered.length&&out.length<limit;ti++){
      if(token!==state.renderToken||state.activeType!==type)return[];
      let raw=[];try{raw=await loadTargetItems(ordered[ti],token,{priority:-20,background:true})}catch{continue}
      if(token!==state.renderToken||state.activeType!==type)return[];
      categoriesScanned++;
  ...
  async function primeStoredHeroReserveType(type){
    ...
    if(!row||Number(row.version)!==HERO_PRECACHE_VERSION||row.targetSignature!==sig||Date.now()-Number(row.savedAt||0)>HERO_PRECACHE_TTL)return[];
    heroPrecacheMemory.set(key,row);
    const out=[];
    for(const entry of Array.isArray(row.entries)?row.entries:[]){
      const item=entry?.item,meta=entry?.meta;
      if(!item)continue;
    ...
    if(out.length)playbackDebug('hero-reserve-prime',{type,count:out.length,ageMs:Math.max(0,Date.now()-Number(row.savedAt||0))});
    return out
  }
  async function collectHeroReserveType(type,limit=HERO_PRECACHE_LIMIT){
    ...
    for(let ti=0;ti<targets.length&&out.length<limit;ti++){
      ...
      let raw=[];try{raw=await loadTargetItems(targets[ti],token,{priority:-40,background:true})}catch{continue}
      if(token!==state.renderToken)break;
      const sampled=stableShuffle(heroSample(raw,type,bucket+ti,72),type+':hero-reserve',bucket+ti);
      for(let i=0;i<sampled.length&&out.length<limit;i+=4){
        ...
        const ready=await Promise.all(batch.map(async item=>{
          ...
          if(!art)return null;
          const [logoOk,artOk]=await Promise.all([preloadHeroCleanLogo(logo,3200).catch(()=>false),preloadHeroImage(art,3200).catch(()=>false)]);
          return logoOk&&artOk?item:null
        }));
        for(const item of ready)if(item&&out.length<limit)out.push(item);
        if(token!==state.renderToken)break
      }
    }
    if(out.length)await writeHeroPrecache(type,out,targets);
  ...
  async function showHero(index,user=false){
    ...
    const cachedHero=type==='live'?null:heroMeta.get(heroMetaKey(item,type))||null;
    const cachedCatalog=type==='live'?null:heroCatalogFallback(item);
    const quickImage=type==='live'?heroImage(item,type,null):cachedHero?heroImage(item,type,cachedHero,cachedCatalog):'';
    if(type==='live'){
      setHeroBrand(node,quickTitle,null,local);
      metaNode.textContent='TV ao vivo';
    }else if(cachedHero?.logo&&quickImage){
      const cachedTitle=stripEmoji(cachedHero?.title||itemTitle(item),'Sem título'),cachedOverview=String(cachedHero?.overview||cachedCatalog?.overview||'').trim();
      setHeroBrand(node,cachedTitle,{...(cachedCatalog||{}),...cachedHero},local);
      plot.textContent=heroSynopsis(cachedOverview,200);
      metaNode.innerHTML=heroMetaMarkup(type,{...(cachedCatalog||{}),...cachedHero});
      playbackDebug('hero-reserve-fast-paint',{type,mediaId:String(itemId(item,type)||''),logo:true,art:true})
    }else{
      holdHeroBrand(node);
      plot.textContent='';
      metaNode.textContent='';
    }
    const quickPaint=quickImage
      ?paintHeroBackground(node,quickImage,type==='live',local).then(ok=>{if(ok&&local===heroToken)skeleton?.classList.add('is-hidden');return ok}).catch(()=>false)
  ...
  async function buildHero(token){
    ...
    if(type==='live'){
      ...
      for(let i=0;i<liveTargets.length&&lists.flat().length<32;i++){
        if(token!==state.renderToken)return;
        try{const raw=await loadTargetItems(liveTargets[i],token,{priority:-20,background:true});if(token!==state.renderToken)return;lists.push(heroSample(raw,type,bucket+i,i===0?72:48))}catch{}
      }
...
(function installStandardR56Guards(){
  if(el.detailClose){
    ...
    el.detailClose.onclick=async e=>{
      ...
      if(closing)return;
      closing=true;
      try{await closeDetail()}catch(err){playbackDebug('detail-close-failed',{message:err?.message||String(err)})}finally{closing=false}
    };
  }
  let scrollTimer=0;
