...
function persistProgress(video=el.video){if(!state.currentMedia||state.currentMedia.type==='live'||!Number.isFinite(video.duration)||video.duration<=0||!Number.isFinite(video.currentTime))return;markStandardLastWatched(state.currentMedia);if(video.currentTime/video.duration>=.97){markStandardCompleted(state.currentMedia,video.duration);removeHistory(state.currentMedia.key,state.currentMedia.type);return}saveHistory({...state.currentMedia,position:video.currentTime,duration:video.duration,updatedAt:Date.now()})}
