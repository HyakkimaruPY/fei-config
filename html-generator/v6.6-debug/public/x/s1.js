...
function attachVideo(video,url,onReady,onError,onAutoplayBlocked=null){
 ...
 const exhausted=async()=>{destroyHls();const terminalToken=state.mediaEpoch,err=last||{message:'mídia indisponível'};if(!isLive()&&!err.status&&lastUrl){const status=await finalProviderStatus(lastUrl);if(status>=400)err.status=status}if(terminalToken!==state.mediaEpoch)return;onError?.(err)};
 ...
 const start=async(current,attempt)=>{
  ...
  const onWaiting=()=>playbackDebug('playback-event',{attachId,event:'waiting',kind,elapsedMs:Date.now()-sourceStartedAt,video:playbackVideoSnapshot(video)});
  const onStalled=()=>playbackDebug('playback-event',{attachId,event:'stalled',kind,elapsedMs:Date.now()-sourceStartedAt,video:playbackVideoSnapshot(video)});
  const onPlaying=()=>{playbackDebug('playback-event',{attachId,event:'playing',kind,elapsedMs:Date.now()-sourceStartedAt,video:playbackVideoSnapshot(video)});markReady()};
  video.addEventListener('timeupdate',markAdvance,{passive:true});video.addEventListener('loadedmetadata',onLoadedMetadata,{passive:true});video.addEventListener('canplay',onCanPlay,{passive:true});video.addEventListener('playing',onPlaying,{passive:true});video.addEventListener('waiting',onWaiting,{passive:true});video.addEventListener('stalled',onStalled,{passive:true});
...
async function closePlayer(restoreFreeze=true){if(!state.playerActive)return;const media=state.currentMedia;el.video.onpause=null;playbackDebug('playback-overlay-teardown-begin',{mediaId:String(media?.key||''),video:playbackVideoSnapshot(el.video)});await persistProgress(el.video,{frame:true});destroyHls('overlay-close');resetPlaybackVideo(el.video);el.playerLayer.classList.add('is-hidden');state.currentMedia=null;state.playerActive=false;playbackDebug('playback-overlay-teardown-end',{mediaId:String(media?.key||''),video:playbackVideoSnapshot(el.video)});if(restoreFreeze)freezePage(!el.detailLayer.classList.contains('is-hidden')||!el.collectionView.classList.contains('is-hidden'));await leaveFullscreenPortrait()}
...
async function stopInlineSeriesVideo(opts=null){const sv=state.seriesVideo;if(!sv)return;const preserveGesture=!!opts?.preserveGesture,media=state.currentMedia;srhClearPlaybackFeedback(sv.art,{restoreBrand:true});sv.video.onpause=null;playbackDebug('playback-series-teardown-begin',{mediaId:String(media?.key||''),preserveGesture,video:playbackVideoSnapshot(sv.video)});const persistTask=Promise.resolve(persistProgress(sv.video,{frame:true})).catch(e=>playbackDebug('series-progress-save-failed',{message:e?.message||String(e)}));if(!preserveGesture)await persistTask;destroyHls('series-close');resetPlaybackVideo(sv.video);sv.video.classList.add('is-hidden');sv.image.classList.remove('is-hidden');sv.actions.classList.add('is-hidden');state.seriesVideo=null;state.currentMedia=null;playbackDebug('playback-series-teardown-end',{mediaId:String(media?.key||''),video:playbackVideoSnapshot(sv.video)});const leaveTask=Promise.resolve(leaveFullscreenPortrait()).catch(()=>{});if(!preserveGesture)await leaveTask}
