...
function vodSourcesFromInfo(data,item){
  ...
  return uniqueMediaUrls([direct,standard,hls])
}
...
const onDemandFormatPreference=new Map();
...
function preferOnDemandSources(values,type,pinned='',pinnedStartupMs=0){const list=uniqueMediaUrls(Array.isArray(values)?values:[values]);if(type==='live'||list.length<2)return list;const pin=String(pinned||'').trim(),base=pin?list.filter(x=>x!==pin):list.slice(),probe=base[0]||pin,pref=probe?onDemandFormatPreference.get(onDemandPreferenceKey(probe,type)):'',slowPin=!!pin&&Number(pinnedStartupMs)>6500;if(pref)base.sort((a,b)=>(mediaExtension(b)===pref?1:0)-(mediaExtension(a)===pref?1:0));if(slowPin)playbackDebug('slow-source-demoted',{type,host:mediaHost(pin),extension:mediaExtension(pin),lastStartupMs:Math.round(Number(pinnedStartupMs)||0),alternatives:base.length});return uniqueMediaUrls(slowPin?[...base,pin]:[pin,...base])}
...
function attachVideo(video,url,onReady,onError){
 ...
 const start=async(current,attempt)=>{
  destroyHls();const token=state.mediaEpoch,kind=mediaKind(current),sourceStartedAt=Date.now();let sourceFailed=false,sourceReady=false,retryTimer=0,watchdog=0,metadataTimer=0,liveStartupTimer=0,lastAdvanceAt=Date.now(),lastTime=0,hlsNetworkRecoveries=0,hlsMediaRecoveries=0;
...
function installFullscreenPlaybackUi(){
  ...
  style.textContent='.srh-custom-fs-active{background:#000!important;position:relative!important;overflow:hidden!important}.srh-custom-fs-active>video,.srh-custom-fs-active video.detail-inline-video,.srh-custom-fs-active #seriesInlineVideo,.srh-custom-fs-active #videoPlayer{position:absolute!important;inset:0!important;width:100%!important;height:100%!important;max-width:none!important;max-height:none!important;object-fit:contain!important;background:#000!important}.srh-custom-fs-active .detail-inline-status,.srh-custom-fs-active .detail-inline-actions,.srh-custom-fs-active #seriesVideoActions,.srh-custom-fs-active #playerExpand{display:none!important}.srh-fs-controls{position:absolute;z-index:2147483000;left:0;right:0;bottom:0;padding:0 max(12px,env(safe-area-inset-right)) max(12px,env(safe-area-inset-bottom)) max(12px,env(safe-area-inset-left));opacity:1;transform:translate3d(0,0,0);transition:opacity .18s ease,transform .18s ease;box-sizing:border-box}.srh-fs-controls.is-hidden{opacity:0;transform:translate3d(0,8px,0);pointer-events:none}.srh-fs-controls__panel{display:flex;align-items:center;gap:10px;width:min(980px,100%);margin:0 auto;padding:9px 10px;border:1px solid rgba(255,255,255,.14);border-radius:14px;background:rgba(12,14,18,.58);backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);box-shadow:0 6px 24px rgba(0,0,0,.18)}.srh-fs-controls button{flex:0 0 38px;width:38px;height:38px;display:grid;place-items:center;border:0;border-radius:10px;background:transparent;color:#fff}.srh-fs-controls button:active{background:rgba(255,255,255,.12)}.srh-fs-controls svg{width:21px;height:21px;fill:none;stroke:currentColor;stroke-width:1.9;stroke-linecap:round;stroke-linejoin:round}.srh-fs-controls .srh-fs-play svg{fill:currentColor;stroke:none}.srh-fs-controls__time{flex:0 0 auto;min-width:43px;color:#fff;font:600 11px/1 system-ui,-apple-system,Segoe UI,sans-serif;font-variant-numeric:tabular-nums;text-align:center}.srh-fs-controls__progress{flex:1 1 auto;min-width:70px;height:28px;margin:0;accent-color:#f5f7fa}.srh-fs-controls__progress::-webkit-slider-runnable-track{height:4px;border-radius:999px;background:rgba(255,255,255,.32)}.srh-fs-controls__progress::-webkit-slider-thumb{margin-top:-5px}.srh-custom-fs-active.srh-fs-cursor-hidden{cursor:none}@media(max-width:520px){.srh-fs-controls{padding-left:max(7px,env(safe-area-inset-left));padding-right:max(7px,env(safe-area-inset-right));padding-bottom:max(7px,env(safe-area-inset-bottom))}.srh-fs-controls__panel{gap:5px;padding:6px 7px;border-radius:11px}.srh-fs-controls button{width:34px;height:34px;flex-basis:34px}.srh-fs-controls__time{font-size:10px;min-width:37px}}';
  ...
  const mount=()=>{
    ...
    const ui=document.createElement('div');ui.className='srh-fs-controls';ui.innerHTML='<div class="srh-fs-controls__panel"><button type="button" class="srh-fs-play" aria-label="Reproduzir ou pausar"></button><span class="srh-fs-controls__time srh-fs-current">0:00</span><input class="srh-fs-controls__progress" type="range" min="0" max="1000" step="1" value="0" aria-label="Progresso do vídeo"><span class="srh-fs-controls__time srh-fs-duration">0:00</span><button type="button" class="srh-fs-exit" aria-label="Sair da tela cheia"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M9 4H4v5M15 4h5v5M9 20H4v-5M15 20h5v-5"/></svg></button></div>';
    root.appendChild(ui);
...
async function resolveModalDetailPackage(type,item,providerData){
  ...
  const brand={
    ...(tmdb||{}),
    id:tmdb?.id||String(info.tmdb_id||item?.tmdb_id||''),
    title,logo,backdrop:art,overview,year,vote,
