...
function attachVideo(video,url,onReady,onError,onAutoplayBlocked=null){
 ...
 const start=async(current,attempt)=>{
  ...
  const onLoadedMetadata=()=>playbackDebug('playback-event',{attachId,event:'loadedmetadata',kind,elapsedMs:Date.now()-sourceStartedAt,video:playbackVideoSnapshot(video)});
  const onCanPlay=()=>playbackDebug('playback-event',{attachId,event:'canplay',kind,elapsedMs:Date.now()-sourceStartedAt,video:playbackVideoSnapshot(video)});
  ...
  if(kind==='hls'){
   ...
   if(nativeHls){playbackDebug('hls-native-fallback',{attachId,origin:mediaOrigin(current)});video.src=current;video.onerror=()=>fail();video.onloadedmetadata=()=>{applyResume();requestMediaPlay(video,'native-hls-metadata',onAutoplayBlocked)};armMetadataTimer();try{video.load()}catch{};requestMediaPlay(video,'native-hls-initial',onAutoplayBlocked);return}
   fail({type:'hls-unavailable',details:'Hls.js/MSE e HLS nativo indisponíveis'});return
  }
  video.src=current;video.onerror=()=>fail();video.onloadedmetadata=()=>{applyResume();requestMediaPlay(video,'native-metadata',onAutoplayBlocked)};armMetadataTimer();try{video.load()}catch{};requestMediaPlay(video,'native-initial',onAutoplayBlocked)
 };
 next()
}
...
async function renderSeriesMetadataPreview(item,token){
  try{
    const fallback=seriesDetailFallback(item,null),pkg=await resolveModalDetailPackage('series',item,fallback);
    if(!isDetailCurrent(token)||state.srhSeriesEpisodes?.token!==token||state.srhSeriesEpisodes?.phase!=='episodes-loading')return null;
    const info=fallback.info||{},backdrop=pkg.art||IMAGE_PLACEHOLDER,plot=pkg.overview||'',title=pkg.title||itemTitle(item);
    state.currentSeries={item,data:fallback,episodes:{},backdrop,tmdbId:pkg.brand?.id||info.tmdb_id||null,tmdbBackdrop:String(pkg.brand?.tmdbBackdrop||pkg.brand?.backdrop||'').trim(),tmdbPoster:String(pkg.brand?.poster||'').trim(),providerBackdrop:String(pkg.brand?.providerBackdrop||'').trim(),providerCover:String(pkg.brand?.providerCover||'').trim(),episodeState:'episodes-loading'};
    state.srhDetailPackage={token,type:'series',mediaId:String(item.series_id||''),brand:pkg.brand};
    el.detailBody.innerHTML='<div class="detail-content srh-series-metadata-ready"><div class="series-static"><div class="detail-art" id="seriesArt"><img class="backdrop" id="seriesImage" src="'+escapeHtml(backdrop)+'" alt="" decoding="async"><video class="is-hidden" id="seriesInlineVideo" controls autoplay playsinline></video><div class="detail-art__actions is-hidden" id="seriesVideoActions"><button class="floating-action" id="seriesStop" type="button" title="Fechar vídeo"><svg class="ui-svg" viewBox="0 0 24 24"><path d="M6.5 6.5l11 11M17.5 6.5l-11 11"/></svg></button><button class="floating-action" id="seriesExpand" type="button" title="Expandir ou minimizar"><svg class="ui-svg" viewBox="0 0 24 24"><path d="M8.5 4.5h-4v4M15.5 4.5h4v4M8.5 19.5h-4v-4M15.5 19.5h4v-4"/></svg></button></div></div><div class="detail-title-row"><h2 class="detail-title">'+escapeHtml(title)+'</h2></div><div class="synopsis" id="seriesSynopsis">'+synopsisMarkup(plot,false)+'</div><div class="season-box"><button class="season-trigger" id="seasonTrigger" type="button" disabled><span id="seasonLabel">Carregando episódios…</span><span class="season-trigger__icon" aria-hidden="true">'+UI_ICON_CHEVRON_DOWN+'</span></button><div class="season-menu is-hidden" id="seasonMenu"></div></div></div><div class="episode-container"><div class="episode-list srh-series-episodes-loading" id="episodeList">'+seriesEpisodeLoadingMarkup()+'</div></div></div>';
    const image=el.detailBody.querySelector('#seriesImage'),syn=el.detailBody.querySelector('#seriesSynopsis');
    if(image)applyManagedImageChain(image,[backdrop,pkg.brand?.backdrop,pkg.brand?.providerBackdrop,pkg.brand?.poster,pkg.brand?.providerCover,item?.backdrop_path,item?.backdrop,item?.cover_big,item?.cover,item?.stream_icon],{label:'series-metadata-preview',eager:true});
    setSynopsisState(syn,plot,false);if(syn)syn.onclick=e=>{e.stopPropagation();setSynopsisState(syn,plot,!state.synopsisExpanded)};
    if(window.__srhInstallDetailBranding&&pkg.brand){window.__srhInstallDetailBranding('series',item,pkg.brand);state.srhDetailBrandingToken=token}
    setSeriesEpisodesState(token,'metadata-ready',{mediaId:String(item?.series_id||'')});
    setSeriesEpisodesState(token,'episodes-loading',{mediaId:String(item?.series_id||'')});
    return pkg
  }catch(e){playbackDebug('series-metadata-preview-failed',{mediaId:String(item?.series_id||''),message:e?.message||String(e)});return null}
}
