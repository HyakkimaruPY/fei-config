...
function continueFrameKey(entry){
  ...
  return 'srhell:'+STANDARD_APP_NS+':standard:continue-frame:v'+CONTINUE_FRAME_VERSION+':'+provider+':'+encodeURIComponent(key);
}
...
function persistProgress(video=el.video,opts=null){
  ...
  markStandardLastWatched(state.currentMedia);
  if(video.currentTime/video.duration>=.97){
    markStandardCompleted(state.currentMedia,video.duration);
    removeHistory(state.currentMedia.key,state.currentMedia.type);
    return
  }
  const row={...state.currentMedia,position:video.currentTime,duration:video.duration,updatedAt:Date.now(),frameKey:continueFrameKey(state.currentMedia),frameVersion:CONTINUE_FRAME_VERSION};
  saveHistory(row);
...
(function installLiteUiOrganization(){
  ...
  if(!document.getElementById('srhContinueFrameStyle')){
    ...
    s.textContent='body.srh-main-stream-style .home-status{height:0!important;min-height:0!important;margin:0!important;padding:0!important;line-height:0!important;overflow:hidden!important;opacity:0!important}body.srh-main-stream-style .stream-hero{margin-top:-12px!important}.continue-card__media{position:relative;width:100%;overflow:hidden;background:#080a0c}.continue-card__media>img,.continue-card__frame-video{display:block;width:100%;height:100%;object-fit:cover;pointer-events:none}.continue-card__frame-video{background:#080a0c}';
