...
const liveFormatPreference=new Map();
function preferredLiveSources(url){const stable=String(url||''),host=mediaHost(stable),preferred=liveFormatPreference.get(host);if(!preferred||preferred===mediaExtension(stable))return[stable];const alternate=liveExtensionFallback(stable);return alternate&&mediaExtension(alternate)===preferred?uniqueMediaUrls([alternate,stable]):[stable]}
function rememberLiveFormat(url){const host=mediaHost(url),ext=mediaExtension(url);if(!host||!['m3u8','ts'].includes(ext))return;const prior=liveFormatPreference.get(host)||'';liveFormatPreference.set(host,ext);if(prior!==ext)playbackDebug('live-format-learn',{host,extension:ext,previous:prior||'none'})}
...
function attachVideo(video,url,onReady,onError){
 ...
 const start=async(current,attempt)=>{
  ...
  const markReady=()=>{if(!alive())return;clearTimeout(metadataTimer);clearTimeout(liveStartupTimer);const mediaId=String(video.dataset.srhMediaId||state.currentMedia?.key||''),sourceKind=String(video.dataset.srhSourceKind||kind);if(state.currentMedia&&state.currentMedia.type!=='live'){state.currentMedia.lastWorkingUrl=current;state.currentMedia.containerExtension=mediaExtension(current)||state.currentMedia.containerExtension||''}else if(isLive()){rememberLiveFormat(current);try{window.SRHDebug?.noteRecovery?.({kind:'playlist',layer:'fetch',origin:mediaOrigin(current),status:200,message:'Formato alternativo de canal iniciou a reprodução',details:{sourceKind,extension:mediaExtension(current)}})}catch{}}lastAdvanceAt=Date.now();markAdvance();try{window.SRHDebug?.noteRecovery?.({kind:'player.stall',layer:'media',origin:mediaOrigin(current),mediaId,status:200,message:'Reprodução retomada',details:{kind,sourceKind,currentTime:Number(video.currentTime)||0}})}catch{}applyResume();if(failedOrigins.size){for(const origin of failedOrigins){try{window.SRHDebug?.noteRecovery?.({kind:'player.error',layer:'media',origin,mediaId,status:200,message:'Fonte alternativa iniciou a reprodução',details:{fallbackCount,sourceKind,winningOrigin:mediaOrigin(current)}});window.SRHDebug?.noteRecovery?.({kind:'player.stall',layer:'media',origin,mediaId,status:200,message:'Fonte alternativa retomou a reprodução',details:{fallbackCount,sourceKind,winningOrigin:mediaOrigin(current)}})}catch{}}playbackDebug('recovered',{mediaId,sourceKind,fallbackCount,failedOrigins:[...failedOrigins],winningOrigin:mediaOrigin(current)});failedOrigins.clear()}if(!readyNotified){readyNotified=true;onReady?.(current)}playbackDebug('ready',{kind,sourceKind,mediaId,attempt,live:isLive(),preload:video.preload});const p=video.play();if(p&&typeof p.catch==='function')p.catch(()=>{})};
  const fail=extra=>{
    ...
    const failureType=String(extra?.type||extra?.details||'').toLowerCase(),structuralLiveFailure=last.code===4||/manifest|level|loader|network|cors|src[_ -]?not[_ -]?supported/.test(failureType),liveRetryLimit=kind==='hls'||structuralLiveFailure?0:1;
  ...
  if(kind==='hls'){
   ...
   if(!alive())return;
   if(H?.isSupported?.()){
    try{const hlsStart=!isLive()&&resumeAt>1?resumeAt:-1,hlsCfg=isLive()?{enableWorker:true,lowLatencyMode:false,startLevel:-1,capLevelToPlayerSize:false,startFragPrefetch:true,maxBufferLength:12,maxMaxBufferLength:20,backBufferLength:8,liveSyncDurationCount:2,liveMaxLatencyDurationCount:5,maxLiveSyncPlaybackRate:1.0}:{enableWorker:true,progressive:true,autoStartLoad:true,startFragPrefetch:true,lowLatencyMode:false,startLevel:-1,startPosition:hlsStart,capLevelToPlayerSize:false,maxBufferLength:24,maxMaxBufferLength:36,backBufferLength:20,liveSyncDurationCount:3,liveMaxLatencyDurationCount:8};state.hls=new H(hlsCfg);state.hls.loadSource(current);state.hls.attachMedia(video);state.hls.on(H.Events.MANIFEST_PARSED,()=>{if(!alive())return;playbackDebug('hlsjs-manifest',{live:isLive(),origin:mediaOrigin(current),startPosition:hlsStart,progressive:!isLive()});const p=video.play();if(p&&typeof p.catch==='function')p.catch(()=>{})});state.hls.on(H.Events.ERROR,(_,d)=>{if(!d?.fatal||!alive())return;const hlsDetails=String(d?.details||''),hlsStatus=d?.response?.code||d?.response?.status||d?.networkDetails?.status||d?.networkDetails?.statusCode||0,startupPlaylistFailure=isLive()&&!readyNotified&&d.type===H.ErrorTypes.NETWORK_ERROR&&/(manifest|level)(load|parsing|empty|timeout|error)/i.test(hlsDetails);if(startupPlaylistFailure){playbackDebug('hls-playlist-fast-fallback',{details:hlsDetails,status:hlsStatus,origin:mediaOrigin(current)});fail({type:'hls-playlist-startup',details:hlsDetails||'playlist indisponível',status:hlsStatus});return}if(isLive()&&d.type===H.ErrorTypes.NETWORK_ERROR&&hlsNetworkRecoveries<1){hlsNetworkRecoveries++;playbackDebug('hls-network-recover',{count:hlsNetworkRecoveries,details:hlsDetails});try{state.hls.startLoad()}catch{fail(d)}return}if(isLive()&&d.type===H.ErrorTypes.MEDIA_ERROR&&hlsMediaRecoveries<1){hlsMediaRecoveries++;playbackDebug('hls-media-recover',{count:hlsMediaRecoveries,details:hlsDetails});try{state.hls.recoverMediaError()}catch{fail(d)}return}fail({type:d?.type||'hls-fatal',details:hlsDetails,status:hlsStatus})});return}catch(e){fail({type:'hls-init',details:e?.message||String(e)});return}
   }
