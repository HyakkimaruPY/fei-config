...
const STANDARD_DB='srhell-standard-state-v1',STANDARD_STORE='state',STANDARD_COMPLETED_KEY=`srhell:${STANDARD_APP_NS}:standard:completed:v1`;
let standardDbPromise=null,standardWriteTail=Promise.resolve(),standardWritesQueued=0,standardWritesDone=0;
const standardCompletedMemory={};
...
async function hydrateStandardRuntime(){const legacy=loadRuntime(),stored=await standardStateGet(storageKey()),runtime=stored&&typeof stored==='object'?stored:legacy;if(runtime&&typeof runtime==='object'){CONFIG={...BASE_CONFIG,...runtime};if(!stored)void standardStateSet(storageKey(),runtime)}try{localStorage.removeItem(storageKey())}catch{}const completed=await standardStateGet(STANDARD_COMPLETED_KEY);if(completed&&typeof completed==='object')Object.assign(standardCompletedMemory,completed);return runtime}
function getStandardCompleted(key){return standardCompletedMemory[String(key||'')]||null}
function markStandardCompleted(entry,duration){if(!entry||entry.type!=='series'||!entry.key)return false;standardCompletedMemory[entry.key]={key:entry.key,seriesId:entry.seriesId,season:entry.season,episodeNumber:entry.episodeNumber,title:entry.title,image:entry.image,containerExtension:entry.containerExtension||'',itemSnapshot:entry.itemSnapshot||null,duration:Number(duration)||Number(entry.duration)||0,position:Number(duration)||Number(entry.duration)||0,completedAt:Date.now()};const rows=Object.values(standardCompletedMemory).sort((a,b)=>Number(b.completedAt||0)-Number(a.completedAt||0)).slice(0,800),next={};for(const row of rows)next[row.key]=row;for(const k of Object.keys(standardCompletedMemory))delete standardCompletedMemory[k];Object.assign(standardCompletedMemory,next);void standardStateSet(STANDARD_COMPLETED_KEY,next);return true}
...
function attachVideo(video,url,onReady,onError){
 ...
 const start=async(current,attempt)=>{
  ...
  const markReady=()=>{if(!alive())return;clearTimeout(metadataTimer);lastAdvanceAt=Date.now();markAdvance();try{window.SRHDebug?.noteRecovery?.({kind:'player.stall',layer:'media',origin:mediaOrigin(current),mediaId:'',status:200,message:'Reprodução retomada',details:{kind,currentTime:Number(video.currentTime)||0}})}catch{}if(resumeAt>1&&Number.isFinite(video.duration)&&resumeAt<video.duration-3){try{video.currentTime=resumeAt}catch{}}if(failedOrigins.size){for(const origin of failedOrigins){try{window.SRHDebug?.noteRecovery?.({kind:'player.error',layer:'media',origin,mediaId:'',status:200,message:'Fonte alternativa iniciou a reprodução',details:{fallbackCount,winningOrigin:mediaOrigin(current)}});window.SRHDebug?.noteRecovery?.({kind:'player.stall',layer:'media',origin,mediaId:'',status:200,message:'Fonte alternativa retomou a reprodução',details:{fallbackCount,winningOrigin:mediaOrigin(current)}})}catch{}}playbackDebug('recovered',{fallbackCount,failedOrigins:[...failedOrigins],winningOrigin:mediaOrigin(current)});failedOrigins.clear()}if(!readyNotified){readyNotified=true;onReady?.(current)}playbackDebug('ready',{kind,attempt,live:isLive(),preload:video.preload});const p=video.play();if(p&&typeof p.catch==='function')p.catch(()=>{})};
  const fail=extra=>{if(!alive()||sourceFailed)return;sourceFailed=true;clearTimeout(metadataTimer);fallbackCount++;failedOrigins.add(mediaOrigin(current));if(Number.isFinite(video.currentTime)&&video.currentTime>1)resumeAt=video.currentTime;last=mediaErrorInfo(video,extra);playbackDebug('fallback',{kind,attempt,live:isLive(),message:last.message,code:last.code||0,fallbackCount,origin:mediaOrigin(current)});const liveRetryLimit=kind==='hls'?0:1;if(isLive()&&attempt<liveRetryLimit){retryTimer=setTimeout(()=>start(current,attempt+1),900+attempt*450);return}if(!isLive()&&last.code===4&&kind==='native'){const ext=mediaExtension(current),origin=mediaOrigin(current);let skipped=0;while(index<queue.length&&mediaKind(queue[index])==='native'&&mediaExtension(queue[index])===ext&&mediaOrigin(queue[index])===origin){index++;skipped++}if(skipped)playbackDebug('skip-equivalent',{reason:'src-not-supported',ext,origin,skipped})}next()};
  ...
  if(isLive()&&(kind==='mpegts'||kind==='m2ts')){
  ...
  if(kind==='hls'){
   ...
   if(!alive())return;
   if(H?.isSupported?.()){
    try{state.hls=new H({enableWorker:true,lowLatencyMode:false,startLevel:-1,capLevelToPlayerSize:false,maxBufferLength:30,backBufferLength:30,liveSyncDurationCount:3,liveMaxLatencyDurationCount:8});state.hls.loadSource(current);state.hls.attachMedia(video);state.hls.on(H.Events.MANIFEST_PARSED,()=>{if(!alive())return;playbackDebug('hlsjs-manifest',{live:isLive(),origin:mediaOrigin(current)});const p=video.play();if(p&&typeof p.catch==='function')p.catch(()=>{})});state.hls.on(H.Events.ERROR,(_,d)=>{if(!d?.fatal||!alive())return;if(d.type===H.ErrorTypes.NETWORK_ERROR&&hlsNetworkRecoveries<2){hlsNetworkRecoveries++;playbackDebug('hls-network-recover',{count:hlsNetworkRecoveries,details:d?.details||''});try{state.hls.startLoad()}catch{fail(d)}return}if(d.type===H.ErrorTypes.MEDIA_ERROR&&hlsMediaRecoveries<1){hlsMediaRecoveries++;playbackDebug('hls-media-recover',{count:hlsMediaRecoveries,details:d?.details||''});try{state.hls.recoverMediaError()}catch{fail(d)}return}fail({type:d?.type||'hls-fatal',details:d?.details||''})});return}catch(e){fail({type:'hls-init',details:e?.message||String(e)});return}
   }
...
function persistProgress(video=el.video){if(!state.currentMedia||state.currentMedia.type==='live'||!Number.isFinite(video.duration)||video.duration<=0||!Number.isFinite(video.currentTime))return;if(video.currentTime/video.duration>=.97){markStandardCompleted(state.currentMedia,video.duration);removeHistory(state.currentMedia.key,state.currentMedia.type);return}saveHistory({...state.currentMedia,position:video.currentTime,duration:video.duration,updatedAt:Date.now()})}
...
(function installLiteUiOrganization(){
  ...
  function writeHistoryBucket(type,list){const bucket=type==='series'?'series':'vod',compact=(Array.isArray(list)?list:[]).slice(0,60);contMemory[bucket]=compact;void standardStateSet(historyKey(bucket),compact);return true}
  ...
  function mergeRows(primary,legacy){const map=new Map();for(const row of [...(Array.isArray(primary)?primary:[]),...(Array.isArray(legacy)?legacy:[])]){const key=String(row?.key||row?.id||'');if(!key||map.has(key))continue;map.set(key,row)}return[...map.values()]}
  window.__srhHydrateStandardPersonal=async()=>{
    const dbFav=await standardStateGet(FAVORITES_KEY),legacyFav=legacyList(FAVORITES_KEY),fav=mergeRows(dbFav,legacyFav).slice(0,160);
    window[FAVORITES_MEMORY_KEY]=fav;void standardStateSet(FAVORITES_KEY,fav);
    for(const type of ['vod','series']){
      const key=historyKey(type),dbRows=await standardStateGet(key),legacy=legacyList(key),rows=mergeRows(dbRows,legacy).sort((a,b)=>Number(b?.updatedAt||0)-Number(a?.updatedAt||0)).slice(0,60);
...
init();
setTimeout(()=>ensureHls().then(()=>playbackDebug('hlsjs-warm',{version:window.Hls?.version||''})).catch(e=>playbackDebug('hlsjs-warm-failed',{message:e?.message||String(e)})),700);
})();
})();
