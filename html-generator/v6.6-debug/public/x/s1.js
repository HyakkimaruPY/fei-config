...
async function requestSeriesEpisodesBounded(item,{force=false,budgetMs=10500}={}){
  const mediaId=String(item?.series_id??item?.id??''),params={action:'get_series_info',series_id:mediaId},provider=normalizeServer(CONFIG.server),seen=new Set(),routes=[];
  ...
  while(performance.now()<deadline){
    ...
    try{
      ...
      if(candidate.base)xuiRouteState.preferred.set(provider+'|get_series_info',{base:candidate.base,endpoint:candidate.endpoint,proxied:candidate.proxied});
      playbackDebug('series-episodes-route-success',{mediaId,transport:candidate.kind,endpoint:candidate.endpoint,origin:(()=>{try{return new URL(candidate.url).origin}catch{return''}})(),ms,seasons:stats.seasons.length,episodes:stats.episodes,budgetUsedMs:Math.round(performance.now()-startedAll)});
      ...
      return stats.data
    }catch(e){last=e;playbackDebug('series-episodes-route-failed',{mediaId,transport:candidate.kind,endpoint:candidate.endpoint,origin:(()=>{try{return new URL(candidate.url).origin}catch{return''}})(),ms:Math.round(performance.now()-started),message:e?.message||String(e),remainingMs:Math.max(0,Math.round(deadline-performance.now()))})}
  }
  ...
  window.SRHDebug?.noteFailure?.({kind:'series.episodes',action:'get_series_info',mediaId,origin:provider,layer:'fetch',ms:elapsed,message:error?.message||String(error),details:{budgetMs:Math.round(Number(budgetMs)||10500),routesTried:index}});
  throw error
}
...
async function renderSeriesRecoveryModal(item,error,token){
  try{
    ...
    if(retry)retry.onclick=e=>{e.stopPropagation();retry.disabled=true;retry.textContent='Carregando…';void openSeries(item,{forceEpisodes:true}).catch(err=>{retry.disabled=false;retry.textContent='Tentar carregar episódios';playbackDebug('series-retry-failed',{message:err?.message||String(err)})})};
...
async function retrySeriesEpisodesOnly(item,token,button){
  ...
  try{const data=await loadSeriesDetailStable(item,{force:true});if(!isDetailCurrent(token))return;seriesProviderGuardSuccess();await openSeries(item,{preloadedData:data,reuseModal:true,detailToken:token})}
  catch(e){if(!isDetailCurrent(token))return;seriesProviderGuardFailure(e,item);renderSeriesEpisodesFailure(item,e,token,'episodes-error')}
}
