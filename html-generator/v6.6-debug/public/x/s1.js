...
function vodSourcesFromInfo(data,item){
  ...
  const declaredExt=String(movie?.container_extension||info?.container_extension||item?.container_extension||'mp4').replace(/^\./,'').toLowerCase();
  const direct=normalizeMediaUrl(movie?.direct_source||info?.direct_source||data?.direct_source||'');
  const standard=mediaUrlWithExtension('vod',id,declaredExt);
  const hls=mediaUrlWithExtension('vod',id,'m3u8');
  /* r40-compatible order: HLS first, then provider direct_source, then the
     container declared by get_vod_info. Do not fabricate MKV for every film. */
  return uniqueMediaUrls([hls,direct,standard])
}
...
let mediaPrimer={video:null,key:'',url:'',startedAt:0,readyAt:0,error:false,timer:0};
function primerNativeSource(sources){
  const list=mediaCandidates(Array.isArray(sources)?sources:[sources]);
  const preferred=list.find(url=>mediaKind(url)==='native'&&['mp4','m4v','webm','mkv','mov'].includes(mediaExtension(url)));
  return preferred||list.find(url=>mediaKind(url)==='native')||''
}
function clearMediaPrimer(exceptVideo=null){
  const p=mediaPrimer;
  clearTimeout(p.timer);
  if(p.video&&p.video!==exceptVideo){
    p.video.onloadedmetadata=null;p.video.onerror=null;
    try{p.video.pause();p.video.removeAttribute('src');p.video.load()}catch{}
  }
  mediaPrimer={video:null,key:'',url:'',startedAt:0,readyAt:0,error:false,timer:0}
}
function primeMediaMetadata(sources,key=''){
  const url=primerNativeSource(sources);
  if(!url)return null;
  if(mediaPrimer.video&&mediaPrimer.key===String(key||'')&&mediaPrimer.url===url&&!mediaPrimer.error)return mediaPrimer.video;
  clearMediaPrimer();
  const video=document.createElement('video');
  video.muted=true;video.volume=0;video.playsInline=true;video.autoplay=false;video.controls=false;video.preload='metadata';
  video.dataset.srhPrimerUrl=url;video.dataset.srhPrimerKey=String(key||'');
  const startedAt=performance.now();
  mediaPrimer={video,key:String(key||''),url,startedAt,readyAt:0,error:false,timer:0};
  warmMediaOrigin(url);
  video.onloadedmetadata=()=>{
    if(mediaPrimer.video!==video)return;
    mediaPrimer.readyAt=performance.now();
    playbackDebug('metadata-prime-ready',{key:String(key||''),origin:mediaOrigin(url),ext:mediaExtension(url),ms:Math.round(mediaPrimer.readyAt-startedAt),readyState:video.readyState})
  };
  video.onerror=()=>{
    if(mediaPrimer.video!==video)return;
    mediaPrimer.error=true;
    playbackDebug('metadata-prime-error',{key:String(key||''),origin:mediaOrigin(url),ext:mediaExtension(url),code:video.error?.code||0})
  };
  mediaPrimer.timer=setTimeout(()=>{
    if(mediaPrimer.video===video&&!mediaPrimer.readyAt&&!mediaPrimer.error)playbackDebug('metadata-prime-slow',{key:String(key||''),origin:mediaOrigin(url),ext:mediaExtension(url),ms:Math.round(performance.now()-startedAt),readyState:video.readyState})
  },9000);
  try{video.src=url;video.load();playbackDebug('metadata-prime-start',{key:String(key||''),origin:mediaOrigin(url),ext:mediaExtension(url)})}catch{mediaPrimer.error=true}
  return video
}
function takeMediaPrimer(sources,key=''){
  const p=mediaPrimer,allowed=mediaCandidates(Array.isArray(sources)?sources:[sources]);
  if(!p.video||p.error||String(key||'')!==p.key||!allowed.includes(p.url)){
    if(p.video)clearMediaPrimer();
    return null
  }
  clearTimeout(p.timer);
  const out={video:p.video,url:p.url,readyState:p.video.readyState,ageMs:Math.round(performance.now()-p.startedAt)};
  p.video.onloadedmetadata=null;p.video.onerror=null;
  mediaPrimer={video:null,key:'',url:'',startedAt:0,readyAt:0,error:false,timer:0};
  playbackDebug('metadata-prime-adopt',{key:String(key||''),origin:mediaOrigin(out.url),ext:mediaExtension(out.url),readyState:out.readyState,ageMs:out.ageMs});
  return out
}
...
function attachVideo(video,url,onReady,onError){
 ...
 const start=async(current,attempt)=>{
  ...
  const primerMatch=kind==='native'&&String(video.dataset.srhPrimerUrl||'')===String(current)&&!video.error&&((video.currentSrc&&video.currentSrc===current)||video.getAttribute('src')===current);
  lastUrl=current;
  if(!primerMatch){
    video.pause();video.onerror=null;video.onloadedmetadata=null;video.removeAttribute('src');video.preload='auto';try{video.load()}catch{}
  }else{
    video.onerror=null;video.onloadedmetadata=null;video.preload='auto';
    playbackDebug('metadata-prime-reuse',{mediaId:String(state.currentMedia?.key||''),origin:mediaOrigin(current),ext:mediaExtension(current),readyState:video.readyState})
  }
  video.dataset.srhMediaId=String(state.currentMedia?.key||'');video.dataset.srhMediaType=String(state.currentMedia?.type||'');video.dataset.srhSourceKind=kind+(mediaExtension(current)?':'+mediaExtension(current):'');video.dataset.srhMediaKind=kind;video.dataset.srhContainerExtension=mediaExtension(current);warmMediaOrigin(current)
  ...
  const fail=extra=>{
    ...
    const ext=mediaExtension(current),origin=mediaOrigin(current);
    ...
    if(!isLive()&&last.code===4&&kind==='native'){
      let skipped=0;
      while(index<queue.length&&mediaKind(queue[index])==='native'&&mediaExtension(queue[index])===ext&&mediaOrigin(queue[index])===origin){index++;skipped++}
      if(skipped)playbackDebug('skip-equivalent',{reason:'src-not-supported',ext,origin,skipped})
    }
    next()
  };
  ...
  if(!primerMatch)video.src=current;
  video.onerror=()=>fail();
  video.onloadedmetadata=()=>{const p=video.play();if(p&&typeof p.catch==='function')p.catch(()=>{})};
  if(!primerMatch)try{video.load()}catch{}
...
async function openFilm(item){openDetail(itemTitle(item));const token=state.detailToken;await waitDetailSkeletonPaint(token);if(!isDetailCurrent(token))return;try{const data=await requestDetail({action:'get_vod_info',vod_id:item.stream_id});if(!isDetailCurrent(token))return;const info=data?.info||{},movie=data?.movie_data||item,art=(info.backdrop_path&&Array.isArray(info.backdrop_path)?info.backdrop_path[0]:info.backdrop_path)||info.movie_image||movie.stream_icon||imageFor(item,'vod'),title=movie.name||itemTitle(item),plot=info.plot||info.description||movie.plot||item.plot||'',sources=vodSourcesFromInfo(data,item),url=sources[0]||streamUrl('vod',{...movie,stream_id:movie.stream_id||item.stream_id,container_extension:movie.container_extension||item.container_extension}),entry={key:'vod:'+(movie.stream_id||item.stream_id),type:'vod',title,image:art,url,sources,tmdbId:data?.__tmdb?.id||info?.tmdb_id||null,containerExtension:String(movie.container_extension||info.container_extension||item.container_extension||'mp4').toLowerCase(),itemSnapshot:{stream_id:movie.stream_id||item.stream_id,name:title,stream_icon:movie.stream_icon||item.stream_icon,movie_image:info.movie_image||movie.movie_image||'',cover:item.cover||'',cover_big:info.cover_big||'',backdrop_path:info.backdrop_path||'',tmdb_id:data?.__tmdb?.id||info?.tmdb_id||null,container_extension:movie.container_extension||info.container_extension||item.container_extension||'mp4'},position:0,duration:0};sources.forEach(warmMediaOrigin);state.currentDetail={type:'vod',entry};if(state.srhOpeningContinue&&state.srhContinueFramePromise){await state.srhContinueFramePromise.catch(()=>null);if(!isDetailCurrent(token))return}const displayArt=state.srhOpeningContinue&&state.srhContinueFrameUrl?state.srhContinueFrameUrl:art;el.detailBody.innerHTML=`<div class="detail-content"><div class="detail-art"><img src="${escapeHtml(displayArt||IMAGE_PLACEHOLDER)}" alt=""></div><div class="detail-title-row"><h2 class="detail-title">${escapeHtml(title)}</h2><button class="watch-button" id="watchFilm"><svg class="watch-svg" viewBox="0 0 24 24" aria-hidden="true"><path d="M8 5.8v12.4L18 12z"/></svg><span>Assistir</span></button></div><div class="synopsis" id="filmSynopsis">${synopsisMarkup(plot,false)}</div></div>`;const filmSynopsis=$('#filmSynopsis');setSynopsisState(filmSynopsis,plot,false);filmSynopsis.onclick=e=>{e.stopPropagation();setSynopsisState(filmSynopsis,plot,!state.synopsisExpanded)};const oldFilm=getHistory('vod').find(x=>x.key===entry.key),filmPlaybackSources=uniqueMediaUrls([oldFilm?.lastWorkingUrl,...entry.sources]);primeMediaMetadata(filmPlaybackSources,entry.key);$('#watchFilm').onclick=()=>{const old=getHistory('vod').find(x=>x.key===entry.key);openGeneralPlayer(entry.sources||url,title,old?{...entry,...old,tmdbId:entry.tmdbId||old.tmdbId||old.itemSnapshot?.tmdb_id||null,itemSnapshot:{...(old.itemSnapshot||{}),...(entry.itemSnapshot||{})},sources:entry.sources}:entry)}}catch(e){if(!isDetailCurrent(token))return;el.detailBody.innerHTML='<div class="rail-load-error"><span>'+escapeHtml(e.message)+'</span><button type="button">Tentar novamente</button></div>';el.detailBody.querySelector('button').onclick=()=>openItem(item,item.series_id!==undefined?'series':'vod')}}
...
function srhFixOverlayPlayer(url,title,entry=null){
  const sources=mediaCandidates(entry?.lastWorkingUrl?[entry.lastWorkingUrl,...(Array.isArray(entry?.sources)?entry.sources:[entry?.sources||url])]:entry?.sources||url);
...
function srhFixInlinePlayer(url,title,entry=null){
  ...
  let sources=mediaCandidates(entry?.lastWorkingUrl?[entry.lastWorkingUrl,...(Array.isArray(entry?.sources)?entry.sources:[entry?.sources||url])]:entry?.sources||url);
  const image=art.querySelector('img'),primer=entry?.key?takeMediaPrimer(sources,entry.key):null;
  if(primer)sources=uniqueMediaUrls([primer.url,...sources]);
  const video=primer?.video||document.createElement('video');
