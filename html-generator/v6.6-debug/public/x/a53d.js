...
function sentinelNeedsMore(){if(activeLibrary||shown>=S.state.filtered.length)return false;const sentinel=$('#feedSentinel');if(!sentinel?.isConnected)return false;let rect;try{rect=sentinel.getBoundingClientRect()}catch{return false}const vh=Math.max(1,window.innerHeight||document.documentElement.clientHeight||1);return rect.top<=vh+700&&rect.bottom>=-160}
function scheduleFeedPump(delay=36){clearTimeout(feedPumpTimer);feedPumpTimer=setTimeout(()=>{feedPumpTimer=0;if(isScrollBusy()){scheduleFeedPump(180);return}if(sentinelNeedsMore())appendBatch()},Math.max(0,delay))}
function appendBatch(){
  ...
  if(shown<list.length&&sentinelNeedsMore()){if('requestIdleCallback'in window)requestIdleCallback(()=>scheduleFeedPump(0),{timeout:120});else scheduleFeedPump(40)}
}
function reset(){shown=0;clearTimeout(feedPumpTimer);$('#shortGrid').replaceChildren();ensureImageObserver();appendBatch();scheduleFeedPump(48)}
...
function applyRenderItems(items){S.state.items=items;S.state.filtered=items.slice();S.reconcileHistoryWithCatalog?.(items);reconcileFeed(S.state.filtered);S.refreshLibraryView?.();$('#appMeta').textContent=items.length+' Shorts · '+((S.cfg.targets||[]).filter(t=>t.type==='vod').length)+' categoria(s)';scheduleFeedPump(44);if(!S.__variantApplying)window.dispatchEvent(new CustomEvent('srh25:catalog-render',{detail:{items:items.slice(),incremental:true}}));return true}
...
S.bindShell=()=>{
  ...
  observer=new IntersectionObserver(es=>{if(es.some(e=>e.isIntersecting)&&shown<S.state.filtered.length)scheduleFeedPump(0)},{rootMargin:'700px 0px'});observer.observe($('#feedSentinel'))
};
S.loadCatalog=async()=>{
