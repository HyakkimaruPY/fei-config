...
function feedPumpAllowed(){
  if(activeLibrary||document.visibilityState==='hidden'||shown>=S.state.filtered.length)return false;
  if(isScrollBusy()||Number(S.__shortsInteractionBusyUntil||0)>performance.now())return false;
  const p=$('#shortPlayer');if(p&&!p.classList.contains('is-hidden'))return false;
  return true
}
...
function scheduleFeedPump(delay=170){
  ...
  feedPumpTimer=setTimeout(()=>{
    feedPumpTimer=0;
    const run=()=>{if(!feedPumpAllowed())return;if(sentinelNeedsMore())appendBatch()};
    if('requestIdleCallback'in window)requestIdleCallback(run,{timeout:500});else setTimeout(run,36)
  },Math.max(90,delay))
}
function appendBatch(){
  if(isScrollBusy()||Number(S.__shortsInteractionBusyUntil||0)>performance.now()){clearTimeout(appendDeferTimer);appendDeferTimer=setTimeout(()=>scheduleFeedPump(90),210);return}
...
async function scan(items){
  ...
  try{
    ...
    await parallel(rows,async item=>{if(generation!==scanGeneration||scanUiBusy())return null;const h=await hashCover(coverUrl(item));if(h)hashes.set(S.id(item),h);return h},2);
...
function schedule(items,delay=2200){
  ...
  const run=()=>{
    ...
    if(current!==pendingScanSignature){schedule(sourceItems,2200);return}
    if(scanUiBusy()){scanTimer=setTimeout(run,900);return}
    const launch=()=>{if(!scanUiBusy()&&catalogSignature(sourceItems)===pendingScanSignature)scan(sourceItems)};
    if('requestIdleCallback'in window)requestIdleCallback(launch,{timeout:1600});else setTimeout(launch,80)
  };
  scanTimer=setTimeout(run,Math.max(900,delay))
}
window.addEventListener('srh25:scroll-start',()=>{scanGeneration++;clearTimeout(scanTimer);clearTimeout(scrollRestartTimer)});
window.addEventListener('srh25:scroll-idle',()=>{clearTimeout(scrollRestartTimer);scrollRestartTimer=setTimeout(()=>{const sig=catalogSignature(sourceItems);if(sourceItems.length&&sig&&sig!==lastScannedSignature)schedule(sourceItems,1200)},900)});
function ensureUi(){
...
function shieldInteraction(e){
  if(!e.target?.closest?.('.srh25-card,#settingsButton,#historyButton,#favoritesButton'))return;
