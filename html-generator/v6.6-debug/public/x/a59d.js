...
let arcDragStartY=null,arcButtons=[],arcCount=0;
...
function setArcButtonState(index,on){const b=arcButtons[index];if(!b)return;b.classList.toggle('active',on);if(on)b.setAttribute('aria-current','true');else b.removeAttribute('aria-current')}
function centerActiveArc(active){
  if(!active||arcDrawer.classList.contains('is-hidden'))return;
  requestAnimationFrame(()=>{
    if(arcDrawer.classList.contains('is-hidden')||!active.isConnected)return;
    let a,g;try{a=active.getBoundingClientRect();g=arcGrid.getBoundingClientRect()}catch{return}
    if(a.top>=g.top+6&&a.bottom<=g.bottom-6)return;
    const delta=(a.top-g.top)-Math.max(0,(arcGrid.clientHeight-a.height)/2);
    arcGrid.scrollTop=Math.max(0,arcGrid.scrollTop+delta)
  })
}
function syncArcSelection(scroll=false){
  if(arcDrawer.classList.contains('is-hidden'))return;
  const index=arcIndex();if(index===activeArc&&!scroll)return;
  const previous=activeArc;if(previous>=0&&previous!==index)setArcButtonState(previous,false);
  activeArc=index;const active=arcButtons[index]||null;if(active)setArcButtonState(index,true);
  if(scroll)centerActiveArc(active)
}
function drawArcs(){
  const d=video.duration;
  if(!Number.isFinite(d)||d<=0){
    if(arcCount!==-1){arcCount=-1;arcButtons=[];arcGrid.innerHTML='<div class="srh25-empty">Aguardando duração…</div>'}
    return
  }
  const n=Math.max(1,Math.ceil(d/120));
  if(arcCount===n&&arcButtons.length===n&&arcButtons[0]?.isConnected){if(!arcDrawer.classList.contains('is-hidden'))syncArcSelection(false);return}
  const frag=document.createDocumentFragment(),buttons=new Array(n);
  for(let i=0;i<n;i++){
    const start=i*120,b=document.createElement('button');b.className='srh25-arc';b.dataset.arcIndex=String(i);
    const strong=document.createElement('strong');strong.textContent='Arco '+(i+1);b.appendChild(strong);
    b.onclick=()=>{const duration=Number(video.duration)||0;video.currentTime=Math.min(Math.max(0,duration-.1),start);activeArc=i;closeArcs();video.play().catch(()=>{})};
    buttons[i]=b;frag.appendChild(b)
  }
  arcGrid.replaceChildren(frag);arcButtons=buttons;arcCount=n;activeArc=-1;
  if(!arcDrawer.classList.contains('is-hidden'))syncArcSelection(false)
}
function openArcs(){
  ensureArcSheetChrome();drawArcs();arcBackdrop.classList.remove('is-hidden');arcDrawer.classList.remove('is-hidden');activeArc=-1;
  requestAnimationFrame(()=>syncArcSelection(true))
}
...
S.openPlayer=(item,resumeOverride)=>{
  ...
  activeArc=-1;
  arcCount=0;arcButtons=[];arcGrid.replaceChildren();
  hideMiniResume();
...
'use strict';
const S=window.SRH25;
if(!S||S.__srhShortsPerfA58)return;
S.__srhShortsPerfA58=true;
const style=document.createElement('style');
style.id='srh25A58ArcsStyle';
style.textContent=[
  '#arcBackdrop.srh25-arc-backdrop{display:block!important;visibility:visible;opacity:1;pointer-events:auto;backdrop-filter:none!important;-webkit-backdrop-filter:none!important;background:rgba(0,0,0,.42);transition:opacity .16s ease;contain:strict}',
  '#arcBackdrop.srh25-arc-backdrop.is-hidden{display:block!important;visibility:hidden;opacity:0;pointer-events:none}',
  '#arcDrawer.srh25-arcs{display:flex!important;flex-direction:column;height:min(76dvh,590px);max-height:none!important;visibility:visible;opacity:1;pointer-events:auto;transform:translate3d(0,0,0);animation:none!important;transition:transform .18s cubic-bezier(.22,1,.36,1),opacity .14s ease;backdrop-filter:none!important;-webkit-backdrop-filter:none!important;background:rgba(10,14,19,.985);box-shadow:0 -12px 36px rgba(0,0,0,.42);contain:layout paint style;will-change:transform,opacity}',
  '#arcDrawer.srh25-arcs.is-hidden{display:flex!important;visibility:hidden;opacity:0;pointer-events:none;transform:translate3d(0,22px,0)}',
  '#arcDrawer .srh25-arcs__grid{flex:1 1 auto;min-height:0;max-height:none!important;contain:layout paint style;will-change:scroll-position}',
  '#arcDrawer .srh25-arc{contain:layout paint style}'
].join('');
document.head.appendChild(style);
try{
  window.SRHDebug?.state.patch('shorts',{
    arcsTuning:{
      version:'a58-resident-cached-arcs-v1',
      resident:true,
      cachedButtons:true,
      selectionUpdate:'previous-current-only',
      autoCenter:'scrollTop-if-needed',
      backdropBlur:false
    }
  });
}catch{}
...
'use strict';
const S=window.SRH25;
if(!S||S.__srhShortsPerfA59)return;
S.__srhShortsPerfA59=true;
try{
  window.SRHDebug?.state.patch('shorts',{
    miniResumeTuning:{
      version:'a59-stable-mini-geometry-v1',
      playerExitMode:'compact-direct',
      delayedAutoCompact:false,
      dragPosition:true
    }
  });
}catch{}
