...
function observeVideos(){
  ...
  const trace=m=>{let x=traces.get(m);if(!x){x={loadAt:0,metadataMs:0,firstPlaying:false,waitingAt:0,startupBufferMs:0,stallTimer:0,origin:''};traces.set(m,x)}return x};
  const mediaOrigin=m=>{try{return new URL(m.currentSrc||m.src||'',location.href).origin}catch{return'media'}};
  document.addEventListener('loadstart',e=>{if(e.target instanceof HTMLMediaElement){const x=trace(e.target);clearTimeout(x.stallTimer);x.loadAt=performance.now();x.metadataMs=0;x.firstPlaying=false;x.waitingAt=0;x.startupBufferMs=0;x.origin=mediaOrigin(e.target);log('info','media.loadstart',{origin:x.origin})}},true);
...
function installPerformanceTracing(){
  ...
  try{
    const longObserver=new PerformanceObserver(list=>{for(const e of list.getEntries()){const ms=Math.round(e.duration);state.performance.longTasks++;state.performance.longestMs=Math.max(state.performance.longestMs,ms);patch('performance',{longTasks:state.performance.longTasks,longestMs:state.performance.longestMs,slowResources:state.performance.slowResources});if(ms>=250)noteSlow({kind:'main-thread',layer:'performance',origin:'page',ms,message:'Main thread bloqueada',details:{name:e.name,start:Math.round(e.startTime)}});else if(ms>=120)log('info','performance.longtask',{ms,name:e.name,start:Math.round(e.startTime)})}});
