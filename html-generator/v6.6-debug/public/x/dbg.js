...
function observeVideos(){
  ...
  const trace=m=>{let x=traces.get(m);if(!x){x={loadAt:0,metadataMs:0,firstPlaying:false,waitingAt:0,startupBufferMs:0,stallTimer:0,origin:'',mediaId:'',retrying:false};traces.set(m,x)}return x};
  const mediaOrigin=m=>{try{return new URL(m.currentSrc||m.src||'',location.href).origin}catch{return'media'}};
  document.addEventListener('loadstart',e=>{if(e.target instanceof HTMLMediaElement){const x=trace(e.target);clearTimeout(x.stallTimer);x.loadAt=performance.now();x.metadataMs=0;x.firstPlaying=false;x.waitingAt=0;x.startupBufferMs=0;x.origin=mediaOrigin(e.target);x.mediaId=String(window.SRH25?.state?.current?.item?(window.SRH25.id?.(window.SRH25.state.current.item)||window.SRH25.state.current.item.stream_id||window.SRH25.state.current.item.id||''):'');log('info','media.loadstart',{origin:x.origin,mediaId:x.mediaId,preload:e.target.preload})}},true);
  ...
  document.addEventListener('loadedmetadata',e=>{if(e.target instanceof HTMLMediaElement){const x=trace(e.target);x.metadataMs=x.loadAt?Math.round(performance.now()-x.loadAt):0;player.attach(e.target);player.ready({duration:e.target.duration,src:safeText(e.target.currentSrc||'')});log('info','player.metadata',{origin:x.origin||mediaOrigin(e.target),mediaId:x.mediaId,ms:x.metadataMs,preload:e.target.preload,readyState:e.target.readyState,networkState:e.target.networkState})}},true);
  document.addEventListener('playing',e=>{if(e.target instanceof HTMLMediaElement){const x=trace(e.target),t=performance.now();if(x.waitingAt){const wait=Math.round(t-x.waitingAt);clearTimeout(x.stallTimer);if(!x.firstPlaying)x.startupBufferMs+=wait;else{noteRecovery({kind:'player.stall',layer:'media',origin:x.origin||mediaOrigin(e.target),ms:wait,status:200});if(wait>1200)noteSlow({kind:'player.stall',layer:'media',origin:x.origin||mediaOrigin(e.target),ms:wait,message:'Buffering perceptível',details:{currentTime:Number(e.target.currentTime)||0}})}x.waitingAt=0}if(!x.firstPlaying){x.firstPlaying=true;const ms=x.loadAt?Math.round(t-x.loadAt):0;if(ms>4000){const metadataSharePct=ms?Math.round(((x.metadataMs||0)/ms)*100):0;noteSlow({kind:'player.startup',layer:'media',mediaId:x.mediaId||'',origin:x.origin||mediaOrigin(e.target),ms,message:'Vídeo demorou para iniciar',details:{phase:'first-playing',metadataMs:x.metadataMs||0,metadataSharePct,startupBufferMs:x.startupBufferMs||0,currentTime:Number(e.target.currentTime)||0,resumed:Number(e.target.currentTime||0)>1,preload:e.target.preload,readyState:e.target.readyState,networkState:e.target.networkState,dominant:metadataSharePct>=75?'metadata':'buffer'}})}else log('info','player.startup_ok',{origin:x.origin||mediaOrigin(e.target),mediaId:x.mediaId,ms,metadataMs:x.metadataMs||0,startupBufferMs:x.startupBufferMs||0,preload:e.target.preload})}patch('player',{status:'playing'})}},true);
  ...
  window.addEventListener('srh25:media-error-final',e=>{const d=e.detail||{},code=Number(d.code)||0,codeName=code===4?'MEDIA_ERR_SRC_NOT_SUPPORTED':'MEDIA_ERR_'+code,retryAttempted=!!d.retryAttempted,retrySuppressed=!!d.retrySuppressed,vodInfoChecked=!!d.vodInfoChecked,noAlternative=!!d.noAlternative,message=retryAttempted?'Erro de mídia após fallback do provedor':noAlternative?'Erro de mídia; provedor não informou alternativa compatível':retrySuppressed?'Erro de mídia; retry já esgotado para este item':'Erro de mídia';noteFailure({kind:'player.error',layer:'media',mediaId:String(d.mediaId||''),origin:d.origin||'media',message,details:{code,codeName,attempts:d.attempts||1,retryAttempted,retrySuppressed,vodInfoChecked,noAlternative,catalogExtension:d.catalogExtension||'',currentExtension:d.currentExtension||'',sourceKind:d.sourceKind||'',canPlayType:d.canPlayType||'',currentTime:Number(d.currentTime)||0,readyState:d.readyState||0,networkState:d.networkState||0}});player.error(new Error(message))},{passive:true});
