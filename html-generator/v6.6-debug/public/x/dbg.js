...
function observeVideos(){
  ...
  document.addEventListener('loadstart',e=>{if(e.target instanceof HTMLMediaElement){const x=trace(e.target);clearTimeout(x.stallTimer);x.loadAt=performance.now();x.metadataMs=0;x.metadataReady=false;x.preMetadataWaitMs=0;x.firstPlaying=false;x.waitingAt=0;x.startupBufferMs=0;x.origin=mediaOrigin(e.target);x.mediaId=String(window.SRH25?.state?.current?.item?(window.SRH25.id?.(window.SRH25.state.current.item)||window.SRH25.state.current.item.stream_id||window.SRH25.state.current.item.id||''):'');log('info','media.loadstart',{origin:x.origin,mediaId:x.mediaId,preload:e.target.preload})}},true);
