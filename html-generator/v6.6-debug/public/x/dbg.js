...
function foregroundNetworkMeta(meta={}){
  if(meta.kind!=='playlist'&&meta.kind!=='media')return meta;
  let media=null;
  try{
    const nodes=[...document.querySelectorAll('video[data-srh-media-id],audio[data-srh-media-id]')].filter(m=>m?.dataset?.srhBackgroundMedia!=='1'&&!m?.matches?.('.srh-resume-frame-worker,.continue-card__frame-video,.srh-resume-preview-video,.srh-probe-media'));
    media=nodes.find(m=>String(m?.dataset?.srhMediaId||'')&&(!m.classList?.contains('is-hidden')||m.currentSrc||m.src))||nodes.find(m=>String(m?.dataset?.srhMediaId||''))||null
  }catch{}
  const mediaId=String(media?.dataset?.srhMediaId||meta.mediaId||''),sourceKind=String(media?.dataset?.srhSourceKind||''),mediaType=String(media?.dataset?.srhMediaType||'');
  return{...meta,mediaId,details:{...(meta.details||{}),source:meta.url||'',path:meta.path||'',sourceKind,mediaType}}
}
...
function observeVideos(){
  ...
  document.addEventListener('error',e=>{if(e.target instanceof HTMLMediaElement){if(backgroundMedia(e.target)){log('info','media.background_error',{code:e.target.error?.code||0,tag:e.target.className||e.target.id||'media'});return;}const x=trace(e.target),code=e.target.error?.code||0,codeName=code===1?'MEDIA_ERR_ABORTED':code===2?'MEDIA_ERR_NETWORK':code===3?'MEDIA_ERR_DECODE':code===4?'MEDIA_ERR_SRC_NOT_SUPPORTED':'MEDIA_ERR_UNKNOWN';clearTimeout(x.stallTimer);if(code===4&&e.target.id==='shortVideo'){log('warn','media.error.retryable',{code,codeName,origin:x.origin||mediaOrigin(e.target),readyState:e.target.readyState,networkState:e.target.networkState});return}noteFailure({kind:'player.error',layer:'media',mediaId:x.mediaId||String(e.target.dataset?.srhMediaId||''),origin:x.origin||mediaOrigin(e.target),message:'Erro de mídia',details:{code,codeName,currentTime:Number(e.target.currentTime)||0,readyState:e.target.readyState,networkState:e.target.networkState,sourceKind:x.sourceKind||mediaSourceKind(e.target)}});player.error(new Error('media error'))}},true);
