...
function observeVideos(){
  ...
  const trace=m=>{let x=traces.get(m);if(!x){x={loadAt:0,metadataMs:0,firstPlaying:false,waitingAt:0,startupBufferMs:0,stallTimer:0,origin:'',mediaId:'',prefetchState:'none',prefetchAgeMs:0,retrying:false};traces.set(m,x)}return x};
  const mediaOrigin=m=>{try{return new URL(m.currentSrc||m.src||'',location.href).origin}catch{return'media'}};
  document.addEventListener('loadstart',e=>{if(e.target instanceof HTMLMediaElement){const x=trace(e.target);clearTimeout(x.stallTimer);x.loadAt=performance.now();x.metadataMs=0;x.firstPlaying=false;x.waitingAt=0;x.startupBufferMs=0;x.origin=mediaOrigin(e.target);x.mediaId=String(e.target.dataset?.srhMediaId||window.SRH25?.state?.current?.item?(window.SRH25.id?.(window.SRH25.state.current.item)||window.SRH25.state.current.item.stream_id||window.SRH25.state.current.item.id||''):'');x.prefetchState=String(e.target.dataset?.srhPrefetchState||'none');x.prefetchAgeMs=Math.max(0,Number(e.target.dataset?.srhPrefetchAge)||0);log('info','media.loadstart',{origin:x.origin,mediaId:x.mediaId,preload:e.target.preload,prefetchState:x.prefetchState,prefetchAgeMs:x.prefetchAgeMs})}},true);
