...
function observeVideos(){
  ...
  const trace=m=>{let x=traces.get(m);if(!x){x={loadAt:0,metadataMs:0,metadataReady:false,preMetadataWaitMs:0,firstPlaying:false,waitingAt:0,startupBufferMs:0,stallTimer:0,origin:'',mediaId:'',retrying:false};traces.set(m,x)}return x};
  const mediaOrigin=m=>{const tagged=String(m?.dataset?.srhMediaOrigin||'');if(tagged&&tagged!=='null')return tagged;try{return new URL(m.currentSrc||m.src||'',location.href).origin}catch{return'media'}};
  document.addEventListener('loadstart',e=>{if(e.target instanceof HTMLMediaElement){const x=trace(e.target);clearTimeout(x.stallTimer);x.loadAt=performance.now();x.metadataMs=0;x.metadataReady=false;x.preMetadataWaitMs=0;x.firstPlaying=false;x.waitingAt=0;x.startupBufferMs=0;x.origin=mediaOrigin(e.target);x.mediaId=String(e.target.dataset?.srhMediaId||(window.SRH25?.state?.current?.item?(window.SRH25.id?.(window.SRH25.state.current.item)||window.SRH25.state.current.item.stream_id||window.SRH25.state.current.item.id||''):''));x.sourceKind=String(e.target.dataset?.srhMediaKind||'');log('info','media.loadstart',{origin:x.origin,mediaId:x.mediaId,sourceKind:x.sourceKind,preload:e.target.preload})}},true);
...
function installGlobalErrors(){addEventListener('error',e=>{
  ...
  const err=e.error,message=String(e.message||err?.message||'Erro JavaScript sem mensagem'),meta={kind:'javascript',layer:'window',origin:'page',message,details:{name:err?.name||'',source:safeUrl(e.filename||''),line:e.lineno||0,col:e.colno||0,stack:safeText(err?.stack||'').slice(0,2400)}};
  if(/^ResizeObserver loop (?:limit exceeded|completed with undelivered notifications\.?)/i.test(message)){log('warn','window.resize_observer_loop',meta);return}
