...
window.addEventListener('srh25:catalog-render',()=>warmMediaOrigin(cfg.server),{once:true});
async function loadVideo(item,resume){
  const token=++mediaLoadToken,url=S.stream(item),hasResume=Number(resume?.position)>0,origin=(()=>{try{return new URL(url,location.href).origin}catch{return'media'}})();let retryCount=0,retryPending=false,retryRecovered=false;
  const reveal=()=>{if(token!==mediaLoadToken)return;poster.classList.add('is-hidden');loading.classList.add('is-hidden');if(retryCount>0&&!retryRecovered){retryRecovered=true;mediaEvent('srh25:media-recovered',{attempts:retryCount,origin,src:url})}};
  ...
  const directError=()=>{if(token!==mediaLoadToken||retryPending)return;const code=video.error?.code||0;if(code===4&&retryCount<1){retryCount++;retryPending=true;mediaEvent('srh25:media-retry',{code,attempt:retryCount,origin,src:url});poster.classList.remove('is-hidden');loading.classList.remove('is-hidden');try{video.pause();video.removeAttribute('src');video.load()}catch{}setTimeout(()=>{if(token!==mediaLoadToken)return;retryPending=false;video.src=url;video.onloadedmetadata=ready;video.onerror=directError;video.load();if(!hasResume)video.play().catch(()=>{})},700);return}mediaEvent('srh25:media-error-final',{code,attempts:retryCount+1,origin,src:url,currentTime:Number(video.currentTime)||0,readyState:video.readyState,networkState:video.networkState});S.toast('Mídia indisponível')};
