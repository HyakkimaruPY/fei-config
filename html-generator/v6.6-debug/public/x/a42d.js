...
function card(item,options={}){
  ...
  b.onclick=()=>S.openPlayer?.(item);b.__srhItem=item;ensureMediaPrefetchObserver();mediaPrefetchObserver?.observe(b);
...
function ensureMediaPrefetchObserver(){
  if(mediaPrefetchObserver||!('IntersectionObserver'in window))return;
  mediaPrefetchObserver=new IntersectionObserver(es=>{for(const e of es)if(e.isIntersecting){const card=e.target,item=card.__srhItem;mediaPrefetchObserver.unobserve(card);if(item)S.queueMediaPrefetch?.(item)}},{rootMargin:'900px 0px',threshold:0.01})
}
...
let mediaLoadToken=0;const mediaRetryExhausted=new Set(),mediaPrefetchState=new Map(),mediaPrefetchQueue=[];let mediaPrefetchActive=null;
function mediaPrefetchTrim(){const rows=[...mediaPrefetchState.entries()].filter(([,x])=>x?.status==='ready').sort((a,b)=>(b[1].readyAt||0)-(a[1].readyAt||0));for(const [id,x] of rows.slice(2)){try{x.video?.pause();x.video?.removeAttribute('src');x.video?.load()}catch{}mediaPrefetchState.delete(id)}}
function mediaPrefetchRun(){if(mediaPrefetchActive||!mediaPrefetchQueue.length)return;const item=mediaPrefetchQueue.shift(),id=String(S.id?.(item)||item?.stream_id||item?.id||'');if(!id||mediaRetryExhausted.has(id)){queueMicrotask(mediaPrefetchRun);return}const url=S.stream(item);if(/\.m3u8(?:$|\?)/i.test(url)){queueMicrotask(mediaPrefetchRun);return}const v=document.createElement('video'),startedAt=Date.now(),state={status:'pending',startedAt,readyAt:0,failedAt:0,video:v};mediaPrefetchState.set(id,state);mediaPrefetchActive={id,video:v};v.preload='metadata';v.muted=true;v.playsInline=true;v.dataset.srhPrefetch='1';const finish=status=>{if(mediaPrefetchActive?.id===id)mediaPrefetchActive=null;state.status=status;if(status==='ready')state.readyAt=Date.now();else state.failedAt=Date.now();clearTimeout(timer);if(status!=='ready')try{v.removeAttribute('src');v.load()}catch{}mediaPrefetchTrim();queueMicrotask(mediaPrefetchRun)};v.onloadedmetadata=()=>finish('ready');v.onerror=()=>finish('error');const timer=setTimeout(()=>finish('timeout'),12000);try{v.src=url;v.load()}catch{finish('error')}}
S.queueMediaPrefetch=item=>{const id=String(S.id?.(item)||item?.stream_id||item?.id||'');if(!id||mediaRetryExhausted.has(id)||S.state.current)return false;const old=mediaPrefetchState.get(id);if(old&&(old.status==='pending'||(old.status==='ready'&&Date.now()-old.readyAt<60000)))return false;if(mediaPrefetchQueue.some(x=>String(S.id?.(x)||x?.stream_id||x?.id||'')===id))return false;if(mediaPrefetchQueue.length>=2)return false;mediaPrefetchQueue.push(item);mediaPrefetchRun();return true};
S.mediaPrefetchInfo=id=>{const x=mediaPrefetchState.get(String(id||''));if(!x)return{status:'none',ageMs:0};const age=x.status==='ready'?Math.max(0,Date.now()-x.readyAt):Math.max(0,Date.now()-x.startedAt);return{status:x.status,ageMs:age,startedAt:x.startedAt,readyAt:x.readyAt}};
S.cancelMediaPrefetch=id=>{id=String(id||'');if(mediaPrefetchActive?.id!==id)return false;const x=mediaPrefetchActive;mediaPrefetchActive=null;try{x.video.pause();x.video.removeAttribute('src');x.video.load()}catch{}const state=mediaPrefetchState.get(id);if(state)state.status='cancelled';queueMicrotask(mediaPrefetchRun);return true};
...
async function loadVideo(item,resume){
  const token=++mediaLoadToken,url=S.stream(item),mediaKey=String(S.id?.(item)||item?.stream_id||item?.id||url),prefetch=S.mediaPrefetchInfo?.(mediaKey)||{status:'none',ageMs:0},hasResume=Number(resume?.position)>0,isHls=/\.m3u8(?:$|\?)/i.test(url),origin=(()=>{try{return new URL(url,location.href).origin}catch{return'media'}})();let retryCount=0,retryPending=false,retryRecovered=false;video.dataset.srhMediaId=mediaKey;video.dataset.srhPrefetchState=prefetch.status||'none';video.dataset.srhPrefetchAge=String(prefetch.ageMs||0);if(prefetch.status==='pending')S.cancelMediaPrefetch?.(mediaKey);
  ...
  const directError=()=>{if(token!==mediaLoadToken||retryPending)return;const code=video.error?.code||0,exhausted=mediaRetryExhausted.has(mediaKey);if(code===4&&retryCount<1&&!exhausted){retryCount++;retryPending=true;mediaRetryExhausted.add(mediaKey);mediaEvent('srh25:media-retry',{code,attempt:retryCount,origin,mediaId:mediaKey});poster.classList.remove('is-hidden');loading.classList.remove('is-hidden');try{video.pause();video.removeAttribute('src');video.load()}catch{}setTimeout(()=>{if(token!==mediaLoadToken)return;retryPending=false;video.src=url;video.onloadedmetadata=ready;video.onerror=directError;video.preload=isHls?'auto':'metadata';video.load();if(isHls&&!hasResume)video.play().catch(()=>{})},700);return}mediaEvent('srh25:media-error-final',{code,attempts:retryCount>0?2:1,retryAttempted:retryCount>0,retrySuppressed:code===4&&exhausted&&retryCount===0,origin,mediaId:mediaKey,currentTime:Number(video.currentTime)||0,readyState:video.readyState,networkState:video.networkState});S.toast('Mídia indisponível')};
  const attachDirect=()=>{if(token!==mediaLoadToken)return;video.src=url;video.onloadedmetadata=ready;video.onerror=directError;video.preload=isHls?'auto':'metadata';video.load();if(isHls&&!hasResume)video.play().catch(()=>{})};
  warmMediaOrigin(url);video.preload=isHls?'auto':'metadata';video.addEventListener('playing',reveal,{once:true});
  if(isHls){
    if(video.canPlayType('application/vnd.apple.mpegurl')){attachDirect();return}
    try{const H=await ensureHls();if(H?.isSupported?.()){const h=new H({enableWorker:true,maxBufferLength:30,capLevelToPlayerSize:false,startLevel:-1});S.state.hls=h;h.loadSource(url);h.attachMedia(video);h.on(H.Events.MANIFEST_PARSED,()=>{if(token!==mediaLoadToken)return;const highest=Math.max(0,(h.levels?.length||1)-1);h.autoLevelCapping=highest;h.currentLevel=highest;h.nextLevel=highest;ready()});h.on(H.Events.LEVEL_SWITCHED,()=>{const highest=Math.max(0,(h.levels?.length||1)-1);if(h.currentLevel!==highest)h.nextLevel=highest});h.on(H.Events.ERROR,(_,d)=>{if(d.fatal){mediaEvent('srh25:media-error-final',{code:0,attempts:1,origin,mediaId:mediaKey,hls:true,details:d?.details||''});S.toast('Falha ao iniciar o vídeo')}});return}}catch(e){S.toast(e?.message||'HLS indisponível')}
  }
  attachDirect()
}
