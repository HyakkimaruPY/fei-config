...
const BATCH=18,LIBRARY_BATCH=14,CACHE_MAX_AGE=6*60*60*1000;let shown=0,observer=null,imgObserver=null,activeLibrary=null,updateCandidate=null,updateCategories=[],updateSelected=new Set(),libraryRenderToken=0,scrollPerfTimer=0,renderDeferTimer=0,renderPendingItems=null,appendDeferTimer=0,imageFlushTimer=0,scrollPerfActive=false;const pendingImageLoads=new Set();
...
S.bindShell=()=>{
  ensureImageObserver();ensureUpdateUi();ensurePerformanceTuningUi();document.addEventListener('scroll',markShortsScrollBusy,{passive:true,capture:true});
...
async function scan(items){
  ...
  try{
    ...
    await parallel(rows,async item=>{if(generation!==scanGeneration||scanScrollBusy())return null;const h=await hashCover(coverUrl(item));if(h)hashes.set(S.id(item),h);return h},2);
...
function schedule(items){
  ...
  const run=()=>{scanTimer=0;if(scanScrollBusy()){scanTimer=setTimeout(run,260);return}scan(sourceItems)};
  if('requestIdleCallback'in window)requestIdleCallback(run,{timeout:2400});else scanTimer=setTimeout(run,800)
}
window.addEventListener('srh25:scroll-start',()=>{scanGeneration++;clearTimeout(scrollRestartTimer)});
window.addEventListener('srh25:scroll-idle',()=>{clearTimeout(scrollRestartTimer);scrollRestartTimer=setTimeout(()=>{if(sourceItems.length)schedule(sourceItems)},140)});
function ensureUi(){
...
const S=window.SRH25;
if(!S||S.__srhShortsPerfA51)return;
S.__srhShortsPerfA51=true;

const VERSION='a51-scroll-media-v1';
const $=selector=>document.querySelector(selector);

function installPerfCss(){
  if(document.getElementById('srh25A51PerfStyle'))return;
  const style=document.createElement('style');
  style.id='srh25A51PerfStyle';
  style.textContent=[
...
function watchSettings(){
  ...
  if(!panel||panel.dataset.srhA51Watch)return;
  panel.dataset.srhA51Watch='1';
  const sync=()=>document.body.classList.toggle('srh25-settings-open',!panel.classList.contains('is-hidden'));
...
function tunePlayerPreload(){
  ...
  if(!video||video.dataset.srhA51Load)return;
  video.dataset.srhA51Load='1';
...
function canWarm(){
  ...
  const player=$('#shortPlayer');
  if(player&&!player.classList.contains('is-hidden'))return false;
  return true;
}
...
function stopWarm(){
  ...
  if(warmVideo){
    try{warmVideo.pause()}catch{}
    try{warmVideo.removeAttribute('src');warmVideo.load()}catch{}
  }
  warmVideo=null;warmId='';warmReady=false;
}

function centerItem(){
  const cards=[...document.querySelectorAll('#shortGrid .srh25-card[data-id]')];
  if(!cards.length)return null;
  const vh=Math.max(1,window.innerHeight||document.documentElement.clientHeight||1),center=vh/2;
  ...
  if(!best)return null;
  const id=String(best.dataset.id||'');
  if(!id)return null;
  return (S.state.filtered||S.state.items||[]).find(item=>String(S.id(item))===id)
    ||(S.state.items||[]).find(item=>String(S.id(item))===id)
    ||null;
}
...
function warmCenter(){
  warmTimer=0;
  if(!canWarm())return;
  const item=centerItem(),id=item?String(S.id(item)):'';
  if(!item||!id||id===warmId)return;
  const url=String(S.stream?.(item)||'').trim();
  if(!url)return;
  stopWarm();
  warmId=id;warmReady=false;warmStarts++;
  const v=document.createElement('video');
  warmVideo=v;
  v.preload='metadata';v.muted=true;v.playsInline=true;
  v.onloadedmetadata=()=>{if(v===warmVideo)warmReady=true};
  v.onerror=()=>{if(v===warmVideo){warmReady=false}};
  try{v.src=url;v.load()}catch{stopWarm()}
}

function scheduleWarm(delay=900){
  if(warmTimer)return;
  warmTimer=setTimeout(function check(){
    const quietFor=performance.now()-lastScrollAt;
    if(quietFor<700){warmTimer=setTimeout(check,Math.max(120,720-quietFor));return}
    warmTimer=0;
    if('requestIdleCallback'in window)requestIdleCallback(()=>warmCenter(),{timeout:700});
    else setTimeout(warmCenter,0);
  },delay);
}
...
window.addEventListener('srh25:catalog-render',()=>scheduleWarm(1100));

const originalOpen=S.openPlayer;
if(typeof originalOpen==='function'){
  S.openPlayer=(item,resume)=>{
    const id=String(S.id(item)||'');
    if(id&&id===warmId&&warmReady)warmHits++;
    stopWarm();
    return originalOpen(item,resume);
  };
}
...
try{
  window.SRHDebug?.state.patch('shorts',{
    performanceTuning:{
      version:VERSION,
      scrollBackdrop:'disabled-while-moving',
      settingsPanel:'resident-composited',
      directVodPreload:'auto-on-load',
      metadataWarmup:'single-center-card-idle',
      warmStats:()=>({starts:warmStarts,hits:warmHits})
    }
  });
}catch{}
