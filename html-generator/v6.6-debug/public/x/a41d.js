...
async function loadVideo(item,resume){
  const token=++mediaLoadToken,url=S.stream(item),mediaKey=String(S.id?.(item)||item?.stream_id||item?.id||url),hasResume=Number(resume?.position)>0,isHls=/\.m3u8(?:$|\?)/i.test(url),origin=(()=>{try{return new URL(url,location.href).origin}catch{return'media'}})();let retryCount=0,retryPending=false,retryRecovered=false;
